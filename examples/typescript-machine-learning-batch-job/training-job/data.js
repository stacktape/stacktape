...
import { readFile, existsSync, createWriteStream } from 'fs-extra';
import { util as _util, tensor4d, oneHot, tensor1d } from '@tensorflow/tfjs-node';
...
async function loadImages(filename) {
  ...
  strictEqual(headerValues[0], IMAGE_HEADER_MAGIC_NUM);
  strictEqual(headerValues[2], IMAGE_HEIGHT);
  strictEqual(headerValues[3], IMAGE_WIDTH);
  ...
  strictEqual(images.length, headerValues[1]);
  return images;
}
...
async function loadLabels(filename) {
  ...
  const headerValues = loadHeaderValues(buffer, headerBytes);
  strictEqual(headerValues[0], LABEL_HEADER_MAGIC_NUM);
  ...
  strictEqual(labels.length, headerValues[1]);
  return labels;
}
...
class MnistDataset {
  ...
  getData_(isTrainingData) {
    ...
    const images = new Float32Array(_util.sizeFromShape(imagesShape));
    const labels = new Int32Array(_util.sizeFromShape([size, 1]));
