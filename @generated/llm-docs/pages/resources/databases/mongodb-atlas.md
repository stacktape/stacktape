...
> **Info:** This example omits `providerConfig.mongoDbAtlas` for brevity. You must configure your Atlas API credentials as described in [Provider configuration](#provider-configuration) for the cluster to deploy.
...
> **Warning:** Store your Atlas API credentials as [secrets](/configuration/secrets) and reference them with the [`$Secret()` directive](/configuration/directives). Never put API keys directly in your config file.
...
> **Warning:** Shared tiers (M2/M5) cannot use `vpc` or `scoping-workloads-in-vpc` access modes. If you use shared tiers, the accessibility mode must be `internet` or `whitelisted-ips-only`.
...
> **Warning:** The BI Connector is CPU-intensive and may degrade performance on M10 and M20 clusters. For production BI workloads, use M30+ or configure analytics nodes in `replication.numAnalyticsNodes` and set `readPreference: 'analytics'`.
...
Yes. MongoDB Atlas clusters require a separate Atlas account with API credentials configured in `providerConfig.mongoDbAtlas`. Store credentials as [secrets](/configuration/secrets) and reference them with `$Secret()`. This is the only Stacktape database resource that requires external provider credentials — AWS-native databases like [relational databases](/resources/databases/relational-database) and [DynamoDB](/resources/databases/dynamodb) work without additional accounts.
