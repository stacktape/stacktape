...
AWS infrastructure costs are billed directly to your AWS account, separate from Stacktape's subscription fee. The Stacktape Console provides cost dashboards, per-stack cost attribution, and budget alerts to help you understand and control your AWS spend. This section also covers optimization strategies to reduce costs.
...
Stacktape does not handle or intermediate your AWS charges — AWS bills you directly. The Stacktape Console displays AWS cost report data for connected AWS accounts. Subscription plans and billing are managed on the [Billing page](/stacktape-console/billing-and-subscription).
...
The **overview dashboard** displays a month-to-date AWS costs KPI with a trend comparison against the previous month and a monthly cost trend sparkline, an AWS Costs by Project chart (top 10 projects plus an "Other" bucket), and an AWS Costs by Resource chart. These give you a quick spending snapshot without navigating away from the main dashboard. Budget alert status also appears on the overview dashboard, showing each configured budget with its name, scope, alert percentages, and threshold.

The **costs per stack page** lets you select a connected AWS account and a time range (month), then shows a table of every attributed stack with its project name, stage, region, and month-to-date costs — sortable by cost. Clicking a row navigates to the per-stage cost detail view. The costs-per-stack report is generated daily from AWS Cost and Usage Reports and may take up to a day to appear. This is where you go to compare spend across stages or find your most expensive stack for a given month.


> **Info:** New stacks may take up to a day to appear in cost data after their first deployment.


For the full walkthrough of these views, see [Dashboards](/managing-costs/dashboards). For guidance on reading the AWS Costs by Resource chart, see [Per-resource breakdown](/managing-costs/per-resource-breakdown).

### Cost attribution

The costs-per-stack view displays attributed stack data by project, stage, region, and cost. Stack-scoped cost and budget views depend on Stacktape deployment metadata. When you deploy a stack through Stacktape, the metadata is attached automatically — no additional configuration is needed.
...
- **Organization-wide budgets** — applied to an entire AWS account. Created in the selected AWS account when you save the budget configuration.
- **Per-stack budgets** — scoped to specific projects and stages. Stack-scoped budget status depends on Stacktape deployment metadata, so deploy matching stacks after creating or changing stack-scoped alerts.
...
Organization-wide budgets are the simpler option — they cover all spend in an AWS account and activate immediately when saved. Most teams start with an organization-wide budget and add per-stack budgets for production stages as they grow.


> **Tip:** Start with a single organization-wide budget at a comfortable monthly cap. Add per-stack budgets later for production stages that you want to monitor individually.
...
2. **Using Lambda functions for intermittent traffic** — Lambda's pay-per-invocation model is usually cheaper than an always-on container when a workload handles fewer than a few requests per second on average.
...
The costs-per-stack report is generated daily from AWS Cost and Usage Reports and may take up to a day to appear in the Console. Existing stacks update once per day. This delay is an AWS-side constraint — AWS generates Cost and Usage Reports on a daily cadence.

### Are AWS costs billed through Stacktape?

No. AWS bills your AWS account directly. Stacktape does not handle or intermediate your AWS charges — the Stacktape Console displays AWS cost report data for connected AWS accounts. Stacktape's own subscription is a separate charge billed by Stacktape to your organization. See [Billing and subscription](/stacktape-console/billing-and-subscription) for subscription details.
...
For most deployments, the top cost drivers are compute (ECS Fargate tasks or Lambda invocations), databases (RDS or Aurora instances), and data transfer. The exact breakdown depends on your architecture. The overview dashboard's AWS Costs by Resource chart shows your actual distribution, so you can identify your biggest cost driver at a glance.

### How can I reduce Lambda function costs?

Reduce Lambda costs by right-sizing memory so functions do not pay for unused capacity, reducing execution duration through code optimization, and using ARM/Graviton architecture which AWS prices lower per GB-second than x86. For high-throughput functions that run continuously, evaluate whether a container-based [web service](/resources/compute/web-service) would be cheaper at sustained load. See [Optimization tips](/managing-costs/optimization-tips) for detailed guidance.

### How often are cost reports updated?

AWS generates Cost and Usage Reports once per day. The Stacktape Console displays these reports for connected AWS accounts, so cost data may take up to a day to reflect recent changes. Real-time cost monitoring is not available through this mechanism — use AWS CloudWatch billing alarms if you need sub-daily alerts on spend thresholds.

### What is the difference between the overview dashboard and the costs page?

The overview dashboard shows a spending summary — month-to-date total with a trend comparison against the previous month, a cost trend sparkline, AWS Costs by Project (top 10), and AWS Costs by Resource. The dedicated costs per stack page provides a full table of every attributed stack with sortable columns, AWS account and time range selectors, and navigation to per-stage detail views. See [Dashboards](/managing-costs/dashboards) for a walkthrough of both.
...
The costs-per-stack table is populated from attributed stack cost data, so Stacktape project and stage attribution is available for stacks that have Stacktape deployment metadata. Resources created outside of Stacktape (manually in the AWS console or by other tools) do not have this metadata. To view all AWS spending including non-Stacktape resources, use AWS Cost Explorer directly in your AWS account.
