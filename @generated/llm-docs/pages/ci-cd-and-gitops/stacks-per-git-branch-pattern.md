...
The [GitOps page](/ci-cd-and-gitops/gitops-with-console) in the Stacktape Console lists configurations by deployment trigger, branch, stage name, AWS region, and AWS account. GitOps configurations support two trigger types: **PR Opened** and **Push to Branch**. To deploy a long-lived branch, create a configuration with the Push to Branch trigger targeting your branch and desired stage name. Every push to that branch triggers a deployment to the named stage.


> **Info:** GitOps auto-deployment currently supports GitHub only through the GitHub App integration. GitLab, Bitbucket, and other Git providers are not supported for the managed GitOps flow. Use [custom CI/CD](/ci-cd-and-gitops/custom-ci-cd) pipelines for these providers.
...
> **Warning:** When using the branch name directly as the stage name, sanitize it first. Branch names like `feature/add-auth` contain slashes and other characters that may not work as stage names. Extract a short identifier instead (e.g. `auth`).
...
2. **Temporary configuration** — A temporary GitOps configuration is created for the PR branch, linking it to the `pr-{number}` stage. This configuration is automatically removed when the PR closes.
3. **PR closed or merged** — If the `deleteStackAfterPrClose` setting is enabled on the configuration, the handler starts a stack deletion for the preview stage and removes the temporary configuration.

On orchestration failure, the PR comment is updated with failure details.


> **Info:** The stage name `pr-{number}` is derived automatically from the PR number by the GitHub App handler. For example, PR #42 deploys to stage `pr-42`. This naming is built into the handler and is not configurable through the GitOps UI.
...
```bash
npx stacktape deploy --stage "pr-${{ github.event.pull_request.number }}" --region eu-west-1 --autoConfirmOperation
```

Deploy on merge request open (GitLab CI):

```bash
npx stacktape deploy --stage "pr-${CI_MERGE_REQUEST_IID}" --region eu-west-1 --autoConfirmOperation
```

Delete on PR close (GitHub Actions — add as a separate job triggered on PR close):

```bash
npx stacktape delete --stage "pr-${{ github.event.pull_request.number }}" --region eu-west-1 --autoConfirmOperation
```

Delete on merge request close (GitLab CI):

```bash
npx stacktape delete --stage "pr-${CI_MERGE_REQUEST_IID}" --region eu-west-1 --autoConfirmOperation
```


> **Tip:** Use `--autoConfirmOperation` in CI pipelines to skip the interactive confirmation prompt that would otherwise block the job.
...
PR preview GitOps configurations include a `deleteStackAfterPrClose` setting. Enabling this is the recommended default — it prevents preview stages from accumulating and incurring ongoing AWS costs for databases, load balancers, or container workloads that no one is using.
...
> **Warning:** Avoid using the full branch name as the stage name. Branch names like `feature/add-new-auth-flow` are typically too long and may contain characters (like slashes) that don't work in AWS resource names. Extract a short identifier instead.
...
**Right-size preview resources.** Use [stage-specific configuration](/configuration/stages-and-environments) or conditional logic in your `defineConfig` function to give preview stages smaller database instances, lower container counts, or fewer replicas than production.

**Auto-delete PR previews.** Enable the `deleteStackAfterPrClose` setting in GitOps configurations, or add a deletion step in your custom CI pipeline. Stale previews running unused databases or containers are the most common source of unexpected bills.
...
|----------|-----|----------|
| **GitOps auto-delete** | Enable `deleteStackAfterPrClose` in the GitOps configuration | PR previews |
...
> **Warning:** Deleting a stage removes all AWS resources provisioned for that stage — databases, containers, buckets, and their data. This is irreversible. Confirm the stage is no longer needed before deleting.
