...
**`--localTunnelingPort`** — sets the starting local port for the tunnel. If not specified, Stacktape automatically selects an available port. When multiple tunnels are opened (e.g. a database with a reader endpoint), each subsequent tunnel uses the next port in sequence starting from this number. Stacktape validates that all required ports are available before starting.
...
| [private-service](/resources/compute/private-service) | Tunnels through the private service's load balancer. Requires the private service to have `loadBalancing` type `application-load-balancer`. |

If you target a resource type not listed above, Stacktape exits with an error listing the supported types.
...
The output prints each local-to-remote mapping. When a connection string is available for the target, Stacktape also prints a substituted version with `127.0.0.1:<localPort>` replacing the remote host and port.
...
If the resource has multiple endpoints, port `5433` is used for the first tunnel, `5434` for the second, and so on.
...
5. Starts SSM port-forwarding sessions — one per resolved target — binding each to a local port (either from `--localTunnelingPort` or auto-selected).
...
> **Info:** The tunnel uses AWS SSM Session Manager under the hood. No inbound ports need to be open on the bastion. The [AWS SSM Session Manager plugin](https://docs.aws.amazon.com/systems-manager/latest/userguide/session-manager-working-with-install-plugin.html) must be installed on your machine.
...
No. The `bastion:tunnel` command uses AWS SSM Session Manager for port forwarding, not SSH. You do not need to manage SSH keys or open any inbound ports. The only prerequisite is the [SSM Session Manager plugin](https://docs.aws.amazon.com/systems-manager/latest/userguide/session-manager-working-with-install-plugin.html) installed on your local machine.
...
Stacktape exits with an error that lists the supported resource types: `relational-database`, `redis-cluster`, `application-load-balancer`, and `private-service` (with `loadBalancing` type `application-load-balancer`).

### Can I run multiple tunnels at the same time?

Yes. Open separate terminal windows and run `bastion:tunnel` for different resources in each. Use `--localTunnelingPort` to avoid port conflicts between sessions.

### Why does my private service tunnel fail?

Tunneling to a [private service](/resources/compute/private-service) requires the service to have `loadBalancing` type `application-load-balancer` configured. If your private service does not use a load balancer, the command fails with an error.

### How do I connect a database GUI through the tunnel?

Run `bastion:tunnel` and note the local address printed in the output (e.g. `127.0.0.1:5433`). In your database GUI, set the host to `127.0.0.1` and the port to the printed local port. Use the database credentials from your [secrets](/configuration/secrets) or stack outputs.

### Does the tunnel stay open indefinitely?

The tunnel stays open as long as the `bastion:tunnel` process is running. Press `Ctrl+C` to close all tunnel sessions and exit. If your terminal disconnects or the process is killed, cleanup hooks close the sessions automatically.

### What is the SSM Session Manager plugin?

The SSM Session Manager plugin is an AWS-provided binary that enables your local machine to establish port-forwarding sessions through AWS Systems Manager. It is required for both [`bastion:tunnel`](/cli/bastion-tunnel) and [`bastion:session`](/cli/bastion-session). Install it from the [AWS documentation](https://docs.aws.amazon.com/systems-manager/latest/userguide/session-manager-working-with-install-plugin.html).
