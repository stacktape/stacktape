...
In agent mode (`--agent`), the command disables the interactive table view and prints the resolved stack name, region, and stack details as a JSON object for programmatic consumption. This is useful in CI scripts or when piping output to tools like `jq`.
...
Get JSON output for a CI pipeline or script. The `--agent` flag disables the interactive view and prints a JSON object containing the stack name, region, and details.
