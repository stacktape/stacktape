...
The `info:operations` command lists recent Stacktape operations (deploys, deletes, rollbacks) across your organization. Each entry shows success or failure status, timestamps, and error descriptions for failed operations. Use filters to narrow results by project, stage, or the current API-key user.
...
This returns the 25 most recent operations across all projects and stages in your organization. You must be logged in with a valid API key — run [`stacktape login`](/cli/login) first if you haven't already.
...
Filter by stage to see operations for a specific stage.
...
Use `--agent` or `--outputFormat jsonl` to get NDJSON output suitable for scripts or AI coding agents. This disables interactive terminal formatting and outputs one JSON object per line.
...
- [`deploy`](/cli/deploy) — deploy a stack to AWS.
- [`delete`](/cli/delete) — delete a deployed stack.
