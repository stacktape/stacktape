...
The `script:run` command executes a script defined in the `scripts` section of your Stacktape configuration. It requires `--scriptName` and `--stage`, accepts additional environment variables via `--env`, and can optionally assume a deployed resource's IAM role with `--assumeRoleOfResource`. Use it for one-off operational tasks like database migrations, data seeding, or maintenance scripts.
...
`--scriptName` and `--stage` are the only required command-specific arguments. Pass `--region` when you do not rely on a configured default (set via [`defaults:configure`](/cli/defaults-configure)).
...
Run a database migration script against the staging stack in a specific region:
...
If the `--scriptName` value does not match any key in the `scripts` section of your configuration, the command throws `stpErrors.e20({ scriptName })`. Double-check the script name in your configuration file — the match is case-sensitive.
...
- [`container:session`](/cli/container-session) — starts an interactive shell session inside a deployed container for ad-hoc debugging.
- [`bastion:tunnel`](/cli/bastion-tunnel) — creates a secure tunnel to a VPC-private resource through a bastion host.
