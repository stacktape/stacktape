...
**`--command`** specifies the command that starts the interactive session inside the container. Use it to select a specific shell (like `bash`). To run a one-shot command and capture its output non-interactively, use [`debug:container-exec`](/cli/debug-container-exec) instead.
