...
// Source: aws-cloudfront-originaccesscontrol.json

/**
 * Creates a new origin access control in CloudFront. After you create an origin access control, you
 * can add it to an origin in a CloudFront distribution so that CloudFront sends authenticated
 * (signed) requests to the origin.
 ...
 * For more information about using a CloudFront origin access control, see [Restricting access to an
 ...
 */
export type AwsCloudfrontOriginaccesscontrol = {
  Id?: string;
  ...
  OriginAccessControlConfig: {
     ...
     * for the most common use case. For more information, see [origin access control advanced
     ...
     * request, you must add the Authorization header to a cache policy for all cache behaviors that use
