...
// Source: aws-ec2-volume.json

/**
 * Specifies an Amazon Elastic Block Store (Amazon EBS) volume.
 * When you use CFNlong to update an Amazon EBS volume that modifies ``Iops``, ``Size``, or
 ...
 * of time.
 * Amazon EBS does not support sizing down an Amazon EBS volume. CFNlong does not attempt to modify
 * an Amazon EBS volume to a smaller size on rollback.
 ...
 * To control how CFNlong handles the volume when the stack is deleted, set a deletion policy for
 * your volume. You can choose to retain the volume, to delete the volume, or to create a snapshot of
 * the volume. For more information, see [DeletionPolicy
 ...
 * snapshot.
 */
export type AwsEc2Volume = {
  /**
   * Indicates whether Amazon EBS Multi-Attach is enabled.
   * CFNlong does not currently support updating a single-attach volume to be multi-attach enabled,
   * updating a multi-attach enabled volume to be single-attach, or updating the size or number of I/O
   * operations per second (IOPS) of a multi-attach enabled volume.
   */
  MultiAttachEnabled?: boolean;
  /**
   * The identifier of the kms-key-long to use for Amazon EBS encryption. If ``KmsKeyId`` is specified,
  ...
  Size?: number;
  /**
   * Indicates whether the volume is auto-enabled for I/O operations. By default, Amazon EBS disables
   * I/O to the volume from attached EC2 instances when it determines that a volume's data is
   ...
   * System](https://docs.aws.amazon.com/ec2/latest/instancetypes/ec2-nitro-instances.html) can support
   * up to 256,000 IOPS. Other instances can support up to 32,000 IOPS.
   */
  Iops?: number;
  /**
