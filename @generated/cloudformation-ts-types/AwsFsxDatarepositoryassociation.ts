...
// Source: aws-fsx-datarepositoryassociation.json

/**
 * Creates an Amazon FSx for Lustre data repository association (DRA). A data repository association
 * is a link between a directory on the file system and an Amazon S3 bucket or prefix. You can have a
 * maximum of 8 data repository associations on a file system. Data repository associations are
 * supported on all FSx for Lustre 2.12 and newer file systems, excluding ``scratch_1`` deployment
 * type.
 ...
 */
export type AwsFsxDatarepositoryassociation = {
   ...
   * associations cannot have overlapping file system paths. For example, if a data repository is
   * associated with file system path ``/ns1/``, then you cannot link another data repository with file
   * system path ``/ns1/ns2``.
  ...
  /**
   * The path to the Amazon S3 data repository that will be linked to the file system. The path can be
   * an S3 bucket or prefix in the format ``s3://myBucket/myPrefix/``. This path specifies where in the
   ...
   * after the data repository association is created. The task runs if this flag is set to ``true``.
   */
  BatchImportMetaDataOnCreate?: boolean;
  /**
   * For files imported from a data repository, this value determines the stripe count and maximum
   * amount of data per file (in MiB) stored on a single physical disk. The maximum number of disks that
   * a single file can be striped across is limited by the total number of disks that make up the file
   * system or cache.
  ...
  /**
   * The configuration for an Amazon S3 data repository linked to an Amazon FSx Lustre file system with
   * a data repository association. The configuration defines which file events (new, changed, or
   * deleted files or directories) are automatically imported from the linked data repository to the
   * file system or automatically exported from the file system to the data repository.
   */
  S3?: {
    /**
     ...
     * your Amazon FSx for Lustre file system as you modify objects in a linked S3 bucket.
    ...
    /**
     * Describes a data repository association's automatic export policy. The ``AutoExportPolicy`` defines
     * the types of updated objects on the file system that will be automatically exported to the data
     * repository. As you create, modify, or delete files, Amazon FSx for Lustre automatically exports the
     * defined changes asynchronously once your application finishes modifying the file.
  ...
  Tags?: {
     ...
     * values can be null and don't have to be unique in a tag set. For example, you can have a key-value
     * pair in a tag set of ``finances : April`` and also of ``payroll : April``.
     * @minLength 0
     * @maxLength 256
     */
    Value: string;
