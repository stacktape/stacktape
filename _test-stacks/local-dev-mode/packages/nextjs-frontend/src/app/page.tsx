...
type OpenSearchDoc = { id: string; title?: string; content?: string; createdAt?: string };
...
export default function Home() {
  const [activeTab, setActiveTab] = useState<'posts' | 'cache' | 'opensearch'>('posts');
  const apiUrl = getApiUrl();

  return (
    <div style={{ maxWidth: 900, margin: '0 auto', padding: 20 }}>
      <h1 style={{ marginBottom: 8 }}>Next.js Frontend - Local Dev Mode</h1>
...
function PostsSection({ apiUrl }: { apiUrl: string }) {
      ...
      <form onSubmit={createPost} style={{ background: '#fff', padding: 16, borderRadius: 8, marginBottom: 20 }}>
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          <input
            ...
            style={{ flex: 1, minWidth: 150, padding: '8px 12px', border: '1px solid #ddd', borderRadius: 4 }}
          />
          <input
            placeholder="Content"
            value={newPost.content}
            onChange={(e) => setNewPost((p) => ({ ...p, content: e.target.value }))}
            style={{ flex: 2, minWidth: 200, padding: '8px 12px', border: '1px solid #ddd', borderRadius: 4 }}
          />
          <select
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  <button
                    ...
                    style={{
                      background: post.published ? '#22c55e' : '#f59e0b',
                      color: '#fff',
                      fontSize: 12,
                      padding: '4px 8px',
                      border: 'none',
...
function CacheSection({ apiUrl }: { apiUrl: string }) {
      ...
      <form onSubmit={createEntry} style={{ background: '#fff', padding: 16, borderRadius: 8, marginBottom: 20 }}>
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          <input
            ...
            style={{ flex: 1, minWidth: 100, padding: '8px 12px', border: '1px solid #ddd', borderRadius: 4 }}
          />
          <input
            placeholder="Value"
            value={newEntry.value}
            onChange={(e) => setNewEntry((p) => ({ ...p, value: e.target.value }))}
            style={{ flex: 2, minWidth: 150, padding: '8px 12px', border: '1px solid #ddd', borderRadius: 4 }}
          />
          <input
            placeholder="TTL (sec)"
            value={newEntry.ttl}
            onChange={(e) => setNewEntry((p) => ({ ...p, ttl: e.target.value }))}
            style={{ width: 80, padding: '8px 12px', border: '1px solid #ddd', borderRadius: 4 }}
            type="number"
          />
          <button
            type="submit"
...
function OpenSearchSection({ apiUrl }: { apiUrl: string }) {
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {displayDocs.map((doc) => (
            ...
            >
              <div style={{ flex: 1 }}>
                <div>
                  <strong style={{ color: '#0070f3' }}>{doc.title || 'Untitled'}</strong>
                  <span style={{ marginLeft: 8, color: '#999', fontSize: 12 }}>ID: {doc.id}</span>
                </div>
                {doc.content && <p style={{ margin: '8px 0 0', color: '#666' }}>{doc.content}</p>}
                {doc.createdAt && (
