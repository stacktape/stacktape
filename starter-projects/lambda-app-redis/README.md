...
```bash
stacktape init --projectId lambda-app-redis
```
...
- Navigate to the URL `<<main-api-gateway-url>>/{<<key>>}/{<<value>>}` to store the key-value pair in Redis. The URL of
...
[dev command](https://docs.stacktape.com/cli/commands/dev/). For example, to develop and debug lambda function `storeKeyValuePair`, you can use
...
- **ConnectTo list** - we are adding redis database `redis` into `connectTo` list. By doing this, Stacktape will
  automatically inject relevant environment variables into the workload's runtime (such as redis connection url required
