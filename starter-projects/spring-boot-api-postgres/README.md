...
- **Packaging** - determines how the Docker container image is built. In this case, we are using `stacktape-image-buildpack`. We only need to configure `entryfilePath`.
  Stacktape automatically builds the application code with all of its dependencies, builds the Docker
