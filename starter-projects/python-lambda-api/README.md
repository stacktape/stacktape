# Python lambda with MongoDB Atlas cluster

- This project deploys a simple Lambda-based HTTP API using Python runtime.
- The application runs in a [Lambda function](https://docs.stacktape.com/resources/lambda-functions/) and uses
  [DynamoDb](https://docs.stacktape.com/resources/dynamo-db-tables/) to store the data. To simplify
  the database access, this project uses [PyMongo](https://pymongo.readthedocs.io/en/stable/).
...
```bash
stacktape init --projectId python-lambda-api
```
...
- Fill in your MongoDb Atlas credentials in the `providerConfig.mongoDbAtlas` section of the stacktape.yml config file. To learn how to get your API keys and organization ID, refer to [MongoDB Atlas tutorial](https://docs.atlas.mongodb.com/configure-api-access/#std-label-atlas-prog-api-key). - Install your projects dependencies. The recommended way is to use [Virtual environment](https://packaging.python.org/en/latest/guides/installing-using-pip-and-virtual-environments/).
...
- **Packaging** - determines how the lambda artifact is built. The easiest and most optimized way to build Python lambda
  function is to use `stacktape-lambda-buildpack`. We only need to configure `entryfilePath`. Stacktape automatically
  builds the application code with all of its dependencies, creates the lambda zip artifact, and uploads it to a
  pre-created S3 bucket on AWS. You can also use
...
- **Events** - Events determine how the function is triggered. In this case, we are triggering the function when an
  event (HTTP request) is delivered to the HTTP API gateway:
