...
```bash
stacktape init --projectId lambda-api-dynamo-db
```
...
- **Access control** - Functions must be allowed to access the DynamoDB table, to be able to read from and write to the
  table. We are doing this by listing the table in `connectTo` list. To see other ways of controlling access to
  resources refer to
