 ...
 * Follows the pattern used by OpenCode.
 *
 * When `init()` returns an object with a `ready` property, the provider
 * defers rendering children until `ready` is truthy.
 */
// eslint-disable-next-line ts/no-empty-object-type -- {} is intentional: allows bare <Provider> with no props
export const createSimpleContext = <T, Props = {}>(input: {
  name: string;
