...
export const commandDeploy = async () => {
  ...
  const [{ packagedWorkloads, abort, cfTemplateDiff }] = await Promise.all([
    ...
    cloudformationRegistryManager.registerLatestCfPrivateTypes(
      configManager.requiredCloudformationPrivateTypes,
    ),
  ]);
  ...
  if (!useHotswap) {
    ...
    await deployedStackOverviewManager.refreshStackInfoMap({
      ...
      budgetInfo: budgetManager.getBudgetInfoForSpecifiedStack({
        stackName: globalStateManager.targetStack.stackName,
      }),
    });
  }
...
const performHotswapDeploy = async () => {
  ...
  const containerWorkloadsToBeUpdated = (
    await Promise.all(
      configManager.allContainerWorkloads.map((workload) => getECSHotswapInformation({ workload })),
    )
  ).filter(
    ({ ecsTaskDefinition, ecsService }) => ecsTaskDefinition.needsUpdate || ecsService.needsUpdate,
  );
