...
seoDescription: 'Deploy pre-built Lambda packages with Stacktape. Provide a zip file, directory, or non-zip file — Stacktape auto-zips non-zip inputs and deploys your artifact to AWS Lambda.'
---
...
The most common cause is a `handler` path that doesn't match the file structure inside your zip. Unzip the artifact locally and confirm the handler file exists at the path Lambda will see — a mismatch typically surfaces as a module-not-found or handler-not-found error at invocation time. Use [`stacktape debug:logs`](/cli/debug-logs) to view the error.
