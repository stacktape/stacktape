...
<Tip>If you see "No rules matched" for events you care about, check that routing rules in [notifications](/observability/notifications), [alarms](/observability/alarms), or [budgets](/managing-costs/budgets) cover the event type and route to an [alert channel](/observability/alert-channels).</Tip>
...
<Info>Alert history is available in the Stacktape Console. For CLI-based alarm inspection, use [`stacktape debug:alarms`](/cli/debug-alarms) to check current CloudWatch alarm states.</Info>
...
No — alert history is a Console-only page. For CLI-based alarm inspection, use [`stacktape debug:alarms`](/cli/debug-alarms) to check current CloudWatch alarm states.