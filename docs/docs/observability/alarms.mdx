...
For threshold-based triggers that expose `comparisonOperator`, the default is `GreaterThanThreshold`, except `database-free-storage` and `database-free-memory`, which default to `LessThanThreshold` — the alarm fires when the metric drops *below* the threshold.
...
<ApiReference definitionName="AlarmDefinition" />
