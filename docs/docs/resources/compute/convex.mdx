...
title: 'Convex'
order: 37
seoTitle: 'Self-Hosted Convex Backend on AWS | Stacktape Docs'
seoDescription: 'Deploy a self-hosted Convex backend on AWS with Stacktape. Reactive database, serverless functions, file storage, and admin dashboard — provisioned as a single resource with PostgreSQL, S3, and Fargate.'
---
  ...
  tabs={[
    {
      label: 'TypeScript',
      lang: 'typescript',
      code: `import { defineConfig, Convex } from 'stacktape';
  ...
  tabs={[
    {
      label: 'TypeScript',
      lang: 'typescript',
      code: `import { defineConfig, Convex, LambdaFunction, StacktapeLambdaBuildpackPackaging } from 'stacktape';
...
<Warning>
Upgrading the Convex backend image can trigger data migrations. Test upgrades on a staging stage before applying to production.
</Warning>
...
<Info>
You cannot bring an existing external database — Convex assumes it owns its PostgreSQL instance entirely. The database is an internal implementation detail of the Convex deployment.
</Info>
...
<Tip>
Enable deletion protection for all production stages. The small inconvenience of a two-step delete process is worth preventing accidental data loss.
</Tip>
...
<ApiReference definitionName="ConvexProps" />
