...
Edge function logs go to CloudWatch Logs in the AWS region where the request was served, not necessarily the region where your stack was deployed. A request handled at a different edge location produces logs outside the region you normally inspect. Use [`stacktape debug:logs`](/cli/debug-logs) and remember the log location follows the execution region.
