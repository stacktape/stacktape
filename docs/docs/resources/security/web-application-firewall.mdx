...
pipelineReviewSummary: 'reviewer-avg 8.5, hard blockers 2, advisory high 0, verifier medium 6'
---
...
<Warning>Some resources support both a regional and a CDN attachment point. For example, a resource fronted by both an ALB and CloudFront can have two independent firewalls — one `regional` on the ALB and one `cdn` on the CloudFront distribution. These use separate firewall resources with different scopes.</Warning>
