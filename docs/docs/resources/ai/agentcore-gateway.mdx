...
order: 73
seoTitle: 'AgentCore Gateway - MCP Tool Gateway for AI Agents | Stacktape'
seoDescription: 'Configure an MCP tool gateway on AWS Bedrock AgentCore with JWT authentication. Define Lambda-backed tools with schemas for AI agent discovery and invocation via Stacktape.'
---
...
export default defineConfig(() => {
  ...
  const supportAgent = new AgentCoreRuntime({
    ...
    environment: [
      ...
      { name: 'GATEWAY_URL', value: "$ResourceParam('supportGateway', 'url')" }
    ]
  });
  ...
  tabs={[
    {
      label: 'TypeScript',
      lang: 'typescript',
      code: `import { defineConfig, AgentCoreGateway } from 'stacktape';
...
<Tip>If you're using a [Stacktape user auth pool](/resources/security/user-auth-pool) (Amazon Cognito), construct the discovery URL from your User Pool ID: `https://cognito-idp.{region}.amazonaws.com/{userPoolId}/.well-known/openid-configuration`.</Tip>
...
export default defineConfig(() => {
  ...
  const agentRuntime = new AgentCoreRuntime({
    ...
    environment: [{ name: 'GATEWAY_URL', value: "$ResourceParam('toolGateway', 'url')" }]
  });
...
<ApiReference definitionName="AgentCoreGatewayProps" />
