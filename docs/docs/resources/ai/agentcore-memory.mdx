...
order: 72
seoTitle: 'AgentCore Memory for AI Agents | Stacktape Docs'
seoDescription: 'Configure persistent memory for AI agents on AWS Bedrock AgentCore with Stacktape. Set up retention, encryption, memory strategies, and runtime wiring in TypeScript.'
---
  ...
  tabs={[
    {
      label: 'TypeScript',
      lang: 'typescript',
      code: `import { defineConfig, AgentCoreMemory } from 'stacktape';
...
<Info>
`AgentCoreRuntimeProps` exposes `useMemory` as an optional string. Set it to the name of your `AgentCoreMemory` resource. To access the memory ID in your application code, pass it explicitly through `environment` using `$ResourceParam()` as shown above.
</Info>
...
<Tip>
Choose a retention window that balances recall usefulness against storage growth. Shorter windows suit transient conversational context; longer windows work for agents that accumulate user preferences or project knowledge over time.
</Tip>
...
<ApiReference definitionName="AgentCoreMemoryProps" />
