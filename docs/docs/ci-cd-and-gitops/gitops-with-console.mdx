...
pipelineReviewSummary: 'reviewer-avg 7.7, hard blockers 1, advisory high 0, verifier medium 4'
---
...
Push-to-deploy is configured from your project's GitOps page in the Stacktape Console. Click **New GitOps configuration** to open the creation modal. Existing configurations are listed with their deployment trigger, branch, stage name, AWS region, and connected AWS account — push configurations appear as **Push to Branch** in the trigger column.
...
PR preview configurations are created from the same **New GitOps configuration** modal on the GitOps page, using the **PR Opened** trigger. When a PR is opened against the configured branch, the handler starts a deployment to a new stage. The Console displays the stage name as `pr-{#number}` (e.g. `pr-42` for PR #42) when no explicit stage name is configured.
...
GitOps deployments need deployment execution infrastructure to run the actual `deploy` or `delete` command. Stacktape offers multiple build-runner options — including AWS CodeBuild-managed runners and EC2-based runners — with different cost, startup, and operational tradeoffs.

For runner types, configuration options, compute sizing, and cost tradeoffs, see [Build runners](/ci-cd-and-gitops/build-runners).
...
In the Stacktape Console, navigate to your project's GitOps page and click **New GitOps configuration** to open the creation modal. After you save a push-to-branch configuration, every push to the watched branch triggers a deployment to the configured stage automatically. No pipeline files or CI configuration are needed in your repository.
...
The GitHub App handler processes matching configurations through `Promise.all`. PR bookkeeping errors and PR-close deletion errors are captured individually and summarized in the handler's response. Deployment failures themselves surface in the deployment output for the affected stage rather than as part of that webhook response summary. Common failure causes include invalid Stacktape configuration, missing [secrets](/configuration/secrets), or AWS permission issues on the connected account. Check your deployment output and [`stacktape debug:logs`](/cli/debug-logs) for diagnostics.
