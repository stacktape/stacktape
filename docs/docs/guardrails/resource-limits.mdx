...
order: 103
seoTitle: 'Resource Limit Guardrails — Cap Lambda & Container Sizes | Stacktape Docs'
...
pipelineReviewSummary: 'reviewer-avg 6.7, hard blockers 9, advisory high 0, verifier medium 14'
---
...
<Info>Guardrails are preventive — they block non-compliant deployments before they start. For reactive monitoring of running resources, use [alarms](/observability/alarms) instead. The two complement each other: guardrails prevent misconfigurations, alarms detect runtime problems.</Info>
...
<Warning>The container resource limit checks per-workload values, not per-container values. In a [multi-container workload](/resources/compute/multi-container-workload), the limit applies to the total resources allocated to the task, not to each individual container within it.</Warning>
