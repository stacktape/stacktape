...
<Info>
Dev mode needs Docker to start local database containers. If Docker is not running, dev mode cannot proceed with local databases.
</Info>
...
<Warning>
The `amazon/dynamodb-local` image does not include DynamoDB Streams, Global Tables, TTL, or on-demand backups. Use `dev.remote: true` if your application depends on these features.
</Warning>
