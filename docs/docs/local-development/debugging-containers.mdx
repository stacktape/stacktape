...
seoDescription: 'Debug deployed container workloads with Stacktape. Use container:session for interactive shell access or container:exec for scripted one-off commands via ECS Exec.'
---
...
<FeatureComparisonTable
  columns={["container:session", "container:exec"]}
  features={[
    { name: "Interactive shell", values: { "container:session": true, "container:exec": false } },
    { name: "Captures output as JSON", values: { "container:session": false, "container:exec": true } },
    { name: "Scriptable / CI-friendly", values: { "container:session": false, "container:exec": true } },
    { name: "Custom command", values: { "container:session": "Optional (--command)", "container:exec": "Required (--command flag)" } },
    { name: "Task selection", values: { "container:session": "Prompted if multiple", "container:exec": "Via --taskArn flag" } },
    { name: "Execution mode", values: { "container:session": "Persistent (stays open)", "container:exec": "One-shot (exits after command)" } }
  ]}
/>
...
<Tip>
If `container:exec` reports "No running tasks found for this resource", or `container:session` fails to connect, check that your service has healthy tasks. Use [`stacktape logs`](/cli/logs) to investigate why containers may be crashing before becoming reachable.
</Tip>
