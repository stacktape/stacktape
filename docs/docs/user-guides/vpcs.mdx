...
Container workloads and batch jobs are connected to a public subnet by default, so they can communicate with the internet without any extra configuration. However, [_Lambda functions_](/compute-resources/functions) cannot be connected to a public subnet.
