...
pipelineReviewSummary: 'reviewer-avg 7.4, hard blockers 5, advisory high 0, verifier medium 16'
---
...
<ConsoleScreenshot src="/static/screenshots/console-users-page.png" alt="Stacktape Console Users page showing a table of organization members with role badges, account status, MFA status, and action buttons for editing, revoking keys, and removing members" caption="The Users page lists organization members with role badges and per-member actions." />
...
<Tip>Revoking API keys and removing a member are separate actions. If you need to fully offboard someone, perform both actions — removing alone does not guarantee their [API keys](/stacktape-console/api-keys) stop working.</Tip>
