...
<FlowDiagram steps={[
  ...
  { title: "Wait for activation", description: "Return to the Stacktape Console. The account shows as PENDING until the backend reports it as ACTIVE. While a PENDING account exists, the Console refetches connected account status every two seconds and updates the table when the state changes." }
]} />
...
The Console refetches connected account status every two seconds while a PENDING account exists. When the backend reports the account as ACTIVE, the table updates.
...
PENDING means the connection link was generated in Stacktape but the backend has not yet reported the account as ACTIVE. While a PENDING account exists, the Console refetches connected account status every two seconds and updates the table when the state changes. If the account stays PENDING, use the "Finish the account connection" link in the Note column to complete the stack creation in AWS.
