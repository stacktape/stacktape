...
pipelineReviewSummary: 'reviewer-avg 8.3, hard blockers 6, advisory high 0, verifier medium 13'
---
...
Per-PR preview environments are a common GitOps pattern with Stacktape stages — each pull request gets its own isolated stage that reviewers can test against. See [GitOps with Console](/ci-cd-and-gitops/gitops-with-console) and [Stacks per git branch pattern](/ci-cd-and-gitops/stacks-per-git-branch-pattern) for setup details.
...
```bash
stacktape secret:create --secretName db-password-dev --region eu-west-1
```

Create a production secret:

```bash
stacktape secret:create --secretName db-password-production --region eu-west-1
```
...
Per-PR preview environments are a common GitOps pattern with Stacktape stages — each pull request gets its own isolated stage that reviewers can test against. See [GitOps with Console](/ci-cd-and-gitops/gitops-with-console) and [Stacks per git branch pattern](/ci-cd-and-gitops/stacks-per-git-branch-pattern) for the full setup.
