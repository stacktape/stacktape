...
<Info>The Stacktape VS Code extension provides autocompletion, syntax validation, and inline documentation for YAML configuration files.</Info>
...
YAML and TypeScript represent the same logical config shape: named resources with resource-specific properties. Rename your `stacktape.yml` to `stacktape.ts` and rewrite the resources using the `defineConfig` pattern and resource classes, keeping resource names and property values stable. Then run [`stacktape compile-template`](/cli/compile-template) to verify the output before deploying.
