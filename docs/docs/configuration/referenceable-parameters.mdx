...
pipelineReviewSummary: 'reviewer-avg 7.2, hard blockers 3, advisory high 0, verifier medium 4'
---
...
SSR framework resources expose the deployed site URL. Static [hosting buckets](/resources/frontend/static-hosting) expose bucket identifiers and — when a CDN is attached — CDN-related parameters. See each resource's table for the exact set.
...
CDN-related parameters are only available on resources that have a `cdn` configuration block attached. In Stacktape, CDN attachment is typed for [buckets](/resources/storage/s3-bucket), [Application Load Balancers](/resources/networking/application-load-balancer), [HTTP API Gateways](/resources/networking/http-api-gateway), and [Lambda functions](/resources/compute/lambda-function). Check the per-resource parameter table above for the exact CDN parameter names exposed by each resource type.
