...
<Warning>
Kafka records are delivered in batches, so design handlers so that repeated processing of the same record is safe when your broader pipeline can retry or replay messages.
</Warning>
