...
seoDescription: 'Learn how Stacktape triggers connect event sources to Lambda functions and containers. HTTP, schedules, queues, streams, S3 events, and filtering with automatic permissions.'
pipelineStatus: 'needs-human-review'
pipelineReviewSummary: 'reviewer-avg 7.6, hard blockers 1, advisory high 0, verifier medium 4'
---

# Triggers Overview

Triggers define what causes a Stacktape compute resource to execute. A [Lambda function](/resources/compute/lambda-function) sitting idle costs nothing — a trigger connects it to an event source (an HTTP request, a file upload, a queue message, a cron schedule) so it runs when work arrives. Stacktape supports trigger integrations for Lambda functions and container workloads.
...
| EventBridge event bus | `event-bus` | Push | [Event bus events](/configuration/triggers/event-bus-events) |
...
<FeatureComparisonTable
  columns={["Lambda function", "Container workloads"]}
  features={[
    { name: "HTTP API Gateway", values: { "Lambda function": true, "Container workloads": true } },
    { name: "Application Load Balancer", values: { "Lambda function": true, "Container workloads": true } },
    { name: "Network Load Balancer", values: { "Lambda function": false, "Container workloads": true } },
    { name: "Schedule", values: { "Lambda function": true, "Container workloads": false } },
    { name: "S3 events", values: { "Lambda function": true, "Container workloads": false } },
    { name: "SQS queue", values: { "Lambda function": true, "Container workloads": false } },
    { name: "SNS topic", values: { "Lambda function": true, "Container workloads": false } },
    { name: "DynamoDB stream", values: { "Lambda function": true, "Container workloads": false } },
    { name: "Kinesis stream", values: { "Lambda function": true, "Container workloads": false } },
    { name: "EventBridge event bus", values: { "Lambda function": true, "Container workloads": false } },
    { name: "CloudWatch Logs", values: { "Lambda function": true, "Container workloads": false } },
    { name: "CloudWatch Alarm", values: { "Lambda function": true, "Container workloads": false } },
    { name: "Kafka topic", values: { "Lambda function": true, "Container workloads": false } },
    { name: "Workload Internal", values: { "Lambda function": false, "Container workloads": true } },
    { name: "Service Connect", values: { "Lambda function": false, "Container workloads": true } }
  ]}
/>

Batch job trigger support is documented on the [batch job](/resources/compute/batch-job) page and on the trigger-specific pages.
...
Your function receives the full batch as a single event. For Kinesis and DynamoDB stream integrations, the source warns that if an error occurs, the entire batch is retried, including records that were processed successfully. Design your handlers to be idempotent.
...
A single Lambda function can have multiple triggers of any supported type in its `events` array. Common patterns:
