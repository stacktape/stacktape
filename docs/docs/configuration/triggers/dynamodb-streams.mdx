...
pipelineReviewSummary: 'reviewer-avg 7.5, hard blockers 4, advisory high 0, verifier medium 4'
---
...
`DynamoDbIntegration` requires the ARN of a DynamoDB table stream. Before attaching the trigger, enable streams on the [DynamoDB table](/resources/databases/dynamodb) — the integration source notes you do this via the table's streaming configuration. See the DynamoDB resource page for the exact property name and the supported stream view types (which control whether each record carries only keys, the new image, the old image, or both).

The view type you choose determines what your handler can read off each record. For most audit-and-diff workflows you want both before and after images so the handler has full context; for "fire-and-fetch" patterns where the handler reloads the item anyway, a keys-only view is cheaper.

Once streams are enabled, pass the stream ARN to `DynamoDbIntegration` via the `streamArn` property. For tables in the same stack, use [`$ResourceParam`](/configuration/directives) to resolve the ARN at deploy time (shown in the example below). For external tables, pass the ARN as a string literal.
...
The `streamArn` property is required on every `DynamoDbIntegration`. In the example above, `$ResourceParam('ordersTable', 'streamArn')` is used to read the stream ARN off the table defined in the same stack — the first argument (`'ordersTable'`) must match the key in the `resources` object. The exact set of referenceable parameters exposed by `DynamoDbTable` is documented on the [DynamoDB resource page](/resources/databases/dynamodb).
...
<Warning>Because the entire batch is retried on failure, your handler must be idempotent — processing the same record twice should produce the same result. Use item keys or another application-level idempotency key for deduplication.</Warning>
...
Enable streams on the [DynamoDB table](/resources/databases/dynamodb) resource — see that page for the table-side configuration and the supported stream view types. Enabling streams on an existing table does not affect existing data or cause downtime. Once enabled, pass the stream ARN to your `DynamoDbIntegration` via the `streamArn` property, using `$ResourceParam` for same-stack references or a literal ARN string for external tables.
