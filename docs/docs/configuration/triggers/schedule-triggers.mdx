...
seoDescription: 'Run Lambda functions on cron or rate schedules with Stacktape. Configure recurring tasks using AWS EventBridge rules, customize event payloads, and monitor invocations.'
pipelineStatus: 'needs-human-review'
pipelineReviewSummary: 'reviewer-avg 8.5, hard blockers 1, advisory high 0, verifier medium 3'
---
...
This page documents `ScheduleIntegration` on [Lambda functions](/resources/compute/lambda-function). A common pattern for running container-based work on a recurring schedule is to invoke a scheduled Lambda function that starts the container task programmatically using the AWS SDK.
