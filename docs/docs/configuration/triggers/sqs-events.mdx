...
Use SQS when you need a durable work queue with one consumer that processes messages at its own pace — order processing, background jobs, traffic buffering. Use [SNS](/configuration/triggers/sns-events) when you need pub/sub fan-out: one message delivered to multiple subscribers at once. Use [EventBridge](/configuration/triggers/event-bus-events) when you need content-based routing — filtering events by field values and routing them to different targets without code. A common pattern combines them — SNS or EventBridge fans out to multiple SQS queues, each with its own consumer Lambda.
