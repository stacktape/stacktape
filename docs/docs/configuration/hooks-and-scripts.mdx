...
pipelineReviewSummary: 'reviewer-avg 7.9, hard blockers 5, advisory high 0, verifier medium 5'
---
...
Local scripts and `local-script-with-bastion-tunneling` can execute JS, TS, or Python files via `executeScript` (single file) and `executeScripts` (list of files). The runtime is determined by [`defaults:configure`](/cli/defaults-configure) or the system default (`node` for JS/TS, `python` for Python). Bastion scripts cannot execute files directly — they only support `executeCommand` and `executeCommands`.
