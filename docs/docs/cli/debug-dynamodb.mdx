...
pipelineReviewSummary: 'reviewer-avg 8.0, hard blockers 1, advisory high 0, verifier medium 1'
---
...
Passing `--agent` selects programmatic output for AI coding assistants and automation scripts. It implies `--outputFormat jsonl`, so the command emits JSONL/NDJSON (one JSON object per line) instead of human-readable text, disables the interactive terminal UI, and auto-confirms operations.

```bash
stacktape debug:dynamodb --stage prod --region eu-west-1 --resourceName myTable --operation sample --agent
```

The result payload includes `ok`, `resource` (the resource name you passed), `operation` (the operation that ran), along with the operation's own result fields.
