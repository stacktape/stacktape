...
- For logs, the CLI command is `stacktape logs`, linked to `/cli/logs`. The 4.0 release renamed it from `debug-logs` to `logs` — DO NOT reference the old name.
- For ad-hoc database/data inspection from the CLI, use the new `query:*` family: `stacktape query:sql` (formerly `debug-sql`), `stacktape query:dynamodb` (formerly `debug-dynamodb`), `stacktape query:redis` (formerly `debug-redis`), `stacktape query:opensearch` (formerly `debug-opensearch`). DO NOT reference any `debug-*` name from before 4.0.
- For one-off CLI commands renamed in 4.0: `synth` (was `compile-template`), `package` (was `package-workloads`), `diff` (was `preview-changes`), `metrics` (was `debug-metrics`), `alarms` (was `debug-alarms`), `container:exec` (was `debug-container-exec`), `aws:call` (was `debug-aws-sdk`), `secret:set` (was `secret-create`), `project:list` (was `info-projects`). Always use the new name.
- `codebuild:deploy` was removed in 4.0. Use `stacktape deploy --runner codebuild` instead. The `--runner` flag accepts `local` (default), `codebuild`, or `ec2`.
- A new command `validate` was added in 4.0. It performs config/template validation distinct from `synth` (which produces synthesized output). Both are valid CLI commands.
