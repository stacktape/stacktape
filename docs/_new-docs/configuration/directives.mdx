...
<Warning>The referenced stack must be deployed before the stack referencing it.</Warning>
...
<Info>Each resource's documentation lists all available parameters.</Info>
...
<Info>
  `$CfStackOutput()` prevents deleting the referenced stack while this stack exists, protecting against accidental
...
<NavBoxGrid columns={2}>
  <NavBox
    url="/configuration/connecting-resources"
    text="Connecting Resources"
    description="Use connectTo for automatic permissions"
  />
  <NavBox url="/security/secrets" text="Secrets Management" description="Learn more about managing secrets" />
</NavBoxGrid>
