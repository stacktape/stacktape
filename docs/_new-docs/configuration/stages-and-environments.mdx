...
<Warning>
  Stage names are limited to **12 characters** and should contain only alphanumeric characters and hyphens.
</Warning>
  ...
  files={[
    {
      ...
      children: [
        ...
        { name: 'api', type: 'folder', children: [{ name: 'stacktape.ts', description: 'References shared-infra' }] },
        { name: 'web', type: 'folder', children: [{ name: 'stacktape.ts', description: 'References shared-infra' }] }
      ]
    }
  ]}
/>
...
<NavBoxGrid columns={2}>
  <NavBox
    url="/configuration/overrides-and-transforms"
    text="Overrides & Transforms"
    description="Customize CloudFormation output"
  />
  <NavBox url="/deployment/gitops" text="GitOps Deployments" description="Automated stage deployments" />
</NavBoxGrid>
