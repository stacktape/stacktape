...
<FlowDiagram
  steps={[
    { title: 'Parse Config', description: 'Read stacktape.ts, resolve directives ($Secret, $Stage)' },
    { title: 'Package Code', description: 'Bundle Lambda with esbuild, build Docker images' },
    { title: 'Generate CloudFormation', description: 'Transform ~30 lines into ~1,200+ lines of CFN' },
    { title: 'Deploy to AWS', description: 'CloudFormation creates resources in correct order' }
  ]}
/>
