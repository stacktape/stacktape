...
different things. Putting all four in `.env` files makes setup look simple while hiding ownership, lifetime, scope, and
rotation. The rules below are the monorepo contract for humans and agents.
...
GitHub variables. `.env.example` may document names with inert placeholders. Local `.env*` files are ignored, but they
are an emergency compatibility path, not the supported credential store.

## Console deployment contract

The private Console owns an executable manifest in `apps/console/api/infrastructure/deployment-parameters.ts`. The
infrastructure definition reads stage-scoped `$SsmParam(...)` references from that manifest, and the preflight validates
the selected stage, AWS account, parameter existence, `SecureString` type, and decryptability without printing values:
...
Console ECS workloads use Stacktape's `secrets` field, which emits native ECS secret references and grants the execution
role access only to the selected parameter ARNs. Lambda functions receive only a value-free parameter map and resolve it
once per warm runtime through `api/src/runtime-parameters.ts`. Database connection strings are assembled inside the
process from a separately injected password; they are not stored in Lambda configuration or ECS task definition
environment values.

The one current exception is the Google Cognito identity-provider client secret. CloudFormation does not support an SSM
secure dynamic reference for that resource property, so it remains in Secrets Manager. Every migrated legacy Console
secret and its replacement parameter is recorded in `apps/console/api/infrastructure/secret-retirement.ts`; delete those
...
- Prefer AWS IAM Identity Center profiles and `aws sso login`; do not create IAM-user access keys for new users or
  agents.
- Prefer `stacktape login`, whose session is kept outside the checkout. Automation uses its own expiring, scoped API key
  only when interactive login is impossible.
...
1. Create or rotate a value directly in the approved manager. Never put it on a command line that will be retained in
   shell history.
2. Give the secret a logical owner and stage-scoped parameter path, then add its non-sensitive contract metadata to the
   manifest.
...
resource ARNs it needs. Delete an Actions secret when no workflow references it; a stored but unused credential is still
attack surface.
