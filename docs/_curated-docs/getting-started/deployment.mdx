...
<FlowDiagram
  steps={[
    { title: '1. Validate', description: 'Config is parsed and validated for errors' },
    { title: '2. Package', description: 'Lambda code is bundled, Docker images are built' },
    { title: '3. Upload', description: 'Artifacts uploaded to S3, images pushed to ECR' },
    { title: '4. Generate', description: 'CloudFormation template is generated (~1,200+ lines)' },
    { title: '5. Deploy', description: 'CloudFormation creates/updates resources in correct order' },
    { title: '6. Hooks', description: 'afterDeploy scripts run (migrations, etc.)' }
  ]}
/>
...
```bash
stacktape diff --stage dev --region us-east-1
```
...
<Warning>
  Deletion is permanent and irreversible. All data in databases, S3 buckets, and other resources will be destroyed. Back
  up important data before deleting.
</Warning>
...
- Consider using `deploy --runner codebuild` to offload the build to AWS CodeBuild
