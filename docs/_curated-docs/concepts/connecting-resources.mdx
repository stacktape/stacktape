...
<Warning>
  Lambda functions in a VPC cannot access the internet by default. If you need internet access (for external APIs),
  consider using a NAT Gateway or keeping the database publicly accessible for development.
</Warning>
...
<NavBoxGrid columns={2}>
  <NavBox
    url="/configuration/stages-and-environments"
    text="Stages & Environments"
    description="Multi-stage deployment configuration"
  />
  <NavBox
    url="/configuration/directives"
    text="Directives"
    description="Learn about $ResourceParam and other directives"
  />
</NavBoxGrid>
