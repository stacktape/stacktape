...
3. **Synthesize template**: `stacktape synth` and inspect the output
...
```bash
stacktape synth --stage dev --region us-east-1
```
...
<NavBoxGrid columns={2}>
  <NavBox
    url="/extending/cloudformation-resources"
    text="CloudFormation Resources"
    description="Add raw CloudFormation resources"
  />
  <NavBox url="/extending/cdk-constructs" text="CDK Constructs" description="Use AWS CDK for complex extensions" />
</NavBoxGrid>
