...
<Tip>The command prints https://console.stacktape.com/domains as the place to continue or check domain setup.</Tip>
...
Although `--agent` is listed as a universal CLI option, `domain:add` still prompts for the domain name and setup confirmations, so treat it as an interactive setup command. Since domain setup is typically a one-time task per region, run it manually rather than scripting it into CI.