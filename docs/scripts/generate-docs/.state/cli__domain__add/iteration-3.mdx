...
4. **Provision TLS certificates** — Requests a regional ACM certificate. When the selected region is not `us-east-1`, the command also requests an additional certificate through `requestCertificateForDomainName(domainName, true)`. When the regional certificate is not yet issued, it adds a DNS validation record from the regional certificate's domain validation options to the Route 53 hosted zone.
...
The command stores detected domain status in AWS Parameter Store after selected steps (such as hosted-zone setup and certificate-validation setup). Later runs load the saved status and continue where possible.
...
The command requests a free AWS ACM certificate in the specified `--region`. When the selected region is not `us-east-1`, the command also requests an additional certificate through `requestCertificateForDomainName(domainName, true)`. When the regional certificate is not yet issued, the command adds a DNS validation record from the regional certificate's domain validation options to the Route 53 hosted zone. Certificate issuance typically takes a few minutes after DNS records are in place.
...
The command creates a regional certificate for the selected `--region` and, when that region is not `us-east-1`, also requests an additional certificate through `requestCertificateForDomainName(domainName, true)`. Run the command in each region where you intend regional certificates for the domain.
...
The command stores detected domain status in AWS Parameter Store after selected steps such as hosted-zone setup and certificate-validation setup. When you run `domain:add` again, it loads that saved status and continues from where possible — though the wizard may still re-confirm prompts before proceeding.
...
`domain:add` is designed as an interactive setup command. The domain name and the confirmation steps (managing DNS in AWS, creating the hosted zone, generating certificates, SES verification) are prompted by the wizard. Since domain setup is typically a one-time task per region, run it manually rather than scripting it into CI.