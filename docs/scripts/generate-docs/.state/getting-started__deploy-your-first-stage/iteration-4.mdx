...
Every Stacktape deploy creates or updates a CloudFormation stack in the connected AWS account. The stack is the single unit of management — updates, rollbacks, and deletions are managed through CloudFormation. Inside the stack, CloudFormation provisions whatever your configuration declares:
...
If CloudFormation fails during a full deploy, AWS may roll back the stack operation. The CLI surfaces the specific error — common causes include IAM permission issues, resource limit quotas, or invalid configuration values. For manual recovery options, see [`stacktape rollback`](/cli/rollback).
...
This removes the CloudFormation stack and the resources it manages from your AWS account. See the [`stacktape delete`](/cli/delete) CLI reference for the full set of flags.
