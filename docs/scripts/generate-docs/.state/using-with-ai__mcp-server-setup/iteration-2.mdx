...
seoDescription: 'Set up the Stacktape MCP server so AI coding assistants can deploy infrastructure, query databases, and search docs directly from your editor.'
---
...
The `stacktape_docs` tool searches Stacktape documentation for configuration syntax, resource types, deployment patterns, and troubleshooting guidance. The MCP server builds a lexical search index from generated AI docs bundled with the CLI and exposes `query`, `mode`, `resourceType`, `docType`, and `maxItems` parameters.

| Parameter | Type | Description |
|-----------|------|-------------|
| `query` | string (required) | Search query (e.g. "how to configure a lambda function") |
| `mode` | `answer` \| `reference` \| `snippet` | `answer` returns full doc content, `reference` returns titles and paths, `snippet` returns code blocks. Default: `answer` |
...
The docs index does not need network access — it is built from files shipped with the Stacktape CLI. This tool is the right starting point for any Stacktape configuration question, since syntax can change between versions.
...
Most operations require `projectName`, `stage`, `region`, and `configPath`. Operations with additional requirements:
...
**Per-operation required arguments:**

| Operation | Required args | Description |
|-----------|--------------|-------------|
| `info_stack` | — | View details of a deployed stack |
| `info_stacks` | — | List deployed stacks |
| `info_operations` | — | View deployment history |
| `info_whoami` | — | Check current identity and account |
| `logs` | `resourceName` | View resource logs (defaults to 50 entries when `limit` is not set) |
| `metrics` | `resourceName`, `metric` | View CloudWatch metrics for a resource |
| `alarms` | `resourceName` | View alarm states for a resource |
| `container_exec` | `resourceName`, `command` | Execute a command in a running container |
| `sql` | `resourceName`, `sql` | Run a SQL query against a relational database |
| `dynamodb` | `resourceName`, `operation` | DynamoDB operations (get, put, query, scan, delete) |
| `redis` | `resourceName`, `operation` | Redis operations |
| `opensearch` | `resourceName`, `operation` | OpenSearch operations |
| `aws_sdk` | `service`, `command` | Call any AWS SDK API directly |

Common optional filters include `startTime`, `endTime`, `limit`, `filter`, and `query` for log operations; `period` and `stat` for metrics; `container` and `taskArn` for container exec; and `bastionResource` for SQL queries that go through a [bastion](/resources/security/bastion-host).
...
The MCP server ships as part of the Stacktape CLI. Updating Stacktape ([`stacktape upgrade`](/cli/upgrade) or updating the npm package) updates the MCP server automatically. After upgrading, you do not need to re-run `mcp:add` — the existing config points to the `stacktape mcp` command, which picks up the new version.

### What happens if a deployment times out through MCP?

Infrastructure operations run as CLI subprocesses. If a deployment exceeds its timeout, the MCP server terminates the subprocess and returns an error response. The dev mode `start` operation has a 12-minute timeout for initial setup. For long-running deployments, run them directly via the [CLI](/cli/deploy) where you get real-time streaming output and no timeout imposed by the MCP layer.
