...
A Stacktape batch job runs a containerized task to completion for work such as data processing, ML training, video encoding, and imports. Use a batch job when the work has a clear finish point, needs more runtime or compute than a request handler, and should be triggered by schedules, HTTP requests, storage events, queues, streams, or event buses.

**Pricing context:** Batch jobs use vCPU and memory billing while the job runs. In us-east-1 list pricing, continuous Fargate usage is approximately ~$7.29/month per 0.25 vCPU plus ~$1.60/month per 512 MB; Spot capacity can save up to 90% when interruptions are acceptable.

## When to use

A batch job is the right choice when the workload should start, do a defined unit of work, and exit. Common examples include nightly exports, object-processing pipelines, one-off video transcodes, queue-driven processing bursts, and ML jobs that can run in a container with explicit CPU, memory, and optional GPU requirements.

- **Long-running jobs** — use a batch job for work that does not fit naturally into a short request/response function.
- **Compute-heavy tasks** — specify `cpu`, `memory`, and optionally `gpu`; Stacktape lets AWS pick a matching instance unless the API reference exposes a narrower option for your use case.
...
A batch job always runs a container image. Stacktape supports the same five container packaging modes used by container workloads: Stacktape image buildpack, custom Dockerfile, prebuilt image, Nixpacks, and external buildpack. Most teams should start with the Stacktape image buildpack, then switch only when the image needs precise runtime control.

| Mode | When to use |
|------|-------------|
| [Stacktape image buildpack](/packaging/containers/stacktape-buildpack) | Build JS/TS, Python, Java, or Go source into a container image with minimal config |
| [Custom Dockerfile](/packaging/containers/custom-dockerfile) | Control the operating system packages, filesystem layout, and image build steps |
| [Prebuilt image](/packaging/containers/prebuilt-image) | Run an image that already exists in a registry |
| [Nixpacks](/packaging/containers/nixpacks) | Let Nixpacks detect and build the app from source |
| [External buildpack](/packaging/containers/external-buildpack) | Use a Cloud Native Buildpack builder and optional buildpack list |
...
export default defineConfig(() => {
  ...
  const exportJob = new BatchJob({
    // [!code focus-start]
    connectTo: ['mainDatabase', 'reportsBucket'],
    // [!code focus-end]
    container: {
      packaging: new StacktapeImageBuildpackPackaging({
  ...
  return { resources: { reportsBucket, mainDatabase, exportJob } };
});`
    }
  ]}
/>

With the resource names above, the job receives database variables such as `STP_MAIN_DATABASE_CONNECTION_STRING`, `STP_MAIN_DATABASE_HOST`, and `STP_MAIN_DATABASE_PORT`, plus bucket variables such as `STP_REPORTS_BUCKET_NAME` and `STP_REPORTS_BUCKET_ARN`. For permissions outside `connectTo`, use `iamRoleStatements`.
...
`retentionDays` accepts the CloudWatch retention values exposed in the API reference, including `1`, `3`, `7`, `14`, `30`, `60`, `90`, `180`, `365`, and longer archival periods. View runtime logs with [`stacktape debug:logs`](/cli/debug-logs) or in the [Stacktape Console](/stacktape-console/console-overview).

## FAQ

### What is a Stacktape batch job?

A Stacktape batch job is a containerized task that runs to completion instead of staying online as a service. It is intended for data processing, ML training, video encoding, imports, and similar workloads with a clear end state. Use the `BatchJob` resource when you want Stacktape to package the container, declare compute needs, and wire event triggers.
...
Yes. A batch job supports `http-api-gateway` and `application-load-balancer` integrations. Use HTTP-triggered batch jobs when a request should enqueue or start long-running compute instead of waiting for the whole task to finish synchronously; see [HTTP triggers](/configuration/triggers/http-triggers).
...
Batch job cost depends on the vCPU, memory, GPU, runtime, and whether on-demand or Spot capacity is used. The provided pricing data describes Fargate-style billing by vCPU-hour and GB-hour, with continuous us-east-1 usage around ~$7.29/month per 0.25 vCPU plus ~$1.60/month per 512 MB. See [cost optimization](/managing-costs/optimization-tips) for broader cost control guidance.
