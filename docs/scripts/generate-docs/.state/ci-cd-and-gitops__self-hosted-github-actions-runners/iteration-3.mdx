...
| **AWS region** | The region for the runner instance | Auto-detected from project deployments, then project default region |
| **Instance type** | EC2 instance size for the runner | `c7a.2xlarge` (8 vCPU, 16 GB RAM) |

At runtime, Stacktape uses the configured runner region. If no runner region is configured, the handler checks the project's existing deployment-region flags, then the project's default region. If no region can be determined, the job is not dispatched and the handler logs an error asking you to configure a region or deploy a stage first. If you do not select an AWS account, the first active connected account is used.
...
If you set a region in the runner configuration, that region is used. Otherwise, Stacktape auto-detects the region from your project's existing deployments — it checks which regions have active stages and picks the first match. If no deployment region is found, it falls back to the project's default region. If no region can be determined, the job is not dispatched and the logs ask you to configure a region or deploy a stage first.
