...
Note: the generic agent-mode reference text below describes "strict JSONL/NDJSON output (one JSON object per line)", but for `debug:logs` specifically the command emits a single pretty-printed JSON object covering the entire response — not one JSON object per line. If you need a strictly newline-delimited stream, transform the output with `jq` after the fact.
...
Both produce JSON output. `--raw` produces JSON only in standard mode and has no effect in Insights mode. `--agent` produces JSON in both standard and Insights modes and additionally disables interactive terminal UI. For `debug:logs`, agent-mode output is a single pretty-printed JSON object covering the entire response, even though the generic agent-mode description elsewhere refers to JSONL/NDJSON. Use `--agent` when you need JSON output from an Insights query or when integrating with AI coding assistants; use `--raw` for simple standard-mode scripting with `jq`.
