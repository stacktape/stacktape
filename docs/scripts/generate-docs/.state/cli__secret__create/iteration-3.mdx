...
Updating a secret changes the value stored in AWS Secrets Manager. If your Stacktape configuration references the secret with `$Secret()`, see [Directives](/configuration/directives) for how the value is resolved.
...
Yes. Running `secret:create` with `--forceUpdate` updates the value stored in AWS Secrets Manager immediately. How that change is picked up by resources that reference the secret with `$Secret()` depends on how the directive is resolved — see [Directives](/configuration/directives) for details.
