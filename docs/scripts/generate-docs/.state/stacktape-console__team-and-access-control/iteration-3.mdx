...
seoDescription: 'Invite team members, assign roles, revoke API keys, and manage organization access in the Stacktape Console. Control permissions for your team''s infrastructure.'
---
...
<Info>The full list of roles and their associated permissions is enforced server-side. The Users page surfaces these four action-level permissions to control what you see and can do on this page.</Info>
...
Once an invitation has been sent, the new member appears in the Users table with status "Invited" until their account is confirmed.
...
<Tip>The Users page exposes separate actions for removing a member and revoking all of that member's [API keys](/stacktape-console/api-keys). For complete offboarding, perform both.</Tip>
...
The invited member has not confirmed their account yet. If the invitation needs to be re-sent, remove them and invite them again.
...
The Users page confirmation dialog states that revocation will immediately invalidate all of that member's active [API keys](/stacktape-console/api-keys) in the organization. This is useful when offboarding a team member or responding to a compromised credential.

### Is removing a member the same as revoking their API keys?

No — the Users page exposes them as two separate actions. For complete offboarding, perform both: remove the member and revoke all of their API keys.
...
The Activity page shows a log of Stacktape CLI operations across your organization and can help track who ran deploys, deletes, scripts, bucket syncs, and other recorded commands.