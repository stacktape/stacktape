...
Build runners execute deployments for Git-backed projects managed in the Stacktape Console — see [GitOps deployments](/ci-cd-and-gitops/gitops-with-console). When you use [custom CI/CD](/ci-cd-and-gitops/custom-ci-cd), your CI system's own runners execute the Stacktape CLI directly — the Console's build runner configuration does not apply.
