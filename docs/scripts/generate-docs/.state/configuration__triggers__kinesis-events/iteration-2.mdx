...
<FeatureComparisonTable
  ...
  features={[
    { name: 'How it works', values: { 'Direct polling': 'Polls each shard ~1/sec', 'Enhanced fan-out': 'Dedicated push connection per shard' } },
...
<Warning>Without `maximumRetryAttempts`, a failing batch can block its shard, causing the iterator age to grow and processing to fall behind the stream tip.</Warning>
