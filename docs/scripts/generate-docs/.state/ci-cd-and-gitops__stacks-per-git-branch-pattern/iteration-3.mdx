...
The [GitOps page](/ci-cd-and-gitops/gitops-with-console) in the Stacktape Console lists configurations by deployment trigger, branch, stage name, AWS region, and AWS account. GitOps configurations support two trigger types: **PR Opened** and **Push to Branch**. To deploy a long-lived branch, create a configuration with the Push to Branch trigger targeting your branch and desired stage name. Pushes to the configured branch trigger deployments to the named stage — the handler requires a branch ref and a commit SHA, so branch-deletion events and pushes missing a commit SHA are ignored.
...
2. **Temporary configuration** — A temporary GitOps configuration is created for the PR branch, linking it to the `pr-{number}` stage. The temporary configuration stores the `deleteStackAfterPrClose` preference and is removed when the PR closes.
3. **PR closed or merged** — The handler looks up the temporary PR configuration for the PR branch and number, which carries the `deleteStackAfterPrClose` setting. That configuration is then used to drive the preview stage's deletion handling.
