...
A Stacktape SolidStart resource deploys a SolidStart app with SSR on AWS Lambda, static assets on S3, and a CloudFront CDN. Set `appDirectory` to the directory containing `app.config.ts`, and Stacktape handles the Vinxi build, SSR function, asset bucket, CDN routing, and optional custom domains. Use `environment` or `connectTo` to pass values into the SSR Lambda.
...
With `connectTo: ['mainDatabase']`, the SolidStart resource follows the `STP_[RESOURCE_NAME]_[PARAM]` pattern. For a relational database named `mainDatabase`, documented injected variables include `STP_MAIN_DATABASE_CONNECTION_STRING`, `STP_MAIN_DATABASE_HOST`, and `STP_MAIN_DATABASE_PORT`. Set `joinDefaultVpc: true` on `serverLambda` when connecting to VPC-protected resources like databases or Redis clusters.
