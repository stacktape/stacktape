...
- **You need to mix subscriber types.** A single SNS topic can deliver to Lambda functions, SQS queues, HTTP endpoints, and email simultaneously. The `SnsIntegration` trigger documents the Lambda function subscriber path.
