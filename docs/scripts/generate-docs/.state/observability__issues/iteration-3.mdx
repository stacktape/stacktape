...
Each issue carries an error-type label shown in both the Console table and the CLI output. The available labels are:
...
Use the label to group similar errors at a glance and to filter your triage queue.
...
Use **Resolve** when you've shipped a fix for the error. Use **Ignore** when the error is expected or non-actionable (for example, a known third-party timeout you can't prevent). Both statuses can be reverted to Open if the error recurs or you change your mind. The CLI reference includes separate pages for [`issues:resolve`](/cli/issues-resolve), [`issues:ignore`](/cli/issues-ignore), and [`issues:reopen`](/cli/issues-reopen).
...
The CLI reference includes separate pages for the status-change commands:
...
Each issue carries one of six error-type labels: Uncaught, Caught, Unhandled Rejection, Exit Error, Handler Not Found, and Panic. The label appears in both the Console table and the CLI output and helps you group similar errors during triage.
...
Yes. [`stacktape issues:list`](/cli/issues-list) accepts API key authentication, supports filtering by project, stage, and status, and emits JSON in agent mode for programmatic consumption. The CLI reference also covers separate commands for [`issues:resolve`](/cli/issues-resolve), [`issues:ignore`](/cli/issues-ignore), and [`issues:reopen`](/cli/issues-reopen).
