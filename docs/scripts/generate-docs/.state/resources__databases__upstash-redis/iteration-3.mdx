...
Upstash Redis requires `providerConfig.upstash` in your Stacktape config. This provides Stacktape with the credentials it uses to create and manage databases in your Upstash account.
...
Remember to also set `providerConfig.upstash` at the top level of your config (see [Provider configuration](#provider-configuration)) — without it, the stack will fail to deploy.
