...
seoDescription: 'Create a secure SSM tunnel through a bastion host to access private VPC resources like databases, Redis clusters, and private services from your local machine.'
---
...
<Info>
The tunnel uses AWS SSM Session Manager under the hood. No inbound ports need to be open on the bastion. The [AWS SSM Session Manager plugin](https://docs.aws.amazon.com/systems-manager/latest/userguide/session-manager-working-with-install-plugin.html) must be installed on your machine.
</Info>
