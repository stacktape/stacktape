...
Stacktape provides a suite of CLI commands for inspecting deployed stacks without leaving your terminal. This page documents `debug:logs` and `debug:metrics` in depth and links to the dedicated CLI reference pages for the rest. The container execution commands (`debug:container-exec`, `container:session`) and the bastion shell command (`bastion:session`) open interactive sessions where you can run arbitrary commands inside live infrastructure, so treat them with appropriate care in production stages.
