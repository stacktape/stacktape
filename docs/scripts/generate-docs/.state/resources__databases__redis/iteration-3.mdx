...
title: 'Redis'
order: 47
seoTitle: 'Redis Cluster | Stacktape Docs'
seoDescription: 'Deploy managed Redis with Stacktape. Sub-millisecond caching, sessions, and pub/sub with in-transit encryption, sharding, and automatic failover.'
---

# Redis

A Stacktape Redis cluster provides a managed, in-memory data store for caching, sessions, queues, and real-time data. It delivers sub-millisecond read and write latency for caching, session storage, real-time leaderboards, rate limiting, and pub/sub messaging. Traffic is encrypted in transit, and the cluster runs inside your stack's VPC with no public endpoint.

ElastiCache (Redis) instance pricing starts at ~$11.52/month for a single `cache.t4g.micro` node (us-east-1). You pay per node-hour for the instance size you choose, with no per-request charges. Larger memory-optimized nodes (`cache.r7g.large` at ~$0.20/hr) and multi-node clusters scale cost linearly.
...
This creates a single-node Redis 6.2 cluster on a `cache.t4g.micro` instance (~$11.52/month). Traffic between clients and the cluster is encrypted in transit.
...
`numShards` is only used with `enableSharding: true` and defaults to `1`. Sharding requires `numReplicaNodes >= 1`, and automatic failover is always enabled for sharded clusters.
...
Redis logging config controls slow-query logging sent to CloudWatch. `logging.disabled` defaults to `false`, `format` defaults to `json`, and `retentionDays` defaults to `90` when logging is configured. View logs with [`stacktape debug:logs`](/cli/debug-logs) or in the Stacktape Console.
