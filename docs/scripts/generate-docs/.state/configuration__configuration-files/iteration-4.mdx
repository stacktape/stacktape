...
<Info>Additional directives — `$Var()`, `$File()`, and `$Format()` — are available using the `$DirectiveName()` string syntax in both TypeScript and YAML configs, but are not importable as TypeScript functions.</Info>
...
Use TypeScript for most projects. It catches configuration errors before deployment, provides full IDE support, and lets you use conditional logic and loops for per-stage differences. YAML is appropriate for very small, static projects or when your team strongly prefers declarative-only configuration. TypeScript and YAML describe the same Stacktape configuration model, so you can move between them later.

### Can I switch from YAML to TypeScript?

YAML and TypeScript describe the same Stacktape configuration model. When converting, keep the resource names and properties the same so Stacktape can produce the same intended infrastructure — rename your `stacktape.yml` to `stacktape.ts` and rewrite the resources using the `defineConfig` pattern and resource classes.
