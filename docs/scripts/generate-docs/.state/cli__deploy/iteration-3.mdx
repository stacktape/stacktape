...
Hot-swap attempts a faster code-only deploy by updating functions and multi-container workloads directly, without CloudFormation. In the implementation, Stacktape also evaluates container workloads for ECS hotswap. This is intended for faster code-only deploys during development.
