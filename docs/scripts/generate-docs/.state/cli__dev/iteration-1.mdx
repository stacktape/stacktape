...
The `stacktape dev` command runs your application locally for development and debugging. It starts workloads on your machine, emulates databases in Docker, injects environment variables and secrets, and streams logs — giving you a production-like environment without waiting for full deployments.
...
- Automatically creates tunnels so AWS Lambda functions can reach your local databases
...
| Databases (emulated) | PostgreSQL, MySQL, MariaDB, Redis, DynamoDB, OpenSearch |
...
In normal mode, databases run locally by default. Use this flag to connect specific resources to their deployed AWS counterparts instead.
...
Disables automatic tunneling for Lambda functions. In normal mode, Stacktape creates tunnels so that Lambda functions deployed to AWS can reach your locally emulated databases. Disable this if you don't need Lambda-to-local connectivity or if tunneling causes issues.
...
Runs the dev command as a detached daemon with an HTTP API for programmatic control. Designed for integration with AI coding agents and automation tools. The daemon spawns in the background and the parent process exits after confirming the agent is ready.

```bash
stacktape dev --stage dev --region eu-west-1 --agent
```

Specify a custom port for the agent HTTP server:
...
While running in interactive mode, you can use keyboard commands:

| Command | Action |
|---------|--------|
| `rs` | Rebuild all workloads |
| `rs <name>` | Rebuild a specific workload |
