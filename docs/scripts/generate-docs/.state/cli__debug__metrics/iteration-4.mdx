...
By default, `debug:metrics` renders a chart in the terminal via `printMetricsChart`. Pass `--agent` to disable the chart and instead print a single JSON object — useful for scripts and AI coding agents. The JSON payload contains `resource`, `metric`, `period`, `stat`, `unit`, and a chronologically sorted `datapoints` array with `timestamp` and `value` fields.
...
Pass the `--agent` flag to disable the interactive chart and print a single JSON object instead. The object includes the resource name, metric name, period, statistic, unit, and a chronologically sorted `datapoints` array. This makes it straightforward for coding assistants and scripts to parse the results.
...
### Where can I view metrics outside the CLI?

For a broader metrics workflow, see [Metrics](/observability/metrics). Use the `debug:metrics` CLI when you want metric data from the terminal — for quick checks, scripting, or AI agent integrations where you don't want to leave the shell.