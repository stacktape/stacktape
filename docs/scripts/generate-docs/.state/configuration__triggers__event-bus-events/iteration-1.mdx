...
order: 9
seoTitle: 'Event Bus Events — Trigger Functions on EventBridge | Stacktape Docs'
seoDescription: 'Trigger Lambda functions, batch jobs, and SQS queues from Amazon EventBridge events. Pattern-based routing, payload transformation, and dead-letter queues — configured declaratively in Stacktape.'
---

# Event Bus Events

Stacktape event bus events trigger a [Lambda function](/resources/compute/lambda-function), [batch job](/resources/compute/batch-job), or [SQS queue](/resources/messaging/sqs-queue) when an event matching a pattern arrives on an Amazon EventBridge event bus. EventBridge is a serverless event router — you publish structured JSON events to a bus, define patterns, and EventBridge delivers matching events to your targets. Stacktape wires the rule, permissions, and dead-letter queue for you.
...
- **Routing events to queues** — EventBridge can deliver matching events directly to an [SQS queue](/resources/messaging/sqs-queue), buffering them for later processing instead of invoking a function immediately.
...
This example creates a custom [event bus](/resources/messaging/event-bus) and a Lambda function that triggers whenever an event with `detail-type` of `"OrderPlaced"` arrives from the `"my-app.orders"` source. Stacktape creates the EventBridge rule, grants invoke permissions, and wires the target automatically.
...
Three resource types support event bus triggers:

| Target resource | Integration class | Extra properties |
|---|---|---|
| [Lambda function](/resources/compute/lambda-function) | `EventBusIntegration` | — |
| [Batch job](/resources/compute/batch-job) | `EventBusIntegration` | — |
| [SQS queue](/resources/messaging/sqs-queue) | `SqsQueueEventBusIntegration` | `messageGroupId` (required for FIFO queues) |

Lambda functions and batch jobs use the same `EventBusIntegration` class in their `events` array. SQS queues use a specialized variant that adds `messageGroupId` support — required when the target queue has `fifoEnabled: true` so that EventBridge can assign messages to the correct FIFO message group.

## Choosing the event source

Every event bus integration needs exactly one source. Specify one of `eventBusName`, `eventBusArn`, or `useDefaultBus`. Specifying more than one is a validation error.
...
By default, EventBridge delivers the full event envelope to your target. The envelope includes metadata fields like `source`, `account`, `time`, and `region` alongside your `detail` payload. You can customize what gets delivered using one of three mutually exclusive options.

Specifying more than one of `input`, `inputPath`, or `inputTransformer` is a validation error.

### Fixed input

Use `input` to replace the event payload entirely with a static JSON object. The target receives this object regardless of the actual event content. Useful when your function only needs to know it was triggered — for example, a scheduled cleanup that runs on any order event.
...
The function receives `{"message":"Order 12345 placed by Jane."}` instead of the full EventBridge event envelope. Placeholders in angle brackets (`<orderId>`, `<customer>`) are replaced with the values extracted by `inputPathsMap`.

## Delivery failure handling

In rare cases — such as when your Lambda function is throttled or unavailable — EventBridge may fail to deliver an event. Use `onDeliveryFailure` to route failed events to an [SQS queue](/resources/messaging/sqs-queue) so they aren't silently dropped. Specify either `sqsQueueName` (for a queue in the same stack) or `sqsQueueArn` (for an external queue). Specifying both is a validation error.
...
Without `onDeliveryFailure`, events that fail delivery are retried by EventBridge for up to 24 hours (per AWS default behavior) and then dropped permanently. For production workloads, always configure a dead-letter queue.

## FAQ

### What is Amazon EventBridge?

Amazon EventBridge is a serverless event bus service from AWS. It receives structured JSON events from your applications, SaaS partners, or AWS services and routes them to targets based on rules you define. Stacktape's event bus integration lets you wire EventBridge rules to Lambda functions, batch jobs, and SQS queues declaratively — without managing IAM policies, rule targets, or dead-letter queues by hand.
...
### Can I route EventBridge events to an SQS queue instead of a Lambda function?

Yes. [SQS queues](/resources/messaging/sqs-queue) support event bus triggers through the `SqsQueueEventBusIntegration` class in their `events` array. This buffers matched events in a queue for later processing — useful when you want to decouple event arrival from processing, or when the downstream consumer needs batching. For FIFO queues, you must also set `messageGroupId` to control message ordering.