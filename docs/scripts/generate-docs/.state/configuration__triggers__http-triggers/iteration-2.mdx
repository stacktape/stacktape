...
HTTP triggers route incoming web requests to your Stacktape [Lambda functions](/resources/compute/lambda-function) and container workloads. Two integration types are available: [HTTP API Gateway](/resources/networking/http-api-gateway) for serverless routing by method and path, and [Application Load Balancer](/resources/networking/application-load-balancer) for content-based routing with headers, hosts, and query parameters.
...
HTTP API Gateway integrations support an optional `authorizer` to protect individual routes. Stacktape supports both Cognito authorizers and Lambda authorizers. When configured, unauthorized requests are rejected with a `401 Unauthorized` response before they reach your function — saving invocation costs and reducing attack surface. For details on setting up authorizers, see [User auth pools](/resources/security/user-auth-pool).
...
ALB integrations support six routing conditions. For headers and query parameters, all configured conditions must be met for the request to be routed. Each condition's `values` list matches any listed value — for example, `paths: ['/users', '/posts']` matches requests to either path.
...
Container workload resources — [web services](/resources/compute/web-service), [private services](/resources/compute/private-service), and [multi-container workloads](/resources/compute/multi-container-workload) — receive HTTP traffic through the same underlying HTTP API Gateway and Application Load Balancer infrastructure as Lambda functions, but the mechanism differs by resource type.

**Web services** and **private services** configure their traffic source through the `loadBalancing` property on the resource itself rather than through per-container event triggers. A web service defaults to `http-api-gateway` load balancing and can be switched to `application-load-balancer` or `network-load-balancer`. A private service defaults to `service-connect` and can use `application-load-balancer`. See the [web service](/resources/compute/web-service) and [private service](/resources/compute/private-service) pages for configuration details.

**Multi-container workloads** use per-container `events` arrays with `MultiContainerWorkloadHttpApiIntegration` and `MultiContainerWorkloadLoadBalancerIntegration`. These integration types mirror the Lambda integration types but add a required `containerPort` property that specifies which port inside the container receives the routed traffic. All other routing properties (`method`, `path`, `priority`, `paths`, `methods`, `hosts`, `headers`, `queryParams`, `sourceIps`) work the same as their Lambda function counterparts.

**Worker services** are background containers with no inbound HTTP traffic — they do not accept HTTP triggers.

Multi-container workloads also support three additional integration types for internal networking:

- **`service-connect`** — opens a container port for connections from other compute resources in the same stack. Supports an optional `alias` for service discovery (e.g., `http://my-service:8080`) and a `protocol` declaration (`http`, `http2`, or `grpc`) for protocol-specific metrics.
...
HTTP API Gateway integrations support an `authorizer` property that rejects unauthorized requests at the gateway level before they reach your function. Stacktape supports Cognito-based authorizers and Lambda authorizers. ALB integrations do not have built-in authorizer support — implement authentication in your application code instead. See [User auth pools](/resources/security/user-auth-pool) for setting up authorization with API Gateway.

### What is the maximum request payload size for HTTP API Gateway?

AWS HTTP API Gateway accepts request and response payloads up to 10 MB. For larger payloads, use an Application Load Balancer (which does not enforce this limit) or upload files directly to an [S3 bucket](/resources/storage/s3-bucket) using presigned URLs. Most REST and GraphQL APIs stay well within the 10 MB boundary.

### Can I use a custom domain with HTTP triggers?

Yes. Both HTTP API Gateways and Application Load Balancers support [custom domains](/resources/networking/custom-domains). Attach your domain to the gateway or load balancer resource, and all routes served through it become available under your domain name. Custom domains are configured on the gateway or load balancer resource, not on individual trigger integrations.
