...
The `priority` property is required for every ALB integration. Load balancer rules are evaluated from the lowest priority number to the highest — the first rule that matches an incoming request handles the request.
...
**Web services** and **private services** configure their traffic source through properties on the resource itself rather than through per-container event triggers. See the [web service](/resources/compute/web-service) and [private service](/resources/compute/private-service) pages for configuration details.

**Multi-container workloads** use per-container `events` arrays with `http-api-gateway` and `application-load-balancer` integrations. These integration types mirror the Lambda integration types but add a required `containerPort` property that specifies which port inside the container receives the routed traffic. All other routing properties (`method`, `path`, `priority`, `paths`, `methods`, `hosts`, `headers`, `queryParams`, `sourceIps`) work the same as their Lambda function counterparts.
