...
1. **The Stacktape GitHub App installed.** Install the Stacktape GitHub App for your GitHub account or organization in the Console. Once installed, repositories in that installation are available for GitOps.
...
3. **A linked repository.** Your Stacktape project must be linked to a repository through the GitHub App installation. The Console uses this link to list available branches.
...
Create a GitOps configuration in the Console that maps a branch to a stage. Navigate to your project's GitOps page and click **New GitOps configuration**. The configuration requires a branch to watch, a stage name, a target AWS account, and a region. When the GitHub App receives a push event for that branch, the Console determines which configurations match and starts a deployment for each one.
...
Create a GitOps configuration with the **PR Opened** trigger and select the target branch (the branch PRs are opened against, typically `main`). When a PR is opened against that branch, the Console deploys a new stage. The Console displays the stage name as `pr-{#number}` (e.g. `pr-42` for PR #42) when no explicit stage name is configured.
...
When configuring PR previews, you can enable deletion of the preview stack after the PR is merged or closed. When this option is set and the GitHub App receives a PR close event, the Console starts a stack deletion on the preview stage. This prevents preview stages from accumulating and incurring ongoing AWS costs.
...
Every GitOps deployment needs a compute environment to run the deployment. GitOps deployments run on a build runner. For a detailed comparison of runner types, configuration options, and cost tradeoffs, see [Build runners](/ci-cd-and-gitops/build-runners).
...
The handler processes all matching configurations in parallel. If one configuration fails (e.g. due to an invalid config file or AWS permission issue), other matching configurations still proceed independently. The handler returns a summary indicating how many actions succeeded or failed.
...
<Warning>
Deleting a GitOps configuration stops future webhook-triggered deployments but does not delete any currently deployed stages. If you have a `main` → `prod` push-to-deploy config and you delete the config, your `prod` stage keeps running. To remove the stage, use [`stacktape delete`](/cli/delete) or delete it through the Console.
</Warning>
...
In the Stacktape Console, navigate to your project's GitOps page and create a new configuration. Choose "Push to Branch" as the trigger, select the branch to watch, set a stage name, and pick your AWS account and region. Every push to that branch triggers a deployment automatically. No pipeline files or CI configuration is needed.
...
The GitHub App handler processes each matching configuration independently. If one deployment fails, others triggered by the same event still proceed. Common failure causes include invalid Stacktape configuration, missing [secrets](/configuration/secrets), or AWS permission issues on the connected account. Check your deployment output and the [CLI logs reference](/cli/debug-logs) for diagnostics.

### How much does AWS CodeBuild cost for deployments?

AWS CodeBuild pricing is based on build minutes and compute type. The AWS free tier includes 100 build minutes per month on the smallest instance type. Beyond that, pricing varies by compute type and region. A typical Stacktape deployment takes a few minutes depending on stack complexity and packaging mode. See [Build runners](/ci-cd-and-gitops/build-runners) for runner type comparison and the [AWS CodeBuild pricing page](https://aws.amazon.com/codebuild/pricing/) for current rates.
...
No changes to your repository code are needed. The Stacktape GitHub App receives webhook events automatically once installed. Your repository needs a valid Stacktape configuration file (a `stacktape.ts` or `stacktape.yml` in the repo root). The GitOps configuration in the Console points to the branch and handles the rest.
