...
| `cliArgs`     | `StacktapeArgs`  | Parsed CLI arguments object for the current command (stage, region, configPath, etc.). See the [CLI reference](/cli/deploy) for the full list of arguments per command. |
| `awsProfile`  | `string`         | Locally-configured AWS profile used to execute the operation. Does not apply when the AWS account is connected in automatic mode. |
