...
- **Admin-only account creation** — set `allowOnlyAdminsToCreateAccount: true` to restrict sign-ups so only your team can create accounts (useful for internal tools and B2B apps). Pair with `unusedAccountValidityDays` to control how long admin-issued temporary passwords remain valid before expiring.
...
For internal tools, B2B dashboards, or any app where you do not want public sign-ups, set `allowOnlyAdminsToCreateAccount: true`. Accounts must then be created through an admin flow (using the Cognito `AdminCreateUser` API from your backend). Cognito issues a temporary password for each admin-created user — control how long it remains valid with `unusedAccountValidityDays`.
...
Setting `unusedAccountValidityDays: 7` means that if an admin-created user does not sign in within 7 days, the temporary password expires and a new one must be issued. The default is 31 days.
...
| `STP_AUTH_PROVIDER_URL` | OAuth/OIDC provider URL for the pool |
| `STP_AUTH_DOMAIN` | Hosted UI domain (custom domain or Cognito-managed domain) |
| `STP_AUTH_CLIENT_SECRET` | Client secret (only injected when `generateClientSecret: true`) |
...
All six [referenceable parameters](#referenceable-parameters) are also accessible via [`$ResourceParam`](/configuration/referenceable-parameters) for use in config directives, hooks, and scripts.
...
Each identity provider requires a `clientId` and `clientSecret` obtained from the provider's developer console. For SAML providers, the primary configuration is typically done through `providerDetails` with your IdP metadata (metadata URL or XML), while `clientId` and `clientSecret` serve as identifiers rather than OAuth credentials. Store secrets using the [`$Secret` directive](/configuration/directives) to avoid exposing them in your config. The syntax is `$Secret('secretName')`, where the secret is created via the [`stacktape secret:create`](/cli/secret-create) command or through the [Stacktape Console](/configuration/secrets).
...
The `hostedUiDomainPrefix` sets the first part of the Hosted UI URL: `https://<prefix>.auth.<region>.amazoncognito.com`. Choose a prefix that matches your project name. You can customize the Hosted UI appearance with the `hostedUiCSS` property to override colors, fonts, and layout.
...
Yes. The `identityProviders` property supports Google, Facebook, SignInWithApple, LoginWithAmazon, OIDC, and SAML providers. Each provider requires a `clientId` and `clientSecret` from the provider's developer console. Store secrets securely using the [`$Secret` directive](/configuration/directives). Stacktape creates the necessary `AWS::Cognito::UserPoolIdentityProvider` resources and registers them in the user pool client's `SupportedIdentityProviders` automatically.
...
When you add a user auth pool to a resource's [`connectTo`](/configuration/connecting-resources) list, Stacktape injects `STP_[RESOURCE_NAME]_ID` (pool ID), `STP_[RESOURCE_NAME]_CLIENT_ID` (client ID), `STP_[RESOURCE_NAME]_ARN` (pool ARN), `STP_[RESOURCE_NAME]_PROVIDER_URL` (provider URL), and `STP_[RESOURCE_NAME]_DOMAIN` (Hosted UI domain). When `generateClientSecret` is enabled, `STP_[RESOURCE_NAME]_CLIENT_SECRET` is also injected. The connecting resource receives full control over the user pool.
