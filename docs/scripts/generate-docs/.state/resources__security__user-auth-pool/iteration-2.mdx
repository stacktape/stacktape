...
Cognito charges per monthly active user (MAU). The first 50,000 MAUs with direct sign-in (username/password) are free. Beyond that, pricing is tiered and decreases at higher volumes. Social login and SAML/OIDC federation cost more per MAU and have no free tier.
...
This creates a user pool where new users verify their email address with a one-time code. By default, users can sign in with either their email address or phone number (`allowEmailAsUserName` and `allowPhoneNumberAsUserName` both default to `true`). Unused admin-created accounts expire after 31 days (`unusedAccountValidityDays` defaults to `31`).

## Connecting to resources

To use the user pool from a [Lambda function](/resources/compute/lambda-function), [web service](/resources/compute/web-service), or any other compute resource, add it to the resource's [`connectTo`](/configuration/connecting-resources) list. Stacktape automatically injects environment variables with the pool's connection details and grants broad Cognito API permissions to the connecting resource, so your code can call any Cognito API operation against this pool.
...
Each identity provider requires a `clientId` and `clientSecret` obtained from the provider's developer console. Store the client secret using the [`$Secret` directive](/configuration/directives) to avoid exposing it in your config. The syntax is `$Secret('secretName')`, where the secret is created via the [`stacktape secret:create`](/cli/secret-create) command or through the [Stacktape Console](/configuration/secrets).
...
When you add an identity provider, Stacktape creates a separate `AWS::Cognito::UserPoolIdentityProvider` resource and registers it in the user pool client. If you do not specify `attributeMapping`, Stacktape defaults to mapping `email` from the provider to the Cognito `email` attribute. You can override this with the `attributeMapping` property to map additional attributes like `given_name` or `picture`.

If you omit `authorizeScopes`, Stacktape uses a reasonable default per provider. Set `authorizeScopes` explicitly when you need additional provider permissions beyond the default set — for example, requesting access to a user's Google Calendar or Facebook friend list.
...
The `hostedUiDomainPrefix` sets the first part of the Hosted UI URL: `https://<prefix>.auth.<region>.amazoncognito.com`. Choose a prefix that matches your project name — it must be globally unique across all Cognito users. You can customize the Hosted UI appearance with the `hostedUiCSS` property to override colors, fonts, and layout.
...
The user pool client's OAuth configuration controls which flows, scopes, and redirect URLs are allowed. These properties are configured at the top level of the user pool properties. Configure them explicitly when using the Hosted UI or integrating with external identity providers:
...
If you do not configure a password policy, Cognito's platform defaults apply. For production applications, set at least `minimumLength: 12` and enable `requireNumbers` and `requireUppercase` — this provides meaningful security without frustrating users.
...
<Tip>
Configure token lifetimes explicitly to match your security requirements. If these properties are not set, Cognito platform defaults apply. Shorter access token lifetimes reduce the window for token theft at the cost of more frequent refresh calls. Longer refresh token lifetimes keep users signed in longer but extend the window for session hijacking. A common starting point is 3600 seconds (1 hour) for access and ID tokens.
</Tip>
...
The firewall must use `scope: 'regional'` — a CDN-scoped firewall cannot be associated with a Cognito user pool. Stacktape validates this constraint and will report an error if a CDN-scoped firewall is referenced.
...
Yes. The `identityProviders` property supports Google, Facebook, SignInWithApple, LoginWithAmazon, OIDC, and SAML providers. Each provider requires a `clientId` and `clientSecret` from the provider's developer console. Store secrets securely using the [`$Secret` directive](/configuration/directives). Stacktape creates the necessary Cognito identity provider resources and registers them in the user pool client automatically.
...
When you add a user auth pool to a resource's [`connectTo`](/configuration/connecting-resources) list, Stacktape injects `STP_[RESOURCE_NAME]_ID` (pool ID), `STP_[RESOURCE_NAME]_CLIENT_ID` (client ID), and `STP_[RESOURCE_NAME]_ARN` (pool ARN). The connecting resource also receives broad Cognito API permissions for the pool, so your code can call operations like `AdminCreateUser`, `InitiateAuth`, or `ListUsers`.
...
Use a Stacktape user auth pool when you want authentication integrated with your AWS infrastructure (IAM permissions, API Gateway JWT validation, Lambda hooks) without managing another vendor. Use Auth0 or Clerk when you need advanced features like passwordless login with passkeys, pre-built UI components for multiple frameworks, or need to stay cloud-agnostic. Cognito's free tier of 50,000 MAUs makes it cost-effective for most apps.
