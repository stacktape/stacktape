...
title: 'bucket:sync'
order: 155
seoTitle: 'bucket:sync CLI Command | Stacktape Docs'
seoDescription: 'Sync a local directory to an S3 bucket with Stacktape. Mirrors files, removes stale objects, invalidates CDN cache, and applies framework-optimized HTTP headers.'
---
...
<Warning>
Files in the destination bucket that are not present in the source directory are deleted. This is a one-way mirror sync, not an additive upload.
</Warning>
