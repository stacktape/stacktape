...
Guardrails are managed in the [Stacktape Console](/stacktape-console/console-overview), not in your project's `stacktape.ts` file. The property shapes described below correspond to the `GuardrailDefinition` union type defined in Stacktape's configuration types — each variant has a `type` discriminator (for example `stage-restriction`) and a `properties` payload (for example `allowedStages`).

Because guardrails are not declared in project config files, there are no `defineConfig` code examples on this page. The rest of the page documents the structure of each deployment guardrail variant and when to reach for it.
...
Command restriction blocks specific CLI commands. Set a `blockedCommands` list with command names — the property accepts a string array, with the JSDoc example showing `["delete", "rollback"]`. Any attempt to run a listed command is rejected.
...
Yes. The command restriction guardrail accepts a list of specific commands in `blockedCommands`. Adding `delete` to the list blocks the delete command while other commands stay available. The JSDoc example uses `["delete", "rollback"]`, but you can block any combination that fits your policy.

### Where do I configure guardrails?

Guardrails are managed in the [Stacktape Console](/stacktape-console/console-overview), not in your project's `stacktape.ts` file. The property shapes documented on this page correspond to the `GuardrailDefinition` union type defined in Stacktape's configuration types.
...
The operation is blocked. For example, a deployment targeting a stage outside `allowedStages` is not permitted by the stage-restriction guardrail. Fix the deployment parameters or configuration to comply with the active guardrails, then retry.
