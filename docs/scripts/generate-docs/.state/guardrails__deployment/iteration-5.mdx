...
The guardrail type definitions are modeled as a `GuardrailDefinition` union — each variant has a `type` discriminator (for example `stage-restriction`) and a `properties` payload (for example `allowedStages`). In Stacktape docs, guardrail management is described as Console-managed rather than project-config-managed; see the [Stacktape Console](/stacktape-console/console-overview) for the management surface.
...
Command restriction defines commands that are blocked. Set a `blockedCommands` list with command names — the property accepts a string array, with the JSDoc example showing `["delete", "rollback"]`. Use it for commands your organization does not want allowed by policy.
...
- **Discourage accidental deletions.** Add `delete` to `blockedCommands` so that stack removal requires a deliberate policy change in the Console before the command becomes allowed again.
...
Yes. The command restriction guardrail accepts a list of specific commands in `blockedCommands`. Adding `delete` to the list expresses that the delete command should not be allowed, while other commands remain unaffected by this guardrail. The JSDoc example uses `["delete", "rollback"]`, but you can block any combination that fits your policy.

### Where do I configure guardrails?

In Stacktape docs, guardrail management is described as being handled in the [Stacktape Console](/stacktape-console/console-overview) rather than in your project's `stacktape.ts` file. The property shapes documented on this page correspond to the `GuardrailDefinition` union type defined in Stacktape's configuration types.
