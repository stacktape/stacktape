...
seoDescription: 'Remove a deployed Stacktape stack and all its AWS resources with the delete command. Supports lifecycle hooks, termination protection, and CI/CD-friendly auto-confirm.'
---

# delete

The `stacktape delete` command permanently removes a deployed stack and all its AWS resources. It deletes deployment artifacts (Lambda zips, container images in ECR), tears down the CloudFormation stack, and cleans up associated infrastructure. Back up any data you need before running this command — the operation is irreversible.
...
**`--autoConfirmOperation`** — Stacktape prompts for confirmation before deleting a stack. Pass this flag to skip the prompt. Required in CI/CD pipelines and non-interactive environments — without it, the command fails in non-TTY contexts.

```bash
stacktape delete --stage staging --region eu-west-1 --autoConfirmOperation
```

**`--configPath`** — Path to your Stacktape configuration file. The config is optional for deletion, but providing it enables `beforeDelete` and `afterDelete` [lifecycle hooks](/configuration/hooks-and-scripts). Without it, hooks are skipped.
...
If you provide a configuration file via `--configPath`, Stacktape runs any `beforeDelete` hooks before tearing down the stack. This is useful for exporting data, cleaning up external resources, or notifying other systems. `afterDelete` hooks run after the stack is removed. Both hook types require `--configPath` and `--stage` to be provided.
...
- [`preview-changes`](/cli/preview-changes) — See what a deployment would change before applying it.
...
Yes. The configuration file is optional for the delete command. You can identify the stack using `--projectName`, `--stage`, and `--region` flags directly. The only trade-off is that `beforeDelete` and `afterDelete` lifecycle hooks will not run without a config file.

### How do I delete a stack in a CI/CD pipeline?

Pass `--autoConfirmOperation` to skip the interactive confirmation prompt. Without this flag, the command fails in non-TTY environments like CI runners. Example: `stacktape delete --stage staging --region eu-west-1 --autoConfirmOperation`.

### How do I delete a stack with termination protection?

You cannot delete a stack that has `terminationProtection` enabled. First [deploy](/cli/deploy) an update that sets `terminationProtection` to `false`, then run the delete command.
...
Deletion time depends on the number and type of resources in the stack. Stacks with databases, VPCs, or large container clusters take longer because AWS must drain connections and clean up dependencies. A minimal stack deletes in under a minute; complex stacks can take 10-20 minutes.

### What if a delete operation fails partway through?

If CloudFormation encounters an error during deletion (e.g., a resource cannot be deleted due to dependencies), the stack enters a `DELETE_FAILED` state. You can retry the delete command, or use [`cf:rollback`](/cli/cf-rollback) to recover. In some cases, you may need to manually remove the blocking resource in the AWS Console before retrying.
