...
The `stacktape debug:aws-sdk` command executes read-only AWS SDK v3 operations against resources in your AWS account. Use it to list Lambda functions, describe DynamoDB tables, inspect S3 objects, or query any supported AWS service directly from your terminal. Stacktape blocks command names whose prefix is not on the read-only allowlist before the request reaches AWS.
...
Stacktape blocks command names that do not start with `List`, `Get`, `Describe`, `Head`, or `Batch`. The check is prefix-based, so use `Batch*` operations only when you know the AWS operation is read-oriented. Attempting any other operation — such as `PutItem`, `DeleteFunction`, or `Scan` — returns an error listing the allowed prefixes before the request reaches AWS.
...
With `--agent`, the command writes a JSON object with `ok`, `service`, `command`, and `data` fields. This is useful for programmatic consumption by scripts or AI coding assistants.

## No config file required

Unlike most Stacktape commands, `debug:aws-sdk` does not require a Stacktape configuration file. You need `--region`, `--service`, and `--command`; use `--profile` or `--awsAccount` only to choose which credentials Stacktape should use. `--awsAccount` selects a connected AWS account from the [Stacktape Console](/stacktape-console/connecting-your-aws-account). This makes the command useful for quick inspection of any AWS account you have access to.
...
Stacktape blocks command names that do not start with `List`, `Get`, `Describe`, `Head`, or `Batch`, and returns an error before the request reaches AWS. The check is prefix-based, so a `Batch*` operation that happens to perform writes on the AWS side would still pass the allowlist — only invoke `Batch*` commands you have confirmed are read-oriented. For known write operations like `PutItem`, `DeleteFunction`, or `UpdateTable`, use the AWS CLI or SDK directly.

### Do I need a Stacktape config file to use this command?

No. Unlike [`deploy`](/cli/deploy) or [`dev`](/cli/dev), this command does not require a `stacktape.ts` or `stacktape.yml` file. You need `--region`, `--service`, and `--command`; use `--profile` or `--awsAccount` only to choose which credentials Stacktape should use. This makes it useful for ad-hoc inspection of any AWS account.
...
Yes. With `--agent`, the command writes a JSON object with `ok`, `service`, `command`, and `data` fields, which is straightforward to parse in scripts or chain with other tools. Without `--agent`, the output is human-readable and includes formatting that may be harder to parse programmatically.
...
Both let you call AWS APIs. `debug:aws-sdk` initializes debug-agent credentials automatically, restricts operations to a read-only prefix allowlist, and integrates with Stacktape's credential management (`--profile`, `--awsAccount`). The AWS CLI gives full read-write access and requires you to manage credentials separately. Use `debug:aws-sdk` when you want the allowlist guardrail; use the AWS CLI when you need write operations or unsupported services.
