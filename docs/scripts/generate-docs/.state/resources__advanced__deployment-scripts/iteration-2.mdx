...
A Stacktape deployment script runs a [Lambda function](/resources/compute/lambda-function) automatically during stack deployment or deletion. Use it for database migrations, seed data, cache warming, or cleanup tasks that execute inside AWS with direct network access to your stack's VPC-protected resources. Deployment scripts use AWS Lambda pricing — you pay only for execution time.
...
- **Database migrations** — run Prisma, Drizzle, or raw SQL migrations after a deploy, with direct network access to your database
...
<FeatureComparisonTable
  ...
  features={[
    ...
    { name: "VPC resource access", values: { "Deployment script": "Direct (joinDefaultVpc)", "Lifecycle hook (local script)": "Via local-script-with-bastion-tunneling" } },
...
Lifecycle hooks run `local-script` on your machine or CI runner. If the local command needs VPC-only resources (databases, Redis), define a `local-script-with-bastion-tunneling` script that tunnels connections through a [bastion host](/resources/security/bastion-host). See [hooks and scripts](/configuration/hooks-and-scripts) for details.
...
The `packagePath` points to a pre-built directory or zip file. If it's a directory, Stacktape zips it automatically. The `handler` specifies the entry point in `filepath:functionName` format. For language-specific tuning (Node.js version, Python version, module format), see [language-specific packaging config](/packaging/function/language-specific-config).
...
export default async (event: { tableName: string; region: string }) => {
  console.info(`Processing table: ${event.tableName} in ${event.region}`);
};
```

## Manual execution

Use [`stacktape deployment-script:run`](/cli/deployment-script-run) to invoke any deployment script on demand without a full deploy:
...
Stack deletion continues. This is by design — a broken cleanup script should never prevent you from deleting a stack. If your cleanup script fails, inspect the script's output to diagnose the issue, but the stack resources will still be removed.

### Can I run a deployment script manually without deploying?

Yes. Use [`stacktape deployment-script:run`](/cli/deployment-script-run) to invoke any deployment script on demand. This packages and uploads the latest code, then invokes the Lambda. Use a full [`stacktape deploy`](/cli/deploy) when you need to apply configuration changes.
...
Use a deployment script when the migration needs direct VPC access to the database (the common case). A deployment script runs inside AWS as a Lambda function, so it can reach VPC-protected databases directly. Use a lifecycle hook with a `local-script` when the migration tool doesn't package well as a Lambda or takes longer than 15 minutes. If the local script needs VPC-only resources, define a `local-script-with-bastion-tunneling` script and attach it to the hook.

### When should I use a batch job instead of a deployment script?

Use a [batch job](/resources/compute/batch-job) when the task exceeds 15 minutes, needs more than 10,240 MB of memory, requires GPU access, or processes large datasets. Batch jobs run as containers and support longer runtimes and heavier compute. Deployment scripts are better for quick tasks (migrations, seeding) that fit within Lambda's constraints.

### How much does running a deployment script cost?

Deployment scripts use AWS Lambda pricing: you pay per request and for compute time measured in GB-seconds. A typical database migration running for 10 seconds with 512 MB memory costs a fraction of a cent. See [AWS Lambda pricing](https://aws.amazon.com/lambda/pricing/) for current rates.
