...
If your code needs DynamoDB actions beyond CRUD, scan, and query (for example, table management operations), add explicit [`iamRoleStatements`](/configuration/connecting-resources) to grant the additional permissions.
...
AWS DynamoDB supports ACID transactions across multiple items within the same table or across tables, using `TransactWriteItems` and `TransactGetItems` operations. Transactions consume twice the normal read/write capacity. The `connectTo` integration grants CRUD plus scan/query permissions; if your code uses transaction APIs or other DynamoDB actions beyond that set, add explicit `iamRoleStatements` to your workload.
