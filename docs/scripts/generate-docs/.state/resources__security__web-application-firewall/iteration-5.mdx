...
- **[HTTP API Gateways](/resources/networking/http-api-gateway) with CDN enabled** — add HTTP request inspection at the CloudFront edge with a `cdn` firewall. HTTP API Gateways don't support direct regional WAF attachment; enable CDN on the gateway and attach the firewall there.
...
The `scope` property determines which AWS service layer the firewall protects. Set it to `'regional'` for resources like [Application Load Balancers](/resources/networking/application-load-balancer) and [user auth pools](/resources/security/user-auth-pool). Set it to `'cdn'` for CloudFront-attached resources, including [HTTP API Gateways](/resources/networking/http-api-gateway) with CDN enabled. The remaining properties — `defaultAction`, `captchaImmunityTime`, `challengeImmunityTime`, `disableMetrics`, and `sampledRequestsEnabled` — have documented defaults; `customResponseBodies` and `tokenDomains` are opt-in.

A firewall on its own does nothing — you activate it by referencing it from a target resource's firewall configuration property. See [Attaching to resources](#attaching-to-resources) below.

## Scope

The `scope` property determines which AWS service layer the firewall operates on. Choose the scope that matches your attachment target: `regional` for ALBs or user auth pools; `cdn` for CloudFront-attached resources (including HTTP API Gateways with CDN enabled).

| Scope | What it protects | Typical targets |
|---|---|---|
| `regional` | Regional AWS resources in a single region | [Application Load Balancers](/resources/networking/application-load-balancer), [user auth pools](/resources/security/user-auth-pool) |
| `cdn` | CloudFront distributions at edge locations worldwide | [Static hosting](/resources/frontend/static-hosting), SSR frontends ([Next.js](/resources/frontend/nextjs), [Astro](/resources/frontend/astro), [Nuxt](/resources/frontend/nuxt), [SvelteKit](/resources/frontend/sveltekit), [SolidStart](/resources/frontend/solidstart), [TanStack Start](/resources/frontend/tanstack-start), [Remix](/resources/frontend/remix)), and resources with CDN enabled (including [HTTP API Gateways](/resources/networking/http-api-gateway)) |
...
A `regional` firewall inspects requests after they arrive at a regional AWS resource — an ALB or user auth pool. A `cdn` firewall inspects requests at CloudFront edge locations worldwide, blocking malicious traffic closer to the client. A `cdn` firewall can block requests before they reach the origin, which can reduce origin load for global traffic.

If a resource supports both a regional attachment point and a CDN attachment point (for example, an ALB with CDN enabled in front of it), you can attach two separate firewalls — one `regional` on the resource itself and one `cdn` on its CloudFront distribution — for layered protection.
...
A Stacktape web application firewall is a standalone resource. To activate it, reference the firewall's resource name from the target resource's firewall configuration. The firewall's `scope` must match the target: `regional` for ALBs and user auth pools; `cdn` for CloudFront-attached resources (including HTTP API Gateways with CDN enabled).

### Regional attachment

The WAF source defines `regional` scope for Application Load Balancers and user auth pools. Reference the firewall by its resource name from the target resource's configuration. See each resource's documentation for the specific property name.
...
<Warning>Some resources support both a regional and a CDN attachment point. For example, an ALB fronted by CloudFront can have two independent firewalls — one `regional` on the ALB and one `cdn` on the CloudFront distribution. These use separate firewall resources with different scopes.</Warning>
...
A `regional` firewall inspects requests at the AWS region level — after they arrive at an ALB or user auth pool. A `cdn` firewall inspects requests at CloudFront edge locations worldwide — before they reach your origin. A `cdn` firewall can block malicious traffic closer to the attacker, reducing origin load for legitimate users. Use `regional` for ALBs and user auth pools; use `cdn` for CloudFront-attached resources like static hosting, SSR frontends, and HTTP API Gateways with CDN enabled.
...
Yes, through CloudFront. HTTP API Gateways don't support direct regional WAF attachment, so enable CDN on the [HTTP API Gateway](/resources/networking/http-api-gateway) and attach a `cdn`-scoped firewall at the CloudFront layer for edge-level filtering. Use a WAF when API Gateway's built-in throttling isn't sufficient — for example, when you need HTTP request inspection for SQL injection, XSS, or bot detection.
