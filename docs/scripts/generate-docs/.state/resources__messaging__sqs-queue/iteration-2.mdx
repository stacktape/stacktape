...
seoDescription: 'Deploy serverless SQS message queues on AWS with Stacktape. Standard and FIFO queues, dead-letter queues, Lambda triggers, EventBridge integration, and pay-per-message pricing.'
---
...
Use `connectTo` on any compute resource ([Lambda function](/resources/compute/lambda-function), [web service](/resources/compute/web-service), [worker service](/resources/compute/worker-service), etc.) to grant it access to the queue. Stacktape grants the necessary IAM permissions to send messages to, receive messages from, and delete messages in the queue — along with the ancillary actions needed to use the queue URL. Three environment variables are injected automatically.
...
An SQS queue can receive messages from an [EventBridge event bus](/resources/messaging/event-bus) using the queue's `events` property. This routes matching events directly into the queue without writing any producer code. Choose exactly one EventBridge source: `eventBusName` for a Stacktape-managed bus, `eventBusArn` for an existing external bus, or `useDefaultBus` for the default AWS event bus.
...
When routing EventBridge events into a FIFO queue, set `messageGroupId` on the integration to control message ordering. Messages in the same group are processed in strict order; different groups can be processed in parallel.
