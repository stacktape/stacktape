...
<Tip>For AWS SQS compatibility, use the same queue type for the DLQ as the source queue — if the main queue is FIFO, the DLQ should also be FIFO.</Tip>
...
<Warning>Use one Lambda event-source consumer for a queue when each message should be handled once. For fan-out, publish to an [SNS topic](/resources/messaging/sns-topic) or [event bus](/resources/messaging/event-bus) and give each downstream consumer its own target.</Warning>
