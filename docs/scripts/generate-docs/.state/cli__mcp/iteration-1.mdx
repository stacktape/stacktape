...
The server builds a lexical (BM25) search index from pre-generated documentation at startup. Queries are matched against this index and the top results are returned as structured text. This ensures agents get accurate, up-to-date answers grounded in the current Stacktape docs rather than relying on training data.
