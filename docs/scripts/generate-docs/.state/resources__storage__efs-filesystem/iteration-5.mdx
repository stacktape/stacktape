...
Container workload types such as [web service](/resources/compute/web-service), [private service](/resources/compute/private-service), [worker service](/resources/compute/worker-service), and [multi-container workload](/resources/compute/multi-container-workload) expose the `volumeMounts` property. Consult each workload's API reference for the exact mount entry shape and supported options.

Multiple containers can mount the same EFS volume and share files through it — EFS handles concurrent reads and writes using standard POSIX file-locking semantics, making it practical for shared upload directories, CMS media, and similar workloads.

<Info>
For Lambda functions, check the [Lambda function API reference](/resources/compute/lambda-function) for any EFS mount support and required VPC settings.
</Info>
...
AWS Lambda supports mounting EFS filesystems, which is useful for functions that need access to large datasets or shared files beyond Lambda's `/tmp` storage. For the exact Stacktape configuration shape and any required VPC settings, see the [Lambda function API reference](/resources/compute/lambda-function).
