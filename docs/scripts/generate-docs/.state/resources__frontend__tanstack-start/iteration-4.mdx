...
Stacktape runs `vinxi build` by default, deploys the server output as a Lambda function, uploads static assets to S3, and routes traffic through CloudFront. The resulting CloudFront URL is available as a [referenceable parameter](#referenceable-parameters) (`url`).
...
The default `serverLambda.memory` is `1024` MB, configurable from `128` to `10240` MB. AWS Lambda allocates CPU proportionally to memory — `1769` MB corresponds to 1 vCPU. The default and maximum `serverLambda.timeout` is `30` seconds. This constraint aligns with CloudFront's default origin response timeout, so responses taking longer would time out at the CDN layer regardless.
...
Increase `memory` when SSR is CPU-bound or the app initializes large dependencies — for example, apps that render charts or process large data sets during SSR benefit from `2048` MB or higher. Increasing memory also reduces cold start duration because AWS Lambda allocates CPU proportionally. Keep the default `1024` MB when pages render quickly and memory pressure is low.

The `serverLambda.logging` sub-property controls CloudWatch log behavior. Log retention defaults to `180` days. Set `retentionDays` to a lower value like `30` to reduce storage costs in development stages, or increase it to `365` for production audit requirements. Set `logging.disabled` to `true` to turn off CloudWatch logging entirely — only do this if you forward logs elsewhere.
...
Stacktape uploads static assets to S3 and serves the entire app through CloudFront. The `fileOptions` property sets custom HTTP headers (such as `Cache-Control`) for uploaded files matching a glob pattern. The `cdn` property configures cache behavior for SSR routes and specific path patterns.
...
The `cdn.defaultCachingOptions.defaultTTL` controls how long CloudFront caches SSR responses when the origin sends no `Cache-Control` header. The example sets it to `60` seconds. Use `cdn.pathCachingOverrides` to set different cache behavior for specific CDN path patterns — for example, caching API-like routes differently from page routes.

By default, all cached CDN content is invalidated on every deploy so users see the latest version immediately. Set `cdn.disableInvalidationAfterDeploy` to `true` if you manage cache invalidation yourself or want to keep cached content between deploys.

Leave both `fileOptions` and `cdn` unset initially — TanStack Start and Stacktape already route static assets and SSR responses correctly. Add custom headers when you need explicit caching for fingerprinted assets, and tune CDN settings when SSR content changes infrequently enough to benefit from edge caching.
