...
To turn an editor template into a config file in your project, run [`stacktape init`](/cli/init) from your project directory. The command writes the contents of the editor into a `stacktape.ts` or `stacktape.yaml` configuration file. From there, use [`stacktape deploy`](/cli/deploy) to deploy your stack, or set up [GitOps](/ci-cd-and-gitops/gitops-with-console) for automatic deployments on push.
...
Yes. The config editor is a standalone tool for writing and managing Stacktape configuration templates in the Console. You can create, edit, and save templates without triggering any deployment. When you're ready, use [`stacktape init`](/cli/init) to write the template content into a project, then run [`stacktape deploy`](/cli/deploy), or set up [GitOps](/ci-cd-and-gitops/gitops-with-console) to deploy automatically on push.
...
When you run [`stacktape init`](/cli/init) to turn an editor template into a project file, the file is written as either `stacktape.ts` or `stacktape.yaml`. TypeScript configs offer richer type checking and the ability to use variables and logic; YAML configs are simpler for straightforward setups. See [configuration files](/configuration/configuration-files) for details on both formats.
...
[`stacktape init`](/cli/init) writes a configuration file into a local project directory using the contents of the config editor. The config editor in the Console is where you draft and manage that configuration before initializing it locally. Use the config editor when you want to explore resource types, try different configurations, or generate a config from an existing repository, and use `stacktape init` when you're ready to write the result into your project.