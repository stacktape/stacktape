...
order: 54
seoTitle: 'Application Load Balancer - HTTP/HTTPS Routing | Stacktape Docs'
seoDescription: 'Create an HTTP/HTTPS load balancer with Stacktape using AWS ALB. Flat ~$18/month pricing, advanced routing by path/host/headers, WebSocket support, WAF, and gradual deployments.'
---

# Application Load Balancer

A Stacktape Application Load Balancer routes HTTP/HTTPS requests to container workloads and [Lambda functions](/resources/compute/lambda-function) based on path, host, headers, query parameters, and source IPs. It uses AWS ALB underneath — flat ~$18/month pricing (AWS Free Tier eligible), automatic TLS, and support for WebSockets, [firewalls](/resources/security/web-application-firewall), and [gradual deployments](/deployment-and-lifecycle/gradual-deployments).
...
- **WebSocket applications** — ALB supports persistent connections natively, which HTTP API Gateway does not
...
<FeatureComparisonTable
  columns={["Application Load Balancer", "HTTP API Gateway"]}
  features={[
    { name: "Pricing model", values: { "Application Load Balancer": "Flat ~$18/month + LCU-based", "HTTP API Gateway": "~$1/million requests" } },
    { name: "Best for", values: { "Application Load Balancer": "Container workloads", "HTTP API Gateway": "Lambda functions" } },
    { name: "Scales to zero cost", values: { "Application Load Balancer": false, "HTTP API Gateway": true } },
    { name: "Routing options", values: { "Application Load Balancer": "Path, method, host, headers, query params, source IPs", "HTTP API Gateway": "Method + path" } },
    { name: "WebSocket support", values: { "Application Load Balancer": true, "HTTP API Gateway": false } },
    { name: "Direct WAF attachment", values: { "Application Load Balancer": true, "HTTP API Gateway": false } },
    { name: "Gradual deployments", values: { "Application Load Balancer": true, "HTTP API Gateway": false } },
    { name: "CDN attachable", values: { "Application Load Balancer": true, "HTTP API Gateway": true } }
  ]}
/>
...
The ALB `myAlb` has no explicit configuration because the defaults are sufficient — HTTPS on port 443 with automatic TLS, plus an HTTP listener on port 80 that redirects to HTTPS. The `api` Lambda function registers a catch-all route on the ALB through its `ApplicationLoadBalancerIntegration` event, with `priority: 1` controlling rule evaluation order (lower numbers are evaluated first).
...
The `priority` values determine evaluation order. Requests to `/users/123` match `usersApi` (priority 1), and `/orders/456` match `ordersApi` (priority 2). Requests that don't match any rule get the listener's default action.

## Listeners

By default, an Application Load Balancer creates two listeners: HTTPS on port 443 (with an auto-provisioned TLS certificate) and HTTP on port 80 (redirecting to HTTPS). Custom listeners let you change ports, add IP restrictions, or define redirect behavior for unmatched requests.

**When to configure custom listeners:** You need a non-standard port, want to restrict access by IP address, or need a custom redirect for unmatched requests. Most applications work fine with the defaults.
...
The `defaultAction` handles requests that don't match any integration rule. Currently only `redirect` is supported. The `statusCode` is either `HTTP_301` (permanent) or `HTTP_302` (temporary). Redirect targets can reuse parts of the original URL with placeholders: `#{protocol}`, `#{host}`, `#{port}`, `#{path}`, and `#{query}`.

## Custom domains

Stacktape can attach your own domain (e.g., `api.example.com`) to an Application Load Balancer. Stacktape automatically provisions a free TLS certificate and creates a DNS record in Route53. Your domain must have a [Route53 hosted zone](https://docs.aws.amazon.com/Route53/latest/DeveloperGuide/hosted-zones-working-with.html) in your AWS account — use [`stacktape domain:add`](/cli/domain-add) to set one up.
...
You can attach multiple domains to a single ALB. Each domain gets its own TLS certificate, managed and auto-renewed by AWS. If you manage DNS records yourself, set `disableDnsRecordCreation: true` and Stacktape provisions the TLS certificate but skips the DNS record. If you have specific certificate requirements, provide `customCertificateArn` with an ACM certificate ARN.
...
See [CDN](/resources/networking/cdn) for the full CDN configuration surface, including custom domains on the CDN itself, edge functions, route rewrites, and caching options.
...
Stacktape supports alarm types for Application Load Balancer: **error rate** (percentage of 4xx/5xx responses) and **unhealthy targets** (percentage of targets failing health checks). Alarm notification targets are configured per-alarm through the shared notification integrations — supported targets include Slack, Microsoft Teams, email, Discord, and webhook. Alarms defined on the resource are merged with any global alarms configured in the Stacktape Console.
...
Both alarm types support a custom `comparisonOperator`: `GreaterThanThreshold` (default), `GreaterThanOrEqualToThreshold`, `LessThanThreshold`, and `LessThanOrEqualToThreshold`.
...
<ApiReference definitionName="ApplicationLoadBalancerProps" />
...
Yes. Add a `customDomains` array to your ALB configuration with your domain name. Stacktape provisions a free TLS certificate and creates a DNS record in Route53 automatically. Your domain must have a Route53 hosted zone — set one up with [`stacktape domain:add`](/cli/domain-add). See [Custom domains](/resources/networking/custom-domains) for details.
...
Yes. AWS ALB natively supports WebSocket connections over HTTP/HTTPS listeners. Once the initial HTTP upgrade handshake completes, the ALB maintains the persistent bidirectional connection between client and backend. This makes ALB the right networking choice for real-time applications using Socket.io, ws, or similar WebSocket libraries. [HTTP API Gateway](/resources/networking/http-api-gateway) does not support WebSockets.
...
AWS ALB continuously checks the health of registered targets (containers or Lambda functions) by sending periodic HTTP requests to a configured health check path. Unhealthy targets are automatically removed from the routing pool and re-added when they recover. Health check parameters (path, interval, timeout) are configured on the target-side integration, not on the ALB resource itself. Container workloads configure health checks through their [web service](/resources/compute/web-service) or [multi-container workload](/resources/compute/multi-container-workload) settings.
