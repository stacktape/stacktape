...
The CLI matches the resource by its deployed resource type, so `--resourceName` must reference a resource whose type is one of the four listed above. For resource types not listed here — DynamoDB, S3, CloudFront, Cognito, and Redis — use the Console dashboard, which supports all nine chart categories.
...
[Multi-container workloads](/resources/compute/multi-container-workload) run on AWS ECS and expose `CPUUtilization` and `MemoryUtilization` through the CLI. Both are percentages of the allocated resources. Sustained CPU above 80% suggests the workload needs more CPU or additional containers. Memory hitting 100% causes OOM kills and container restarts — check [logs](/observability/logs) for restart events if you suspect this. The Console dashboard renders ECS charts for any deployed ECS service in the stage.
