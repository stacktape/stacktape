...
The `stacktape upgrade` command only moves you forward: it resolves the latest published version and proceeds only when that version is greater than the installed one. The command metadata includes a version-related option, but the shown upgrade implementation always resolves the latest published version.
