...
Cloud Native Buildpacks may use layer caching depending on the builder and build environment — dependencies are typically cached in a separate image layer from application code, so unchanged dependencies can be reused on subsequent builds. The Stacktape external-buildpack configuration surface (`builder`, `buildpacks`, `sourceDirectoryPath`, `command`) does not expose explicit cache controls; caching behavior is determined by the builder.
...
### Does the external buildpack cache builds between deploys?

Cloud Native Buildpacks are designed around layer caching — dependencies are stored in a separate image layer from application code, so unchanged dependencies can be reused across builds. The Stacktape `ExternalBuildpackPackaging` config surface does not expose explicit cache controls; whether a given build reuses prior layers depends on the builder and the build environment.
