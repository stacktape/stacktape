...
The `useFirewall` property names a [web application firewall](/resources/security/web-application-firewall) resource that protects this CDN from common web exploits. Configure inspection and blocking behavior on the [firewall resource](/resources/security/web-application-firewall) itself.
