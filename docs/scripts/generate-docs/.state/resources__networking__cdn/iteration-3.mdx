...
When a route rewrite targets specific resource types, additional properties become available. Bucket route targets support `disableUrlNormalization` (disables clean URL normalization like `/about` to `/about.html`, defaults to `false`). ALB route targets support `listenerPort` (for custom listener setups) and `originDomainName` (when the ALB has no custom domains and uses `customCertificateArns`).
...
The top-level `defaultRoutePrefix` applies the same transformation to all requests that don't match any rewrite. This is useful for versioned APIs or when your origin serves content from a sub-path. For example, setting `defaultRoutePrefix: '/v2'` on the CDN forwards all unmatched requests with `/v2` prepended.
...
| `onResponse` | Before returning response to client (does not run if origin returned 400+ error or `onRequest` already generated a response) | Security headers, cookies, response header modification |
...
The `useFirewall` property names a `web-app-firewall` resource that protects the CDN from common web exploits at the CloudFront edge layer. Configure protection rules on the [web application firewall](/resources/security/web-application-firewall) resource itself — see that page for available managed rule sets and custom rule configuration. Requests blocked by the firewall never reach your origin, saving compute costs and reducing attack surface.
...
When CDN routes target specific resource types in route rewrites, additional properties become available beyond the base configuration. Bucket route targets (`CdnBucketOrigin`) support `disableUrlNormalization`. ALB route targets (`CdnLoadBalancerOrigin`) support `listenerPort` and `originDomainName`. These properties are reachable through the API Reference drill-down below.
