...
<Warning>When using `sqsQueueArn`, make sure the external queue allows the Lambda function execution role to receive and delete messages. Stacktape cannot infer all access rules for queues managed outside the stack.</Warning>
...
Yes. Use the `sqsQueueArn` property to reference an existing SQS queue by its ARN. Any cross-account setup must additionally satisfy AWS Lambda and SQS event source requirements, including the queue's resource policy and the Lambda execution role's permissions. See [using an external queue](#using-an-external-queue) above.
