...
- Buttons, links, form controls, tabs, badges, dialogs, tooltips, alerts, progress/loaders, grid lists and product
  resource/framework icons come from `@stacktape/ui-react`. Console's adapters keep only what is Console's: React
  Router navigation, React Hook Form translation, hint persistence, brief post-click acknowledgement, or private
  orchestration. Emotion `Css` types never cross into the shared package; Emotion's `css` prop may style a shared
  component through its ordinary `className` contract.
- `@stacktape/ui-react/config-editor` is the reusable editor frame and document state. Console's
  `ConsoleConfigEditor` is its private controller: tRPC compilation, pricing, saving, presets, and account/project
  context remain there. Do not create a second visual editor frame in Console.
...
- Keep `ui/src/components/IsometricDiagramView.tsx` inside the `lazy()` boundary and keep the diagram's stylesheet imported there. The isopack
...
- Console design experiments are development-only and live under `ui/src/dev/design-preview`; compare them at
  `/ui-lab`. The Ø option applies no properties, so it is always exactly what production ships. Correct a defect in
  the production code; use a preview only for a choice that is genuinely a matter of taste.
- A preview may change a coherent visual treatment — palette, surfaces, elevation, radii, chrome and interaction
  states — but must not override layout geometry such as widths, heights, spacing, positioning, overflow, or icon
  sizes. Stress-test dense content and wrapping. Every production hook uses `var(--hook, <original value>)`, so
  deleting the preview code cannot change the released UI.
