...
- Deterministic generators required by ordinary build/typecheck/test tasks run through Turbo, and committed outputs
  ...
  The larger LLM corpus and its enhanced documentation schema use a separate cached task keyed by canonical
  `apps/docs` content, the config package, retained CLI declarations and generator code. It stages the complete corpus
  before replacement, contains no clock-derived metadata, and is covered by the same scoped generated diff.
