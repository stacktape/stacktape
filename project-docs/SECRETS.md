...
once per warm runtime through `server/runtime-parameters.ts`. Database connection strings are assembled inside the
...
secret and its replacement parameter is recorded in `apps/console/api/secret-retirement.ts`; delete those legacy secrets
only after the production deployment and smoke tests pass.
