 ...
 *
 * The first plan downloads nixpacks from its release if this machine does not have it yet
 * (`@utils/external-tools`). Failure here is always an absent answer, never a failed scan: a download
 * that cannot happen (offline), a directory the planner cannot make sense of, or output that does not
 * parse all return `null`, and the pipeline behaves exactly as it did before this oracle existed.
 */
