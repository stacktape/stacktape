...
export const preparePipelineResources = async ({
  ...
  // if not create one
  let project = await awsSdkManager.codeBuild.getProject({
    projectName: awsResourceNames.codebuildProject(region)
  });
  if (!project) {
    project = await awsSdkManager.codeBuild.createProject({
      projectName: awsResourceNames.codebuildProject(region),
...
export const startCodebuildDeployment = async ({
  ...
  let build: Build;
  try {
    build = await awsSdkManager.codeBuild.startDeployment({
    ...
    await awsSdkManager.codeBuild.waitForBuildPhase({
      buildId: build.id,
      awsAccountId
    });
  } finally {
  ...
  return awsSdkManager.codeBuild.getBuild({ buildId: build.id });
};
