...
Everything the Stacktape CLI writes to a terminal goes through this module. The `tuiManager` facade
(`index.ts`) is the application-facing surface for terminal effects and composes the layers below without
owning view rendering. Direct imports are intentional only for pure formatting helpers, output/JSONL types,
the dev manager, and the launcher; application code must not reach into progress state or renderer internals.
...
- `runtime/` — OpenTUI ownership. `opentui.ts` creates the renderer and sets `OTUI_USE_CONSOLE=false`.
  The production CLI entrypoint sets the same flag before loading the application or launcher; progress and
  dev mounts load `opentui.ts` before loading their view modules. Preserve this ordering when adding an
  entrypoint or mount path—OpenTUI must never take ownership of `global.console`. `lifecycle.ts` is the
