...
import { executeAwsSdkCommand } from './aws-sdk-executor';
...
describe('AWS SDK executor', () => {
  ...
  test('never sends an operation outside the reviewed list, whoever calls it and however it is spelled', async () => {
    ...
    for (const [service, command, input] of [
      ...
      const result = await executeAwsSdkCommand(service, command, input, context);
