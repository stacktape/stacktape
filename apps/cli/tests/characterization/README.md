...
| Packaging              | A real custom artifact is zipped; source files are reported; content changes invalidate the digest; excluded directories do not; cache hits skip work. `packages/packaging` additionally pins Lambda-layer assignment (usage/size thresholds, dependency promotion, over-capacity fallback, bin packing), the layer artifact layout and content hash, and chunk-import rewriting           | Permanent `packages/packaging` suite                                                          | The split bundler itself, language buildpacks, Docker/image builds, monorepo dependency tracing, native-zip parity across operating systems |
