...
| Stack lifecycle        | Operation-level tests preserve consequential cleanup and mutation ordering for full deploy, delete, Stacktape version rollback, and CloudFormation rollback. They distinguish ordinary deploy failure from uncertain monitoring failure, enforce termination protection before deletion, fail rollback before mutation when artifacts are absent, and keep bucket restoration best-effort.                                         | Permanent workflow safety gate                                                                | Real AWS deploy/no-op/update/failure/rollback/delete behavior remains in the guarded slow/costed tier                                       |
| Packaging              | A real custom artifact is zipped; source files are reported; content changes invalidate the digest; excluded directories do not; cache hits skip work. `packages/packaging` additionally pins Lambda-layer assignment (usage/size thresholds, dependency promotion, over-capacity fallback, bin packing, and the dependency closure that survives packing), the layer artifact layout and content hash, and chunk-import rewriting | Permanent `packages/packaging` suite                                                          | The split bundler itself, language buildpacks, Docker/image builds, monorepo dependency tracing, native-zip parity across operating systems |
