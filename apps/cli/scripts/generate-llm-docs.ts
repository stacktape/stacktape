...
import { copyFile, mkdtemp, readdir as readDirectory, rename, rm } from 'node:fs/promises';
...
const relativeSourcePath = (filePath: string): string => normalizePath(relative(WORKSPACE_ROOT, filePath));

type EnhancedConfigSchema = {
  definitions: Record<string, unknown>;
};

type RenameDirectory = typeof rename;

let apiReferenceData: ReturnType<typeof buildApiReferenceData>;
let resources: any[];
...
// `shared.ts` is the renamed `__helpers.d.ts` logical documentation source. Keep that human-facing category rather
// than deriving routes from the package's physical filename.
const OUTPUT_GROUP_BY_SOURCE_STEM: Record<string, string> = {
  shared: 'helpers'
};

// This single statement historically belonged to the shared declaration source. Its corpus path is kept stable even
// though the v4 authored type now lives beside bucket configuration.
const DEFINITION_OUTPUT_GROUP_OVERRIDES: Record<string, string> = {
  BucketPolicyIamRoleStatement: 'shared'
};
...
const listFiles = async (dir: string, predicate: (path: string) => boolean): Promise<string[]> => {
  ...
  return files.flat().sort(compareLlmDocPaths);
};
...
const markdownToTableText = (value = ''): string => {
  const lines = htmlToMarkdownText(value)
    .split(/\r?\n/)
    .map((line) => line.trim())
    .filter(Boolean);
  const heading = lines[0]?.match(/^#{1,6}\s+(.+)$/)?.[1];
  if (heading) lines.shift();
  if (/^(?:-{3,}|\*{3,}|_{3,})$/.test(lines[0] ?? '')) lines.shift();
...
const buildDefinitionSourceLocations = (): Map<string, DefinitionSourceLocation> => {
  ...
  // The config package is the canonical owner and intentionally precedes ambient CLI compatibility declarations.
  // Keep each root deterministic without globally re-sorting the roots back into filesystem-name order.
  const files = TYPE_SOURCE_DIRS.flatMap((directory) =>
    existsSync(directory) ? ts.sys.readDirectory(directory, ['.ts', '.d.ts']).sort(compareLlmDocPaths) : []
  );

  for (const filePath of files) {
    const stem = basename(filePath).replace(/(?:\.d)?\.ts$/, '');
    const resourceType = FILENAME_TO_RESOURCE_TYPE[stem];
    const sourceOutputGroup = OUTPUT_GROUP_BY_SOURCE_STEM[stem] ?? resourceType ?? stem.replace(/^_+/, '') ?? 'shared';
    const sourceFile = ts.createSourceFile(filePath, readFileSync(filePath, 'utf-8'), ts.ScriptTarget.Latest, false);
    const visit = (node: ts.Node) => {
      ...
      if (!locations.has(definitionName)) {
        const outputGroup = DEFINITION_OUTPUT_GROUP_OVERRIDES[definitionName] ?? sourceOutputGroup;
        locations.set(definitionName, {
          sourcePath: relativeSourcePath(filePath),
          route: `/config-reference/${outputGroup}`,
          outputGroup,
          resourceType: DEFINITION_OUTPUT_GROUP_OVERRIDES[definitionName] ? undefined : resourceType
        });
      }
    };
    visit(sourceFile);
...
const apiReferenceSummaryToMarkdown = (definitionName: string): string => {
  ...
  if (definition.shortDescription) lines.push(htmlToMarkdownText(definition.shortDescription), '');
  lines.push(
...
const renderUnionDetails = (property: ApiReferenceGeneratedDefinition['properties'][number]): string[] => {
  ...
  for (const branch of typeInfo.branches) {
    ...
    const description = htmlToMarkdownText(branch.shortDescription);
...
const apiReferenceDefinitionToMarkdown = (definitionName: string): string => {
  ...
  for (const property of definition.properties) {
    ...
    const description = [htmlToMarkdownText(property.shortDescription), htmlToMarkdownText(property.longDescription)]
      .filter(Boolean)
      .join('\n\n');
...
const referenceableParamsToMarkdown = (resourceType: string): string => {
  ...
  for (const [paramName, description] of Object.entries(referenceableParams)) {
    ...
    lines.push(
      `| \`${escapeMarkdownTableCell(paramName)}\` | ${escapeMarkdownTableCell(htmlToMarkdownText(normalizedDescription)) || '-'} | \`$ResourceParam("<<resource-name>>", "${escapeMarkdownTableCell(paramName)}")\` |`
    );
  }
  return `${lines.join('\n')}\n\n`;
};
...
const referenceableParamsFromTypeDeclaration = (resourceType: string): Record<string, string> | undefined => {
  ...
  const files = TYPE_SOURCE_DIRS.flatMap((directory) =>
    existsSync(directory) ? ts.sys.readDirectory(directory, ['.ts', '.d.ts']) : []
...
const cliCommandReferenceToMarkdown = (command: string, args: any[]): string => {
  ...
  for (const arg of args) {
    ...
    const description = [
      markdownToTableText(arg.shortDescription || arg.description),
      markdownToTableText(arg.longDescription)
    ]
      .filter(Boolean)
      .join(' ');
    lines.push(
...
const buildConfigReferencePages = async (): Promise<LlmDocPage[]> => {
  const pages: LlmDocPage[] = [];
  for (const definitionName of Object.keys(apiReferenceData).sort(compareLlmDocPaths)) {
    const definition = apiReferenceData[definitionName];
    const location = definitionSourceLocations.get(definitionName) || {
...
const writePagesAndChunks = async (pages: LlmDocPage[], outputDirectory: string) => {
  ...
  for (const page of pages) {
    const outputPath = join(outputDirectory, page.outputPath);
    await ensureDir(dirname(outputPath));
    await writeFile(outputPath, page.content.endsWith('\n') ? page.content : `${page.content}\n`, 'utf-8');
  }

  await ensureDir(join(outputDirectory, 'chunks'));
  const chunksJsonl = chunks.map((chunk) => JSON.stringify(chunk)).join('\n');
  await writeFile(join(outputDirectory, 'chunks', 'chunks.jsonl'), chunksJsonl, 'utf-8');
  await writeFile(
    join(outputDirectory, LEXICAL_INDEX_FILE_NAME),
  ...
  await writeFile(join(outputDirectory, 'index.json'), JSON.stringify(manifest, null, 2), 'utf-8');
  return { chunks, manifest };
};

const writeLlmsFiles = async (pages: LlmDocPage[], outputDirectory: string) => {
  ...
  await writeFile(join(outputDirectory, 'llms-full.txt'), `${full}\n`, 'utf-8');
  const apiReference = [
  ...
  await writeFile(join(outputDirectory, 'llms-api-reference.txt'), `${apiReference.trim()}\n`, 'utf-8');
};

export const recoverInterruptedCorpus = async (
  distDirectory: string,
  renameDirectory: RenameDirectory = rename
): Promise<void> => {
  const backupDirectory = `${distDirectory}.previous`;
  if (!existsSync(distDirectory) && existsSync(backupDirectory)) {
    await renameDirectory(backupDirectory, distDirectory);
  }
};

const isBlockedDirectoryRename = (error: unknown): boolean =>
  error instanceof Error &&
  'code' in error &&
  (error.code === 'EPERM' || error.code === 'EACCES' || error.code === 'EBUSY');

const synchronizeGeneratedDirectory = async (sourceDirectory: string, destinationDirectory: string): Promise<void> => {
  await ensureDir(destinationDirectory);
  const [sourceEntries, destinationEntries] = await Promise.all([
    readDirectory(sourceDirectory, { withFileTypes: true }),
    readDirectory(destinationDirectory, { withFileTypes: true })
  ]);
  const destinationEntriesByName = new Map(destinationEntries.map((entry) => [entry.name, entry]));

  for (const sourceEntry of sourceEntries) {
    const sourcePath = join(sourceDirectory, sourceEntry.name);
    const destinationPath = join(destinationDirectory, sourceEntry.name);
    const destinationEntry = destinationEntriesByName.get(sourceEntry.name);

    if (sourceEntry.isDirectory()) {
      if (destinationEntry && !destinationEntry.isDirectory()) {
        await rm(destinationPath, { force: true, recursive: true });
      }
      await synchronizeGeneratedDirectory(sourcePath, destinationPath);
      ...
      throw new Error(`Unsupported generated corpus entry: ${sourcePath}`);
    }
  }
  ...
  await Promise.all(
    destinationEntries
      ...
      .map((entry) => rm(join(destinationDirectory, entry.name), { force: true, recursive: true }))
  );
};

export const installGeneratedCorpus = async ({
  distDirectory,
  stagingDirectory,
  ...
  const backupDirectory = `${distDirectory}.previous`;
  let movedCurrentCorpus = false;

  if (existsSync(distDirectory)) {
    await rm(backupDirectory, { recursive: true, force: true });
    try {
      await renameDirectory(distDirectory, backupDirectory);
    } catch (error) {
      if (!isBlockedDirectoryRename(error)) throw error;
      console.warn('[llm-docs] Directory swap is blocked; updating the generated corpus in place.');
      await synchronizeGeneratedDirectory(stagingDirectory, distDirectory);
      return;
    }
    movedCurrentCorpus = true;
  }

  try {
    await renameDirectory(stagingDirectory, distDirectory);
  } catch (error) {
    if (movedCurrentCorpus && !existsSync(distDirectory) && existsSync(backupDirectory)) {
      await renameDirectory(backupDirectory, distDirectory);
    }
    throw error;
  }

  await rm(backupDirectory, { recursive: true, force: true }).catch((error: unknown) => {
    console.warn(`[llm-docs] Generated corpus installed, but old corpus cleanup failed: ${String(error)}`);
  });
};
...
export const generateLlmDocs = async ({
  ...
  const pages = [...docsPages, ...configReferencePages].sort((left, right) =>
