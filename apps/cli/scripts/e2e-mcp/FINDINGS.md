...
If you want this hardened: cli_run for `destructive` safety could require a _second_ signal — e.g. a `userConfirmedAfterSeeingPlan: true` flag that's only valid in the same turn the user replied "yes" after seeing a plan. Adds API complexity but materially reduces "agent yolo'd a delete" risk.
