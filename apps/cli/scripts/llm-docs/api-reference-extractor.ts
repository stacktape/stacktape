...
export const normalizeDefinition = (definitionName: string, definitions: Definitions): NormalizedDefinition | null => {
  ...
  const sortGroup = (props: NormalizedProperty[]) =>
    [...props].sort((left, right) => (left.name < right.name ? -1 : left.name > right.name ? 1 : 0));
