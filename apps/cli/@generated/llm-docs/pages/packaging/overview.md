...
| Mode | Use when |
|------|----------|
| [**Stacktape buildpack**](/packaging/function/stacktape-buildpack) | **Recommended.** Point to your source file — Stacktape handles bundling, minification, source maps, and dependency installation. For JS/TS, code is bundled into a single minified file, code shared between functions moves into a shared layer, and dependencies with native binaries are installed separately. Supports JavaScript, TypeScript, Python, Java, Go, Ruby, PHP, and .NET. |
...
AWS Lambda has deployment package size limits that can constrain large applications. The Stacktape buildpack helps by bundling JavaScript and TypeScript into a single minified file, leaving the runtime-provided AWS SDK out of the package, and moving code shared between functions into [a shared layer](/packaging/function/stacktape-buildpack#shared-code-between-functions). If your package exceeds AWS limits, consider moving to a container-based resource like a [web service](/resources/compute/web-service) — container images can be significantly larger.
