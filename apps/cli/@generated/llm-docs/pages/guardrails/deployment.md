...
The type definitions include 18 guardrail types. This page focuses on stage, region, command, resource-type, and resource-count restrictions.
...
> **Info:** Other guardrail types include VPC database requirements, stack and database deletion protection, recoverable data stores, dead-letter queues, redundant container instances, Lambda and container limits, database restrictions, WAF requirements, and custom-domain requirements. See [Security and data protection](/guardrails/security-and-data-protection), [Resource limits](/guardrails/resource-limits), and [Database guardrails](/guardrails/databases). The [Guardrails overview](/guardrails/overview) covers the full set.
...
- **You want to protect production.** CloudFormation termination protection prevents accidental stack removal at the AWS layer. Command restrictions can add an earlier Stacktape policy check.
...
Command restriction defines infrastructure-changing commands that are blocked. Supported values are `deploy`, `delete`, `dev`, `deployment-script:run`, `script:run`, `rollback`, and `cf:rollback`. Read-only `diff` and `validate` intentionally remain available so a developer can inspect and diagnose a policy violation.
...
> **Info:** Command restriction is enforced by Stacktape only. For deletion protection that also covers AWS Console and direct AWS API calls, enable the `require-stack-termination-protection` guardrail.
...
Resource count limit caps the user-declared Stacktape resources allowed in a single configuration. Set a `maxResources` threshold — if that count exceeds it, the deployment is blocked. Generated nested and CloudFormation resources are not counted.

Large configurations are harder to review and carry a wider blast radius. `maxResources` is an architecture-governance threshold, not a CloudFormation quota check: one Stacktape resource can synthesize many CloudFormation resources.
...
| Command restriction | `blockedCommands: ["delete"]` | Add a Stacktape-only deletion policy check |
| [Stack termination protection](/guardrails/security-and-data-protection) | `enabled: true` | Prevent deletion at the CloudFormation layer |
...
It does not enforce the CloudFormation resource quota. `maxResources` counts user-declared Stacktape resources, while one Stacktape resource can map to many CloudFormation resources. Use it as an organization architecture limit; template synthesis and AWS validation remain responsible for CloudFormation limits.
