...
- **Connect alert channels** such as Slack, email, or a webhook so budget alerts reach the tools your team already monitors. In a channel of the connected Slack workspace, a budget card can be snoozed until the end of the month (`org:manage-budgets` permission); the budget's details in the Console show the snooze and can resume the alerts.
