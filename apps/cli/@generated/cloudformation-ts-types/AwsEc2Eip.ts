 ...
 * Addresses](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/elastic-ip-addresses-eip.html) in
 * the *Amazon EC2 User Guide*.
 */
export type AwsEc2Eip = {
  PublicIp?: string;
  AllocationId?: string;
  /**
   * The network (``vpc``).
  ...
  /**
   * The Elastic IP address you are accepting for transfer. You can only accept one transferred address.
   * For more information on Elastic IP address transfers, see [Transfer Elastic IP
   ...
   * Updates to the ``Tags`` property may require *some interruptions*. Updates on an EIP reassociates
   * the address on its associated resource.
   * @uniqueItems false
   */
  Tags?: {
    /** The tag key. */
