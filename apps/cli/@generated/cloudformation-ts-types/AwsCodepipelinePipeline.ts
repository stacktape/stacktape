...
// Source: aws-codepipeline-pipeline.json
 ...
 */
export type AwsCodepipelinePipeline = {
  /**
  ...
  ArtifactStores?: {
    ArtifactStore: {
      ...
      EncryptionKey?: {
        /**
         * The type of encryption key, such as an AWS KMS key. When creating or updating a pipeline, the value
         * must be set to 'KMS'.
         */
        Type: string;
        /**
       ...
       * bucket but not a folder in the bucket. A folder to contain the pipeline artifacts is created for
       * you based on the name of the pipeline. You can use any S3 bucket in the same AWS Region as the
  ...
  Stages: ({
    ...
    Actions: ({
      ActionTypeId: {
         ...
         * action category section within your pipeline structure: AWS, ThirdParty, and Custom.
         */
        Owner: string;
        /**
         ...
         * @enum ["Source","Build","Test","Deploy","Invoke","Approval","Compute"]
         */
      ...
      EnvironmentVariables?: ({
         ...
         * @enum ["PLAINTEXT","SECRETS_MANAGER"]
         */
       ...
       * specified in Quotas for AWS CodePipeline. This attribute is available only to the manual approval
       * ActionType.
       */
    ...
    OnFailure?: {
      ...
      RetryConfiguration?: {
        /**
         * The specified retry mode type for the given stage. FAILED_ACTIONS will retry only the failed
         * actions. ALL_ACTIONS will retry both failed and successful
      ...
      Conditions?: {
        ...
        Rules?: {
          ...
          Region?: string;
          /**
           * The ARN of the IAM service role that performs the declared rule. This is assumed through the
           * roleArn for the pipeline.
           * @pattern arn:aws(-[\w]+)*:iam::[0-9]{12}:role/.*
           */
          RoleArn?: string;
   ...
   * SUPERSEDED.
   * @default "SUPERSEDED"
   * @enum ["QUEUED","SUPERSEDED","PARALLEL"]
   */
