...
// Source: aws-apigatewayv2-authorizer.json

/**
 * The ``AWS::ApiGatewayV2::Authorizer`` resource creates an authorizer for a WebSocket API or an HTTP
 * API. To learn more, see [Controlling and managing access to a WebSocket API in API
...
export type AwsApigatewayv2Authorizer = {
   ...
   * are also used as the cache key when caching is enabled. To learn more, see [Working with Lambda
  ...
  JwtConfiguration?: {
    /**
     * The base domain of the identity provider that issues JSON Web Tokens. For example, an Amazon
     * Cognito user pool has the following format:
     * ``https://cognito-idp.{region}.amazonaws.com/{userPoolId}``. Required for the ``JWT`` authorizer
     * type. Supported only for HTTP APIs.
     */
    Issuer?: string;
    /**
     * A list of the intended recipients of the JWT. A valid JWT must provide an ``aud`` that matches at
     * least one entry in this list. See [RFC
     * 7519](https://docs.aws.amazon.com/https://tools.ietf.org/html/rfc7519#section-4.1.3). Required for
   ...
   * Lambda authorizers. Supported values are ``1.0`` and ``2.0``. To learn more, see [Working with
   ...
   * instead of an IAM policy. Supported only for HTTP APIs. To learn more, see [Working with Lambda
   * authorizers for HTTP
   * APIs](https://docs.aws.amazon.com/apigateway/latest/developerguide/http-api-lambda-authorizer.html).
   */
