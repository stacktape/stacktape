...
// Source: aws-autoscaling-scalingpolicy.json

/**
 * The AWS::AutoScaling::ScalingPolicy resource specifies an Amazon EC2 Auto Scaling scaling policy so
 * that the Auto Scaling group can scale the number of instances available for your application.
 */
export type AwsAutoscalingScalingpolicy = {
  /**
   * The aggregation type for the CloudWatch metrics. The valid values are Minimum, Maximum, and
   * Average. If the aggregation type is null, the value is treated as Average. Valid only if the policy
   * type is StepScaling.
   */
   ...
   * must specify a positive value. Required if the policy type is SimpleScaling. (Not used with any
   * other policy type.)
   */
  ...
  Cooldown?: string;
  /**
   * A set of adjustments that enable you to scale based on the size of the alarm breach. Required if
   * the policy type is StepScaling. (Not used with any other policy type.)
   * @uniqueItems true
   */
  StepAdjustments?: {
   ...
   * Auto Scaling group. Valid only if the policy type is TargetTrackingScaling or StepScaling.
   */
