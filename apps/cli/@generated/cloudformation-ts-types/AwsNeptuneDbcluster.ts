...
// Source: aws-neptune-dbcluster.json

/** The AWS::Neptune::DBCluster resource creates an Amazon Neptune DB cluster. */
export type AwsNeptuneDbcluster = {
  /**
  ...
  ReadEndpoint?: string;
  /**
   * The resource id for the DB cluster. For example: `cluster-ABCD1234EFGH5678IJKL90MNOP`. The cluster
   * ID uniquely identifies the cluster and is used in things like IAM authentication policies.
   */
  ...
  AssociatedRoles?: {
     ...
     * list of supported feature names, see DBEngineVersion in the Amazon Neptune API Reference.
     */
    FeatureName?: string;
  ...
  /**
   * The DB cluster identifier. Contains a user-supplied DB cluster identifier. This identifier is the
   * unique key that identifies a DB cluster stored as a lowercase string.
   * @minLength 1
   * @maxLength 63
  ...
  /**
   * The name of the DB parameter group to apply to all instances of the DB cluster. Used only in case
   * of a major EngineVersion upgrade request.
   */
  DBInstanceParameterGroupName?: string;
  /**
   * The port number on which the DB instances in the DB cluster accept connections.
   * If not specified, the default port used is `8182`.
   * Note: `Port` property will soon be deprecated from this resource. Please update existing templates
   * to rename it with new property `DBPort` having same functionalities.
   */
  DBPort?: number;
  /**
  ...
  ServerlessScalingConfiguration?: {
    /**
     * The minimum number of Neptune capacity units (NCUs) for a DB instance in an Neptune Serverless
     * cluster. You can specify NCU values in half-step increments, such as 8, 8.5, 9, and so on. The
     * smallest value you can use is 1, whereas the largest is 128.
     * @minimum 1
     * @maximum 128
     */
    MinCapacity: number;
    /**
     * The maximum number of Neptune capacity units (NCUs) for a DB instance in an Neptune Serverless
     * cluster. You can specify NCU values in half-step increments, such as 40, 40.5, 41, and so on. The
   ...
   * true.
   * If you enable the StorageEncrypted property but don't specify KmsKeyId property, then the default
   * KMS key is used. If you specify KmsKeyId property, then that KMS Key is used to encrypt the
   * database instances in the DB cluster.
   * If you specify the SourceDBClusterIdentifier property and don't specify this property or disable
   * it. The value is inherited from the source DB cluster, and if the DB cluster is encrypted, the
   * KmsKeyId property from the source cluster is used.
