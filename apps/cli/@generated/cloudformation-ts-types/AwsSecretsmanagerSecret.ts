...
// Source: aws-secretsmanager-secret.json
 ...
 * Secrets Manager.
 * For RDS master user credentials, see [AWS::RDS::DBCluster
 ...
 * For RS admin user credentials, see
 * [AWS::Redshift::Cluster](https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-resource-redshift-cluster.html).
 * To retrieve a secret in a CFNshort template, use a *dynamic reference*. For more information, see
 * [Retrieve a secret in an
...
export type AwsSecretsmanagerSecret = {
   ...
   * the secret. An alias is always prefixed by ``alias/``, for example ``alias/aws/secretsmanager``.
   ...
   * If the secret is in a different AWS account from the credentials calling the API, then you can't
   * use ``aws/secretsmanager`` to encrypt the secret, and you must create and use a customer managed
   * KMS key.
   */
  KmsKeyId?: string;
  /**
   * The text to encrypt and store in the secret. We recommend you use a JSON structure of key/value
   * pairs for your secret value. To generate a random password, use ``GenerateSecretString`` instead.
   * If you omit both ``GenerateSecretString`` and ``SecretString``, you create an empty secret. When
   * you make a change to this property, a new secret version is created.
   */
  SecretString?: string;
  /**
   * A structure that specifies how to generate a password to encrypt and store in the secret. To
   * include a specific string in the secret, use ``SecretString`` instead. If you omit both
  ...
  GenerateSecretString?: {
    ...
    /**
     * The JSON key name for the key/value pair, where the value is the generated password. This pair is
     * added to the JSON structure specified by the ``SecretStringTemplate`` parameter. If you specify
     * this parameter, then you must also specify ``SecretStringTemplate``.
     */
    ...
    /**
     * A template that the generated string must match. When you make a change to this property, a new
   ...
   * Secrets Manager tag key names are case sensitive. A tag with the key "ABC" is a different tag from
   * one with key "abc".
   ...
   * have restrictions on allowed characters. Generally allowed characters: letters, spaces, and numbers
