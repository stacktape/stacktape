...
// Source: aws-kms-key.json

/**
 * The ``AWS::KMS::Key`` resource specifies an [KMS
 * key](https://docs.aws.amazon.com/kms/latest/developerguide/concepts.html#kms_keys) in KMSlong. You
 * can use this resource to create symmetric encryption KMS keys, asymmetric KMS keys for encryption
 * or signing, and symmetric HMAC KMS keys. You can use ``AWS::KMS::Key`` to create [multi-Region
 * primary
 ...
 * values.
 * KMS replaced the term *customer master key (CMK)* with ** and *KMS key*. The concept has not
 * changed. To prevent breaking changes, KMS is keeping some variations of this term.
 ...
 * encryption KMS key to encrypt data stored in AWS services that are [integrated
 * with](https://docs.aws.amazon.com//kms/features/#AWS_Service_Integration). For more information,
 ...
 * ``KeyUsage`` value. For details, see [Asymmetric keys
 * in](https://docs.aws.amazon.com/kms/latest/developerguide/symmetric-asymmetric.html) in the
 * *Developer Guide*.
 ...
 * a ``KeyUsage`` value of ``GENERATE_VERIFY_MAC``. For details, see [HMAC keys
 * in](https://docs.aws.amazon.com/kms/latest/developerguide/hmac.html) in the *Developer Guide*.
 ...
 * about multi-Region keys, see [Multi-Region keys
 * in](https://docs.aws.amazon.com/kms/latest/developerguide/multi-region-keys-overview.html) in the
 * *Developer Guide*.
 ...
 * *Regions*
 * KMS CloudFormation resources are available in all Regions in which KMS and CFN are supported. You
 * can use the ``AWS::KMS::Key`` resource to create and manage all KMS key types that are supported in
 * a Region.
 */
export type AwsKmsKey = {
   ...
   * The actual key state of the KMS key might be affected by actions taken outside of CloudFormation,
   * such as running the
   ...
   * key](https://docs.aws.amazon.com/kms/latest/developerguide/key-state.html) in the *Developer
   * Guide*.
   */
  Enabled?: boolean;
  /**
   * Enables automatic rotation of the key material for the specified KMS key. By default, automatic key
   ...
   * ``EnableKeyRotation`` to ``true`` on the primary key (created by using ``AWS::KMS::Key``). KMS
   ...
   * When you enable automatic rotation, KMS automatically creates new key material for the KMS key one
   * year after the enable date and every year thereafter. KMS retains all key material until you delete
   * the KMS key. For detailed information about automatic key rotation, see [Rotating KMS
   * keys](https://docs.aws.amazon.com/kms/latest/developerguide/rotate-keys.html) in the *Developer
   * Guide*.
   */
   ...
   * in the *Developer Guide*. (To omit this condition, set ``BypassPolicyLockoutSafetyCheck`` to true.)
   * +  Each statement in the key policy must contain one or more principals. The principals in the
   * key policy must exist and be visible to KMS. When you create a new AWS principal (for example, an
   * IAM user or role), you might need to enforce a delay before including the new principal in a key
   * policy because the new principal might not be immediately visible to KMS. For more information, see
   ...
   * KMS key. The default is ``AWS_KMS``, which means that KMS creates the key material.
   ...
   * Material](https://docs.aws.amazon.com/kms/latest/developerguide/importing-keys.html) in the
   * *Developer Guide*.
   ...
   * Operations.
   * +  CFN doesn't support creating an ``Origin`` parameter of the ``AWS_CLOUDHSM`` or
   ...
   * key with a 256-bit symmetric key for encryption and decryption. In China Regions,
   * ``SYMMETRIC_DEFAULT`` creates a 128-bit symmetric key that uses SM4 encryption. You can't change
   * the ``KeySpec`` value after the KMS key is created. For help choosing a key spec for your KMS key,
   * see [Choosing a KMS key
   * type](https://docs.aws.amazon.com/kms/latest/developerguide/symm-asymm-choose.html) in the
   * *Developer Guide*.
   ...
   * use a condition key in its key policy or IAM policy. For more information, see [condition
   ...
   * with](https://docs.aws.amazon.com/kms/features/#AWS_Service_Integration) use symmetric encryption
   ...
   * keys](https://docs.aws.amazon.com/kms/latest/developerguide/find-symm-asymm.html) in the *Developer
   * Guide*.
  ...
  /**
   * Creates a multi-Region primary key that you can replicate in other AWS-Regions. You can't change
   * the ``MultiRegion`` value after the KMS key is created.
   * For a list of AWS-Regions in which multi-Region keys are supported, see [Multi-Region keys
   * in](https://docs.aws.amazon.com/kms/latest/developerguide/multi-region-keys-overview.html) in the
   * **.
   ...
   * *Multi-Region keys* are an KMS feature that lets you create multiple interoperable KMS keys in
   * different AWS-Regions. Because these KMS keys have the same key ID, key material, and other
   * metadata, you can use them to encrypt data in one AWS-Region and decrypt it in a different
   ...
   * create a multi-Region key with imported key material. However, you cannot create a multi-Region key
  ...
  MultiRegion?: boolean;
  /**
   * Specifies the number of days in the waiting period before KMS deletes a KMS key that has been
   * removed from a CloudFormation stack. Enter a value between 7 and 30 days. The default value is 30
   * days.
   * When you remove a KMS key from a CloudFormation stack, KMS schedules the KMS key for deletion and
   * starts the mandatory waiting period. The ``PendingWindowInDays`` property determines the length of
   * waiting period. During the waiting period, the key state of KMS key is ``Pending Deletion`` or
   * ``Pending Replica Deletion``, which prevents the KMS key from being used in cryptographic
   * operations. When the waiting period expires, KMS permanently deletes the KMS key.
   * KMS will not delete a [multi-Region primary
   ...
   * ``PendingWindowInDays`` begins. When this waiting period expires, KMS deletes the primary key. For
   * details, see [Deleting multi-Region
   ...
   * [ScheduleKeyDeletion](https://docs.aws.amazon.com/kms/latest/APIReference/API_ScheduleKeyDeletion.html)
   * operation in the *API Reference* and [Deleting KMS
   * keys](https://docs.aws.amazon.com/kms/latest/developerguide/deleting-keys.html) in the *Developer
   * Guide*.
   * @minimum 7
   * @maximum 30
   */
   ...
   * [ABAC for](https://docs.aws.amazon.com/kms/latest/developerguide/abac.html) in the *Developer
   * Guide*.
   * For information about tags in KMS, see [Tagging
   * keys](https://docs.aws.amazon.com/kms/latest/developerguide/tagging-keys.html) in the *Developer
   * Guide*. For information about tags in CloudFormation, see
  ...
  Tags?: {
     ...
     * can't be prefixed with ``aws:``. digits, whitespace, ``_``, ``.``, ``:``, ``/``, ``=``, ``+``,
     * ``@``, ``-``, and ``"``.
  ...
  /**
   * Specifies a custom period of time between each rotation date. If no value is specified, the default
   * value is 365 days.
   * The rotation period defines the number of days after you enable automatic key rotation that KMS
   * will rotate your key material, and the number of days between each automatic rotation thereafter.
   * You can use the
