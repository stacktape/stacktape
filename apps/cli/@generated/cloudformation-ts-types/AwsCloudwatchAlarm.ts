...
// Source: aws-cloudwatch-alarm.json
...
export type AwsCloudwatchAlarm = {
   ...
   * ``notBreaching``, ``ignore``, and ``missing``. For more information, see [Configuring How Alarms
   * Treat Missing
   ...
   * a metric. Valid values are 10, 20, 30, 60, and any multiple of 60.
   ...
   * alarm that requires that a number of consecutive data points be breaching to trigger the alarm,
   * this value specifies that number. If you are setting an "M out of N" alarm, this value is the N,
   * and ``DatapointsToAlarm`` is the M.
   ...
   * ``Metrics`` instead.
   * For a list of namespaces for metrics from AWS services, see [Services That Publish
   ...
   * state. Specify each action as an Amazon Resource Name (ARN). For more information about creating
   ...
   * For an alarm based on a math expression, you use ``Metrics`` instead and you can't specify
   * ``MetricName``.
   */
  MetricName?: string;
  /**
  ...
  Metrics?: {
     ...
     * CW dashboard widget, the label is shown. If ``Label`` is omitted, CW generates a default.
     */
    Label?: string;
    /**
    ...
    MetricStat?: {
       ...
       * metrics that are collected at intervals of less than one minute, the period can be 1, 5, 10, 20,
      ...
      /**
       * The statistic to return. It can include any CW statistic or extended statistic. For a list of valid
  ...
  /**
   * The name of the alarm. If you don't specify a name, CFN generates a unique physical ID and uses
   ...
   * setting an "M out of N" alarm. In that case, this value is the M, and the value that you set for
   * ``EvaluationPeriods`` is the N value. For more information, see [Evaluating an
   * Alarm](https://docs.aws.amazon.com/AmazonCloudWatch/latest/monitoring/AlarmThatSendsEmail.html#alarm-evaluation)
   * in the *User Guide*.
