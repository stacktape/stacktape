...
// Source: aws-rds-dbshardgroup.json
 ...
 */
export type AwsRdsDbshardgroup = {
  ...
  /**
   * Specifies whether to create standby standby DB data access shard for the DB shard group. Valid
   ...
   * When the DB shard group is publicly accessible, its Domain Name System (DNS) endpoint resolves to
   * the private IP address from within the DB shard group's virtual private cloud (VPC). It resolves to
   * the public IP address from outside of the DB shard group's VPC. Access to the DB shard group is
   * ultimately controlled by the security group it uses. That public access is not permitted if the
  ...
  Endpoint?: string;
  /**
   * An optional set of key-value pairs to associate arbitrary data of your choosing with the DB shard
   * group.
   * @maxItems 50
   * @uniqueItems true
   */
