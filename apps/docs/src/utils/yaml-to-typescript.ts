...
const replaceIdentifierValues = (value: TsParsedValue | IdentifierValue): TsParsedValue => {
  if (isIdentifierValue(value)) return value.__identifier;
  if (Array.isArray(value)) return value.map((item) => replaceIdentifierValues(item));
  if (value && typeof value === 'object') {
    return Object.fromEntries(
      Object.entries(value).map(([key, childValue]) => [key, replaceIdentifierValues(childValue)])
    );
  }
  return value;
};
  ...
  for (const [outputName, declaredValue] of Object.entries(sectionValue)) {
    ...
    if (declaration?.kind === kind) {
      section[outputName] = {
        ...
        properties: replaceIdentifierValues(declaration.properties)
      };
    }
  }
...
export const convertTypescriptToYaml = (typescriptContent: string): string | null => {
  try {
    ...
    for (const [key, value] of Object.entries(returnedConfig)) {
      ...
      yamlConfig[key] = replaceIdentifierValues(value);
    }
