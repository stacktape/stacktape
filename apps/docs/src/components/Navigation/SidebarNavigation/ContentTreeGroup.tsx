...
// Cheap equality for measurement state — avoids a setState (and the re-render storm through the
// whole sub-tree) when ResizeObserver fires with the same values.
...
const useGuidePath = (containerRef: React.RefObject<HTMLDivElement | null>, flat: { depth: number }[]) => {
  ...
  return (
    <div className="block p-0 relative mb-2">
      <button
        ...
        className="stp-nav-group-header"
      >
