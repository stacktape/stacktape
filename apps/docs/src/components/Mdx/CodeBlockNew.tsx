...
type ShikiRenderOptions = { lang: string; theme: string; transformers?: unknown[] };

type ShikiRuntime = {
  codeToHtml: (code: string, options: ShikiRenderOptions) => Promise<string>;
};
    ...
    codeToHtml: (code, options) =>
      Promise.resolve(
        highlighter.codeToHtml(code, {
          theme: options.theme,
          // Shiki's transformer type is structural and varies by transformer package; the twoslash
          // and YAML-hover transformers are both accepted at runtime.
          transformers: options.transformers as Parameters<typeof highlighter.codeToHtml>[1]['transformers'],
...
const preprocessCode = ({
  ...
  const displayFocusedLineSpecs = focusedLineSpecs.map((spec): FocusedLineSpec => {
    ...
    return spec.kind === 'line'
      ? { kind: 'line', fullLineNumber: spec.fullLineNumber, text: spec.text, displayText }
      : { kind: 'placeholder', text: displayText };
  });
...
const renderWithShiki = async ({
  ...
  const html = await runtime.codeToHtml(code, {
    lang: language,
    theme: SHIKI_THEME,
    transformers: [...(twoslashTransformer ? [twoslashTransformer] : []), ...extraTransformers]
  });
...
/**
 * Shiki's Twoslash transformer, backed by the locally served virtual filesystem.
 *
 * Built at most once per page and shared by every code block, on first use rather than at import
 * time — the declarations are several megabytes and only pages with an intellisense block need them.
 * There is no automatic type acquisition: `stacktape` and the TypeScript standard library are
 * already in the fs map, and any other import resolves to `any` instead of silently pulling a
 * package off a CDN.
 */
let twoslashTransformerPromise: Promise<unknown> | undefined;

const getTwoslashTransformer = () => {
  twoslashTransformerPromise ??= (async () => {
    const [{ createTransformerFactory, rendererRich }, { createTwoslasher }, fsMap] = await Promise.all([
      import('@shikijs/twoslash'),
      import('twoslash'),
      loadTwoslashFsMap()
    ]);

    return createTransformerFactory(createTwoslasher({ compilerOptions: TWOSLASH_COMPILER_OPTIONS, fsMap }))({
...
export function MdxCodeBlockNew({
  ...
  return <CodeBlockNew intellisense={shouldEnableIntellisense} lang={lang} tabs={tabs} />;
}
...
export function CodeBlockNew({ tabs, lang, intellisense = false, stacktapeConfig = false }: CodeBlockNewProps) {
  ...
  useEffect(() => {
    ...
    const renderHighlightedCode = async () => {
      try {
        ...
        const twoslashTransformer = shouldUseTwoslash ? await getTwoslashTransformer() : undefined;
  ...
  }, [activeTab.code, activeTab.lang, lang, processedCode, shouldEnableIntellisense]);
