...
const loadConfigSchema = () => {
  if (!schemaPromise) {
    // The published configuration schema, straight from the package that owns it. Loaded lazily so
    // its weight only lands on readers who hover a Stacktape YAML sample.
    schemaPromise = import('@stacktape/config/config-schema.json').then(
...
export const computeYamlHovers = async (originalCode: string, lineMap?: number[]) => {
  try {
    ...
    if (lineMap) {
      ...
      hovers = rawHovers
        .map((hover): HoverData | null => {
          const rendered = originalToRendered.get(hover.line);
          if (rendered === undefined) return null;
          return {
            line: rendered,
            charStart: hover.charStart,
            charEnd: hover.charEnd,
            title: hover.title,
            description: hover.description,
            defaultValue: hover.defaultValue
          };
        })
        .filter((hover): hover is HoverData => hover !== null);
