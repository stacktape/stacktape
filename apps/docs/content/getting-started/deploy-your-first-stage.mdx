...
<Tip>
Connecting an AWS account is free. AWS resources created by your stacks are billed by AWS in your AWS account.
</Tip>
...
<FlowDiagram steps={[
  ...
  { title: "Post-deploy", description: "For stacks that include hosting buckets or CDNs, Stacktape syncs static files and invalidates CDN caches. Then it prepares deployed stack information and resource links." }
]} />
...
<PreviousNext left={{ title: 'Use the Dev Mode', url: '/getting-started/use-the-dev-mode' }} right={{ title: 'Using Console UI', url: '/getting-started/using-console-ui' }} />