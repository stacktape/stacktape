...
<Info>This page covers `logs` and `metrics` in depth. For the other commands, use the linked CLI reference pages for their flags, operations, and requirements.</Info>
...
<Warning>`container:exec` and `container:session` execute arbitrary commands inside live containers. These are NOT read-only operations — use caution in production stages.</Warning>
