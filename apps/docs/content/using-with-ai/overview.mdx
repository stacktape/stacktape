...
order: 122
seoTitle: 'Using Stacktape with AI Coding Assistants | Stacktape Docs'
seoDescription: 'Use Stacktape with AI coding assistants through the MCP server, agent mode for local development, and AI config generation. Set up with mcp:add.'
---

# Using Stacktape with AI

<Warning>
Preview — in active development. The Stacktape MCP server is under active development. Available tools, safety behavior, and integration details may change between releases.
</Warning>
...
<FlowDiagram steps={[
  { title: "Generate config", description: "AI writes stacktape.ts from your requirements or existing codebase" },
  { title: "Start dev mode", description: "Agent mode launches local workloads with an HTTP API" },
  { title: "Write application code", description: "AI implements handlers, services, and business logic" },
  { title: "Test and iterate", description: "AI reads logs, queries databases, triggers rebuilds, and fixes errors" },
  { title: "Deploy", description: "AI deploys the working stack to AWS via the MCP server" }
]} />
...
<FeatureComparisonTable
  columns={["MCP server", "Agent mode", "Config generation", "mcp:add"]}
  features={[
    { name: "What it does", values: { "MCP server": "Structured Stacktape operations from your editor", "Agent mode": "HTTP API for iterative local development", "Config generation": "Generate stacktape.ts from your codebase", "mcp:add": "Register MCP server in your editor" } },
    { name: "When to use", values: { "MCP server": "Ongoing development with AI", "Agent mode": "AI-driven build-test-fix loops", "Config generation": "Starting a new Stacktape project", "mcp:add": "One-time setup per editor" } },
    { name: "Can deploy to AWS", values: { "MCP server": true, "Agent mode": false, "Config generation": false, "mcp:add": false } },
    { name: "Reads logs", values: { "MCP server": "Deployed stacks", "Agent mode": "Local dev", "Config generation": false, "mcp:add": false } },
    { name: "Queries databases", values: { "MCP server": "Deployed databases", "Agent mode": "Local databases", "Config generation": false, "mcp:add": false } }
  ]}
/>
