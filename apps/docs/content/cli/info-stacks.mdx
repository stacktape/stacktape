...
seoDescription: 'List all deployed CloudFormation stacks in an AWS region with status, timestamps, and available spend data. Audit deployments and find forgotten infrastructure.'
---
...
The `--stage` and `--projectName` flags are accepted by the CLI parser but are not used in the stack-listing logic. The command lists all non-deleted stacks returned for the selected AWS region and account.
