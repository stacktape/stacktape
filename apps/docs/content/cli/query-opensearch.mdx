...
order: 171
seoTitle: 'query:opensearch CLI Command | Stacktape Docs'
seoDescription: 'Run read-only queries against a deployed OpenSearch domain from the terminal. List indices, count documents, inspect mappings, retrieve by ID, and search with Query DSL using Stacktape CLI.'
---
...
The `--query` flag accepts [OpenSearch Query DSL](https://opensearch.org/docs/latest/query-dsl/) as a JSON string. The command parses the JSON before sending it — malformed JSON is rejected with `Invalid JSON in --query` and the hint `Provide valid OpenSearch query DSL JSON`. On Windows, escape inner double quotes with backslashes. On Linux/macOS, wrap the entire value in single quotes.
