...
title: 'logs'
order: 158
seoTitle: 'stacktape logs — Fetch & Filter Logs | Stacktape CLI'
seoDescription: 'Fetch and filter logs from deployed Lambda functions and containers with stacktape logs. Filter by pattern, run CloudWatch Insights queries, and get structured JSON output.'
---
...
<Info>
This command requires a Stacktape configuration file (set via `--configPath` or auto-detected in the working directory). The configuration is used to resolve the deployed stack and locate the correct CloudWatch log group for the specified resource.
</Info>
...
<Tip>
If no log streams exist for the resource, the command prints an empty result. In agent mode, this is `{"logGroup": "...", "events": []}`.
</Tip>
