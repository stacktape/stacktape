...
<Warning>
Do not pass secrets through `buildArgs` — they are visible in the image layer history. Use runtime configuration for secrets instead. See [secrets](/configuration/secrets) for details.
</Warning>
