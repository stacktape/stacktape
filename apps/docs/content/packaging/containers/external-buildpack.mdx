...
<Info>
Only `paketobuildpacks/builder-jammy-base` is the Stacktape-configured default. The other builders listed above are commonly used external options — consult each builder's documentation for current language support and available buildpacks.
</Info>
