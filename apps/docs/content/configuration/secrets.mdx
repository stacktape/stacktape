...
seoDescription: 'Store API keys and passwords in AWS Secrets Manager and reference them with $Secret() in your Stacktape config. Values stay encrypted and out of version control.'
---
...
<ConsoleScreenshot
  ...
  alt="Stacktape Console Secrets page showing a table of secrets with name, date columns, description, and action icons for view, edit, and delete"
  ...
  tabs={[
    {
      label: 'TypeScript',
      lang: 'typescript',
      code: `import { $Secret, $SsmParam, WebService, StacktapeImageBuildpackPackaging, defineConfig } from 'stacktape';
...
<Warning>
Using `$Secret()` or `$SsmParam()` under a container's ordinary `environment` field makes CloudFormation resolve the
...
<Warning>
Dot notation requires the secret value to be valid JSON. If the value is not valid JSON, or the selected key does
...
<Info>
After updating a secret, redeploy the stack for workloads to pick up the new value. The `$Secret()` directive pins to
...
<Warning>
Do not delete secrets that are referenced by active stacks. The next deployment that references the missing secret
...
<FeatureComparisonTable
  ...
  features={[
    ...
    { name: "JSON key extraction (dot notation)", values: { "$Secret()": true, "$SsmParam()": "Not implemented" } },
    { name: "Encryption", values: { "$Secret()": "Always encrypted (KMS)", "$SsmParam()": "SecureString type uses KMS" } },
    { name: "CF dynamic reference type", values: { "$Secret()": "secretsmanager", "$SsmParam()": "ssm / ssm-secure" } },
    { name: "Version pinning", values: { "$Secret()": "Pins to VersionId", "$SsmParam()": "Pins to Version number" } }
  ]}
/>
