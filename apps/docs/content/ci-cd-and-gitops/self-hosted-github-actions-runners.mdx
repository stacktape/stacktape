...
<FlowDiagram steps={[
  ...
  { title: "Runner ensured", description: "The handler calls ensureRunner for the project, AWS account, region, and instance type." },
...
<Tip>
Start with `c7a.2xlarge`. If your workflows consistently finish quickly and don't use much memory, downsize to `c7a.xlarge` or `m6a.large`. If Docker builds or test suites are the bottleneck, try `c7a.4xlarge`.
</Tip>
...
<FeatureComparisonTable
  columns={["Self-hosted runners", "GitOps with Console", "Custom CI/CD"]}
  features={[
    { name: "Pipeline customization", values: { "Self-hosted runners": "Full GitHub Actions", "GitOps with Console": "None — git event triggers deploy", "Custom CI/CD": "Full — any CI system" } },
    { name: "Pre-deploy testing", values: { "Self-hosted runners": true, "GitOps with Console": false, "Custom CI/CD": true } },
    { name: "Setup effort", values: { "Self-hosted runners": "Enable in Console + modify workflow YAML", "GitOps with Console": "Configure in Console only", "Custom CI/CD": "Write workflow file" } },
    { name: "Compute options", values: { "Self-hosted runners": "Up to 32 vCPU", "GitOps with Console": "See Build runners page", "Custom CI/CD": "Varies by CI provider" } },
    { name: "Cost model", values: { "Self-hosted runners": "EC2 per-second + EBS", "GitOps with Console": "See Build runners page", "Custom CI/CD": "CI provider minutes" } }
  ]}
/>
