...
seoDescription: 'Restrict allowed stages, AWS regions, CLI commands, resource types, and stack size with Stacktape deployment guardrails. Block non-compliant deployments before they happen.'
---
...
<Info>
Command restriction is enforced by Stacktape only. For deletion protection that also covers AWS Console and direct AWS API calls, enable the `require-stack-termination-protection` guardrail.
</Info>

<Tip>
A common pattern is to block `delete` so that stack removal requires removing it from `blockedCommands` first. This adds a deliberate review step for destructive operations without removing the capability entirely.
</Tip>
