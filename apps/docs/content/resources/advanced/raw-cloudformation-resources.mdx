...
<Warning>
To reference Stacktape-managed resources by logical ID in intrinsic functions, run [`stacktape info:stack`](/cli/info-stack) to discover the logical names Stacktape generates. These are internal naming conventions and may change between Stacktape versions — always verify before hardcoding references.
</Warning>
...
<Warning>
Stacktape's internal naming conventions are implementation details and may change between versions. Always verify logical names with `stacktape info:stack` rather than guessing or hardcoding references to Stacktape-managed names.
</Warning>
