...
seoDescription: 'Use stacktape diff to see what a deployment would create, update, replace, or remove in your AWS stack before applying changes. Catch breaking changes early.'
---
...
<Tip>
Use [`stacktape defaults:configure`](/cli/defaults-configure) to set a default stage, region, and AWS profile so you can run `stacktape diff` without flags during development.
</Tip>
