...
seoDescription: 'Learn how Stacktape deploys infrastructure to AWS with CloudFormation. Full deploy, hot-swap for fast code iteration, CodeBuild for remote builds, and auto-rollback on failure.'
---
...
<FlowDiagram
  steps={[
    ...
    { title: 'Package & prepare template', description: 'Bundle Lambda code, build container images, resolve resources, prepare CloudFormation template' },
    { title: 'Plan & protect', description: 'Fingerprint the final target, template, workload builds, and protected-resource risks' },
    { title: 'Upload', description: 'Upload packaged artifacts to S3, push container images to ECR' },
    { title: 'Deploy', description: 'Create or update the CloudFormation stack (or hot-swap eligible workloads directly)' },
    { title: 'Post-deploy', description: 'Run afterDeploy hooks, sync hosting buckets, invalidate CDN caches, send notifications' }
  ]}
/>
...
<FeatureComparisonTable
  ...
  features={[
    { name: 'Can update all resource types', values: { 'Full deploy': true, 'Hot-swap': false, 'CodeBuild deploy': true } },
    { name: 'Adds or removes resources', values: { 'Full deploy': true, 'Hot-swap': false, 'CodeBuild deploy': true } },
    { name: 'Changes IAM, networking, config', values: { 'Full deploy': true, 'Hot-swap': false, 'CodeBuild deploy': true } },
    { name: 'Automatic rollback on failure', values: { 'Full deploy': true, 'Hot-swap': false, 'CodeBuild deploy': true } },
    { name: 'Typical duration', values: { 'Full deploy': 'Minutes (varies with stack complexity)', 'Hot-swap': 'Seconds', 'CodeBuild deploy': 'Minutes (includes upload and build overhead)' } },
    { name: 'Build environment', values: { 'Full deploy': 'Local machine', 'Hot-swap': 'Local machine', 'CodeBuild deploy': 'AWS CodeBuild' } }
  ]}
/>
...
<Warning>
Hot-swap updates resources outside of CloudFormation, which creates drift between the CloudFormation template and actual infrastructure. Use hot-swap only for development stages. Always use full deployment for production.
</Warning>
...
<Warning>
Don't leave production stacks in a failed state. Fix the issue and redeploy promptly, or recover the stack with [`stacktape cf:rollback`](/cli/cf-rollback).
</Warning>
