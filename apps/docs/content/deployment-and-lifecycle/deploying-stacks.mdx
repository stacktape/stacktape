...
seoDescription: 'Learn how Stacktape deploys infrastructure to AWS with CloudFormation, hot-swap for fast code iteration, change plans, and automatic rollback on failure.'
---
...
<FeatureComparisonTable
  columns={['Full deploy', 'Hot-swap']}
  features={[
    { name: 'Can update all resource types', values: { 'Full deploy': true, 'Hot-swap': false } },
    { name: 'Adds or removes resources', values: { 'Full deploy': true, 'Hot-swap': false } },
    { name: 'Changes IAM, networking, config', values: { 'Full deploy': true, 'Hot-swap': false } },
    { name: 'Automatic rollback on failure', values: { 'Full deploy': true, 'Hot-swap': false } },
    { name: 'Typical duration', values: { 'Full deploy': 'Minutes (varies with stack complexity)', 'Hot-swap': 'Seconds' } },
    { name: 'Execution environment', values: { 'Full deploy': 'Invoking CLI or Console EC2 runner', 'Hot-swap': 'Invoking CLI' } }
  ]}
/>
