...
seoDescription: 'View CloudWatch metrics for Lambda, ECS, RDS, and API Gateway using the Stacktape Console dashboard or the metrics CLI command. Nine metric categories, no setup required.'
---
...
<FeatureComparisonTable
  columns={["Console dashboard", "CLI metrics"]}
  features={[
    { name: "Interaction", values: { "Console dashboard": "Visual charts", "CLI metrics": "Terminal sparkline or JSON" } },
    { name: "Scope", values: { "Console dashboard": "All supported metric categories at once", "CLI metrics": "One resource, one metric per invocation" } },
    { name: "Resource categories", values: { "Console dashboard": "9 (Lambda, ECS, RDS, API Gateway, DynamoDB, S3, CloudFront, Cognito, Redis)", "CLI metrics": "4 (Lambda, ECS, RDS, API Gateway)" } },
    { name: "Time range", values: { "Console dashboard": "Presets (default: 7d)", "CLI metrics": "Custom (relative or absolute start/end)" } },
    { name: "Custom statistic", values: { "Console dashboard": false, "CLI metrics": "Any CloudWatch statistic (default: Average)" } },
    { name: "Scriptable output", values: { "Console dashboard": false, "CLI metrics": "JSON in agent mode" } },
    { name: "Best for", values: { "Console dashboard": "Daily monitoring, team visibility", "CLI metrics": "Debugging, CI scripts, quick checks" } }
  ]}
/>
