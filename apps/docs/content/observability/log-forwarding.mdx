...
<Warning>
CloudWatch Logs Infrequent Access does not support the subscription filters used for forwarding. Stacktape therefore rejects a logging block that combines `logClass: infrequent-access` with `logForwarding`; keep the default Standard class for forwarded logs.
</Warning>
