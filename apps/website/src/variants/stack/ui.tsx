...
export function PreviewButton({ variant, icon, iconPosition = 'start', small = false, children }: PreviewButtonProps) {
  return (
    <Button
      aria-disabled="true"
      className={small ? 'st-btn st-btn--small' : 'st-btn'}
...
export function StatusBadge({ tone, dot = false, children }: StatusBadgeProps) {
  return (
    <Badge className={dot ? 'st-badge st-badge--dot' : 'st-badge'} tone={tone}>
      {dot ? <i aria-hidden="true" className="st-badge__dot" /> : null}
      {children}
    </Badge>
  );
}
...
export function RuleSwitch({ label }: { label: string }) {
  return <Switch checked className="st-rule__switch" disabled label={label} onCheckedChange={() => {}} />;
}
