...
export function CommandBox({ label = 'Get started', hint = true }: { label?: string; hint?: boolean }) {
  ...
  return (
    <div className={`st-cmd st-cmd--${state}`}>
      <div className="st-cmd__top">
        <Tabs
          appearance="segmented"
          ariaLabel={`${label}: install method`}
          className="st-cmd__tabs"
          ...
          width="fit"
        />
        {hint ? <p className="st-cmd__hint">{COMMAND_HINT}</p> : null}
      </div>

      <div className={overflows ? 'st-cmd__row st-cmd__row--overflow' : 'st-cmd__row'} id={panelId} role="tabpanel">
        <span aria-hidden="true" className="st-cmd__prompt">
          $
        </span>
        <code className={long ? 'st-cmd__code st-cmd__code--long' : 'st-cmd__code'} ref={codeRef}>
          {entry.command}
          <span aria-hidden="true" className="st-cmd__caret" />
        </code>
        <Button aria-label={`Copy command: ${entry.command}`} className="st-cmd__copy" onClick={copy} variant="primary">
          <span className="st-cmd__labels" aria-hidden="true">
            <span className="st-cmd__label st-cmd__label--copy">
              <CopyIcon />
              Copy
            </span>
            <span className="st-cmd__label st-cmd__label--copied">
              <CheckIcon />
              Copied
            </span>
            <span className="st-cmd__label st-cmd__label--failed">Select and copy</span>
          </span>
        </Button>
      </div>

      <p aria-live="polite" className="st-visually-hidden">
        {status}
      </p>
    </div>
  );
