 ...
 * hero card and in the closing card.
 *
 ...
 */
export function CommandBox({ label = 'Get started', hint = true }: { label?: string; hint?: boolean }) {
  ...
  return (
    <div className={`dk-cmd dk-cmd--${state}`}>
      <div className="dk-cmd__top">
        <Tabs
          appearance="segmented"
          ariaLabel={`${label}: install method`}
          className="dk-cmd__tabs"
          ...
          width="fit"
        />
        {hint ? <p className="dk-cmd__hint">opens the wizard in your browser</p> : null}
      </div>

      <div className={overflows ? 'dk-cmd__row dk-cmd__row--overflow' : 'dk-cmd__row'} id={panelId} role="tabpanel">
        <span aria-hidden="true" className="dk-cmd__prompt">
          $
        </span>
        <code className={long ? 'dk-cmd__code dk-cmd__code--long' : 'dk-cmd__code'} ref={codeRef}>
          {entry.command}
          <span aria-hidden="true" className="dk-cmd__caret" />
        </code>
        <Button aria-label={`Copy command: ${entry.command}`} className="dk-cmd__copy" onClick={copy} variant="primary">
          <span className="dk-cmd__labels" aria-hidden="true">
            <span className="dk-cmd__label dk-cmd__label--copy">
              <CopyIcon />
              Copy
            </span>
            <span className="dk-cmd__label dk-cmd__label--copied">
              <CheckIcon />
              Copied
            </span>
            <span className="dk-cmd__label dk-cmd__label--failed">Select and copy</span>
          </span>
        </Button>
      </div>

      <p aria-live="polite" className="dk-visually-hidden">
        {status}
      </p>
    </div>
  );
