...
export function CommandBox({
  ...
  return (
    <div className={`ls-cmd ls-cmd--${state}`} id={id}>
      <div className="ls-cmd__top">
        <Tabs
          appearance="segmented"
          ariaLabel={`${label}: install method`}
          className="ls-cmd__tabs"
          ...
          width="fit"
        />
        {hint ? <p className="ls-cmd__hint">opens the wizard in your browser</p> : null}
      </div>

      <div className={overflows ? 'ls-cmd__row ls-cmd__row--overflow' : 'ls-cmd__row'} id={panelId} role="tabpanel">
        <span aria-hidden="true" className="ls-cmd__prompt">
          $
        </span>
        <code className={long ? 'ls-cmd__code ls-cmd__code--long' : 'ls-cmd__code'} ref={codeRef}>
          {entry.command}
          <span aria-hidden="true" className="ls-cmd__caret" />
        </code>
        <Button aria-label={`Copy command: ${entry.command}`} className="ls-cmd__copy" onClick={copy} variant="primary">
          <span className="ls-cmd__labels" aria-hidden="true">
            <span className="ls-cmd__label ls-cmd__label--copy">
              <CopyIcon />
              Copy
            </span>
            <span className="ls-cmd__label ls-cmd__label--copied">
              <CheckIcon />
              Copied
            </span>
            <span className="ls-cmd__label ls-cmd__label--failed">Select and copy</span>
          </span>
        </Button>
      </div>

      <p aria-live="polite" className="ls-visually-hidden">
        {status}
      </p>
    </div>
  );
