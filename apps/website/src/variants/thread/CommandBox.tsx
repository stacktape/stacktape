...
export function CommandBox({ label = 'Get started', hint = true }: { label?: string; hint?: boolean }) {
  ...
  return (
    <div className={`th-cmd th-cmd--${state}`}>
      <div className="th-cmd__top">
        <Tabs
          appearance="segmented"
          ariaLabel={`${label}: install method`}
          className="th-cmd__tabs"
          ...
          width="fit"
        />
        {hint ? <p className="th-cmd__hint">opens the wizard in your browser</p> : null}
      </div>

      <div className={overflows ? 'th-cmd__row th-cmd__row--overflow' : 'th-cmd__row'} id={panelId} role="tabpanel">
        <span aria-hidden="true" className="th-cmd__prompt">
          $
        </span>
        <code className={long ? 'th-cmd__code th-cmd__code--long' : 'th-cmd__code'} ref={codeRef}>
          {entry.command}
          <span aria-hidden="true" className="th-cmd__caret" />
        </code>
        <Button aria-label={`Copy command: ${entry.command}`} className="th-cmd__copy" onClick={copy} variant="primary">
          <span className="th-cmd__labels" aria-hidden="true">
            <span className="th-cmd__label th-cmd__label--copy">
              <CopyIcon />
              Copy
            </span>
            <span className="th-cmd__label th-cmd__label--copied">
              <CheckIcon />
              Copied
            </span>
            <span className="th-cmd__label th-cmd__label--failed">Select and copy</span>
          </span>
        </Button>
      </div>

      <p aria-live="polite" className="th-visually-hidden">
        {status}
      </p>
    </div>
  );
