/*
 * Everything the "thread" study says and shows.
 *
 * The page copy, the six surfaces, the example project, the values on every screen and the
 * testimonials live here so the Astro components read one source. Nothing runs at import time. The
 * YAML highlighter at the bottom is a pure function used from `.astro` frontmatter; the command box
 * island imports `COMMANDS` alone.
 */
...
export const TERMINAL = {
  command: 'npx stacktape init',
  output: ['Reading acme/acme-project on this machine… 41 files in 38 s', 'Opening the wizard at http://localhost:3000']
} as const;
...
export const WIZARD = {
  ...
  command: 'stacktape init',
  rail: ['Start', 'Read', 'Review', 'Deploy']
} as const;
...
export const DEPLOY = {
  ...
  phases: [
    { name: 'Package', detail: 'web 41 s · apiService 58 s · worker 6 s' },
    { name: 'Deploy', detail: '33 resources created in your account' },
    { name: 'Outputs', detail: '2 public URLs' }
  ]
} as const;
...
export const PULL_REQUEST = {
  ...
  checks: [
    { name: 'Preview deployed', detail: 'pr-128.acme-preview.com' },
    { name: 'Guardrails', detail: '18 rules passed' },
    { name: 'Image scan', detail: '0 critical' }
  ]
} as const;
...
export const GUARDRAILS = {
  ...
  rule: 'A deploy that would break a rule is blocked before anything changes.'
} as const;
...
const span = (cls: string, text: string) => `<span class="th-y-${cls}">${escapeHtml(text)}</span>`;
