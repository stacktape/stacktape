...
export function PreviewButton({ variant, icon, iconPosition = 'start', small = false, children }: PreviewButtonProps) {
  return (
    <Button
      aria-disabled="true"
      className={small ? 'th-btn th-btn--small' : 'th-btn'}
...
export function StatusBadge({ tone, dot = false, caps = true, children }: StatusBadgeProps) {
  return (
    <Badge caps={caps} className={dot ? 'th-badge th-badge--dot' : 'th-badge'} tone={tone}>
      {dot ? <i aria-hidden="true" className="th-badge__dot" /> : null}
      {children}
    </Badge>
  );
}
...
export function OptInSwitch({ label }: { label: string }) {
  return <Switch checked={false} className="th-optin__switch" disabled label={label} onCheckedChange={() => {}} />;
}
