...
/**
 * The only call to action, drawn as a small dark terminal window on the paper: the install method
 * in the title strip, one command in the body, a primary Copy button. Server-rendered so it is
 * visible before hydration; the same component sits in the hero and in the closing band.
 *
...
export function CommandBox({
  ...
  return (
    <div className={`sh-cmd sh-cmd--${state}`} id={id}>
      <div className="sh-cmd__bar">
        <span aria-hidden="true" className="sh-dots">
          <i />
          <i />
          <i />
        </span>
      ...
      <div className="sh-cmd__body">
        <div className={overflows ? 'sh-cmd__row sh-cmd__row--overflow' : 'sh-cmd__row'} id={panelId} role="tabpanel">
          <span aria-hidden="true" className="sh-cmd__prompt">
            $
          </span>
          <code className={long ? 'sh-cmd__code sh-cmd__code--long' : 'sh-cmd__code'} ref={codeRef}>
            {entry.command}
            <span aria-hidden="true" className="sh-cmd__caret" />
          </code>
          <Button
            aria-label={`Copy command: ${entry.command}`}
            className="sh-cmd__copy"
            onClick={copy}
            variant="primary"
          >
            <span className="sh-cmd__labels" aria-hidden="true">
              <span className="sh-cmd__label sh-cmd__label--copy">
                <CopyIcon />
                Copy
              </span>
              <span className="sh-cmd__label sh-cmd__label--copied">
                <CheckIcon />
                Copied
              </span>
              <span className="sh-cmd__label sh-cmd__label--failed">Select and copy</span>
            </span>
          </Button>
        </div>
        {hint ? <p className="sh-cmd__hint">{COMMAND_HINT}</p> : null}
      </div>

      <p aria-live="polite" className="sh-visually-hidden">
        {status}
      </p>
    </div>
  );
