...
/** A button that looks pressable and is not: previews never navigate or submit. */
export function PreviewButton({ variant, icon, iconPosition = 'start', small = false, children }: PreviewButtonProps) {
  return (
    <Button
      aria-disabled="true"
      className={small ? 'hm-btn hm-btn--small' : 'hm-btn'}
...
export function StatusBadge({ tone, dot = false, caps = true, variant = 'soft', children }: StatusBadgeProps) {
  return (
    <Badge caps={caps} className={dot ? 'hm-badge hm-badge--dot' : 'hm-badge'} tone={tone} variant={variant}>
      {dot ? <i aria-hidden="true" className="hm-badge__dot" /> : null}
      {children}
    </Badge>
  );
}
...
export function OptInSwitch({ label }: { label: string }) {
  return <Switch checked={false} className="hm-optin__switch" disabled label={label} onCheckedChange={() => {}} />;
}
