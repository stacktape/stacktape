...
export function CommandBox({
  ...
  return (
    <div className={`hr-cmd hr-cmd--${state}`} id={id}>
      <div className="hr-cmd__bar">
        <span aria-hidden="true" className="hr-dots">
          <i />
          <i />
          <i />
        </span>
      ...
      <div className="hr-cmd__body">
        <div className={overflows ? 'hr-cmd__row hr-cmd__row--overflow' : 'hr-cmd__row'} id={panelId} role="tabpanel">
          <span aria-hidden="true" className="hr-cmd__prompt">
            $
          </span>
          <code className={long ? 'hr-cmd__code hr-cmd__code--long' : 'hr-cmd__code'} ref={codeRef}>
            {entry.command}
            <span aria-hidden="true" className="hr-cmd__caret" />
          </code>
          <Button
            aria-label={`Copy command: ${entry.command}`}
            className="hr-cmd__copy"
            onClick={copy}
            variant="primary"
          >
            <span className="hr-cmd__labels" aria-hidden="true">
              <span className="hr-cmd__label hr-cmd__label--copy">
                <CopyIcon />
                Copy
              </span>
              <span className="hr-cmd__label hr-cmd__label--copied">
                <CheckIcon />
                Copied
              </span>
              <span className="hr-cmd__label hr-cmd__label--failed">Select and copy</span>
            </span>
          </Button>
        </div>
        {hint ? <p className="hr-cmd__hint">{COMMAND_HINT}</p> : null}
      </div>

      <p aria-live="polite" className="hr-visually-hidden">
        {status}
      </p>
    </div>
  );
