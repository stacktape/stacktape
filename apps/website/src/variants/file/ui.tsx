 ...
 *
 * None of these behave: a screen is a picture of the wizard or the Console, not the thing itself.
 * The `.astro` files render them without a client directive, so no JavaScript ships for them. They
 * are React wrappers only because an `.astro` template cannot hand a React component an element
 * prop such as `icon`.
 */
...
export function PreviewButton({ variant, icon, iconPosition = 'start', small = false, children }: PreviewButtonProps) {
  return (
    <Button
      aria-disabled="true"
      className={small ? 'fl-btn fl-btn--small' : 'fl-btn'}
...
export function StatusBadge({ tone, dot = false, caps = true, children }: StatusBadgeProps) {
  return (
    <Badge caps={caps} className={dot ? 'fl-badge fl-badge--dot' : 'fl-badge'} tone={tone}>
      {dot ? <i aria-hidden="true" className="fl-badge__dot" /> : null}
      {children}
    </Badge>
  );
}
...
export function OptInSwitch({ label }: { label: string }) {
  return <Switch checked={false} className="fl-optin__switch" disabled label={label} onCheckedChange={() => {}} />;
}
