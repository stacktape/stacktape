 ...
 */
export function CommandBox({ label = 'Get started', hint }: { label?: string; hint?: string }) {
  ...
  return (
    <div className={`ky-cmd ky-cmd--${state}`} data-ky-cmd>
      <div className="ky-cmd__top">
        <Tabs
          appearance="segmented"
          ariaLabel={`${label}: install method`}
          className="ky-cmd__tabs"
          ...
          width="fit"
        />
        {hint ? <p className="ky-cmd__hint">{hint}</p> : null}
      </div>

      <div className={overflows ? 'ky-cmd__row ky-cmd__row--overflow' : 'ky-cmd__row'} id={panelId} role="tabpanel">
        <span aria-hidden="true" className="ky-cmd__prompt">
          $
        </span>
        <code className={long ? 'ky-cmd__code ky-cmd__code--long' : 'ky-cmd__code'} ref={codeRef}>
          {entry.command}
          <span aria-hidden="true" className="ky-cmd__caret" />
        </code>
        <Button aria-label={`Copy command: ${entry.command}`} className="ky-cmd__copy" onClick={copy} variant="primary">
          <span className="ky-cmd__labels" aria-hidden="true">
            <span className="ky-cmd__label ky-cmd__label--copy">
              <CopyIcon />
              Copy
            </span>
            <span className="ky-cmd__label ky-cmd__label--copied">
              <CheckIcon />
              Copied
            </span>
            <span className="ky-cmd__label ky-cmd__label--failed">Select and copy</span>
          </span>
        </Button>
      </div>

      <p aria-live="polite" className="ky-visually-hidden">
        {status}
      </p>
    </div>
  );
