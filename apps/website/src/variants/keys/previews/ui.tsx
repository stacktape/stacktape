 ...
 * render them without a client directive, so no JavaScript ships for them. They exist as React
 * wrappers only because an `.astro` template cannot hand a React component an element prop such
 * as `icon`.
 */
...
export function PreviewButton({ variant, icon, iconPosition = 'start', small = false, children }: PreviewButtonProps) {
  return (
    <Button
      aria-disabled="true"
      className={small ? 'ky-btn ky-btn--small' : 'ky-btn'}
...
export function StatusBadge({ tone, dot = false, caps = true, variant = 'soft', children }: StatusBadgeProps) {
  return (
    <Badge caps={caps} className={dot ? 'ky-badge ky-badge--dot' : 'ky-badge'} tone={tone} variant={variant}>
      {dot ? <i aria-hidden="true" className="ky-badge__dot" /> : null}
      {children}
    </Badge>
  );
}
...
export function OptInSwitch({ label }: { label: string }) {
  return <Switch checked={false} className="ky-optin__switch" disabled label={label} onCheckedChange={() => {}} />;
}
