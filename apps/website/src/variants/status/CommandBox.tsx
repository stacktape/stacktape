...
export function CommandBox({ label = 'Get started', hint = true }: { label?: string; hint?: boolean }) {
  ...
  return (
    <div className={`su-cmd su-cmd--${state}`}>
      <div className="su-cmd__top">
        <Tabs
          appearance="segmented"
          ariaLabel={`${label}: install method`}
          className="su-cmd__tabs"
          ...
          width="fit"
        />
        {hint ? <p className="su-cmd__hint">{COMMAND_HINT}</p> : null}
      </div>

      <div className={overflows ? 'su-cmd__row su-cmd__row--overflow' : 'su-cmd__row'} id={panelId} role="tabpanel">
        <span aria-hidden="true" className="su-cmd__prompt">
          $
        </span>
        <code className={long ? 'su-cmd__code su-cmd__code--long' : 'su-cmd__code'} ref={codeRef}>
          {entry.command}
        </code>
        <Button aria-label={`Copy command: ${entry.command}`} className="su-cmd__copy" onClick={copy} variant="primary">
          <span className="su-cmd__labels" aria-hidden="true">
            <span className="su-cmd__label su-cmd__label--copy">
              <CopyIcon />
              Copy
            </span>
            <span className="su-cmd__label su-cmd__label--copied">
              <CheckIcon />
              Copied
            </span>
            <span className="su-cmd__label su-cmd__label--failed">Select and copy</span>
          </span>
        </Button>
      </div>

      <p aria-live="polite" className="su-visually-hidden">
        {status}
      </p>
    </div>
  );
