/*
 * Everything the "status" page says and shows.
 *
 * The page copy, the seven components, the example project, the screens, the incident history and
 * the testimonials live here so the Astro components and the islands read one source. Nothing runs
 * at import time; the helpers at the bottom are pure functions the screens call from frontmatter.
 */
...
export const SIGN_IN_LINE = { before: 'Already set up?', link: 'Sign in to the Console' } as const;
...
export type Component = {
  ...
  /** The 90-day availability the list shows for this component. */
  uptime: string;
  /** Zero-based day (0 = 90 days ago, 89 = today) that carries an amber tick, if any. */
  incidentDay?: number;
  /** What follows the `Operational` badge in the component's card header, if anything. */
  note?: string;
};
...
/* ── The status page's own lines ───────────────────────────────────────────────────────────── */

export const STATUS = {
  banner: 'All systems operational',
  scope: `${PROJECT.name} · ${PROJECT.stage} · ${PROJECT.region}`,
  state: 'Operational',
  listNote: '90 days · updated a minute ago · billed by AWS, no markup',
  windowDays: 90
} as const;

export const ACTIVITY = {
  title: 'Current activity',
  line: 'Deploying v42 to production',
  progress: '91 %'
} as const;
...
export const HISTORY = {
  ...
  date: 'Wed 3 Sep 2026',
  incidentTitle: 'Uptime check failed · api.acme.com/health · resolved in 11 min',
  none: 'No other incidents in the last 90 days.',
  maintenanceTitle: 'Scheduled maintenance',
  maintenance: 'None. Deploys are zero-downtime.'
} as const;
