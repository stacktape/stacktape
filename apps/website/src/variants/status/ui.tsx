...
export function PreviewButton({ variant, icon, iconPosition = 'start', small = false, children }: PreviewButtonProps) {
  return (
    <Button
      aria-disabled="true"
      className={small ? 'su-btn su-btn--small' : 'su-btn'}
...
type StatusBadgeProps = {
  tone: BadgeTone;
  /** A dot before the word, for states that are happening now. It does not move. */
  dot?: boolean;
  children: ReactNode;
};
...
export function StatusBadge({ tone, dot = false, children }: StatusBadgeProps) {
  return (
    <Badge className={dot ? 'su-badge su-badge--dot' : 'su-badge'} tone={tone}>
      {dot ? <i aria-hidden="true" className="su-badge__dot" /> : null}
      {children}
    </Badge>
  );
}
...
export function RuleSwitch({ label }: { label: string }) {
  return <Switch checked className="su-rule__switch" disabled label={label} onCheckedChange={() => {}} />;
}
