/*
 * Everything the "track" study says and shows.
 *
 * Page copy, the six things Stacktape does, the four things the visitor does, the example project
 * and every screen's numbers live here so wording stays consistent across components. The YAML
 * highlighter at the bottom runs on the server: import this module from `.astro` frontmatter only
 * (the command box island imports `COMMANDS` alone).
 */
...
/** Backticks become a mono chip; everything else is escaped. For copy that names a command. */
export const inline = (text: string) => escapeHtml(text).replace(/`([^`]+)`/g, '<code class="tk-chip">$1</code>');
...
/* ── The four things the visitor does (the left track) ─────────────────────────────────────── */

export type VisitorNode = {
  id: 'init' | 'deploy' | 'review' | 'budget';
  /** First line, 14px/600. Backticks mark the command chip. */
  what: string;
  /** Second line, 13px muted. */
  note: string;
};

export const NODES: VisitorNode[] = [
  { id: 'init', what: 'You run `npx stacktape init`', note: 'It opens the wizard in your browser.' },
  { id: 'deploy', what: 'You read what it decided and press Deploy', note: 'After that, a push ships.' },
  { id: 'review', what: 'You review the fix and approve it', note: 'The agent’s pull request, your call.' },
  { id: 'budget', what: 'You set a budget once', note: 'Alerts arrive before the invoice.' }
];
...
export const POSTURE = {
  ...
  total: 30,
  findings: [
    { text: 'apiService image: 2 critical CVEs in openssl 3.0.13', badge: 'Fix PR opened', action: 'View PR' },
    { text: 'Plaintext STRIPE_KEY in config', action: 'Convert to $Secret' },
    { text: 'mainDatabase backups retained 1 day (staging)', action: 'Fix in config' }
  ]
} as const;
...
export const INCIDENT = {
  ...
  control: 'Copy details for agent',
  timeline: [
...
export const BUDGET = { limit: 150, usedPct: 75, alertPct: 80, alertAt: 120 } as const;
...
const span = (cls: string, text: string) => `<span class="tk-y-${cls}">${escapeHtml(text)}</span>`;
