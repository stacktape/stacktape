...
export function CommandBox({ label = 'Get started' }: { label?: string }) {
  ...
  return (
    <div className={`tk-cmd tk-cmd--${state}`} data-tk-cmd>
      <div className="tk-cmd__top">
        <Tabs
          appearance="segmented"
          ariaLabel={`${label}: install method`}
          className="tk-cmd__tabs"
          ...
          width="fit"
        />
        <p className="tk-cmd__hint">opens the wizard in your browser</p>
      </div>

      <div className={overflows ? 'tk-cmd__row tk-cmd__row--overflow' : 'tk-cmd__row'} id={panelId} role="tabpanel">
        <span aria-hidden="true" className="tk-cmd__prompt">
          $
        </span>
        <code className={long ? 'tk-cmd__code tk-cmd__code--long' : 'tk-cmd__code'} ref={codeRef}>
          {entry.command}
          <span aria-hidden="true" className="tk-cmd__caret" />
        </code>
        <Button aria-label={`Copy command: ${entry.command}`} className="tk-cmd__copy" onClick={copy} variant="primary">
          <span className="tk-cmd__labels" aria-hidden="true">
            <span className="tk-cmd__label tk-cmd__label--copy">
              <CopyIcon />
              Copy
            </span>
            <span className="tk-cmd__label tk-cmd__label--copied">
              <CheckIcon />
              Copied
            </span>
            <span className="tk-cmd__label tk-cmd__label--failed">Select and copy</span>
          </span>
        </Button>
      </div>

      <p aria-live="polite" className="tk-visually-hidden">
        {status}
      </p>
    </div>
  );
