...
export function PreviewButton({ variant, icon, iconPosition = 'start', small = false, children }: PreviewButtonProps) {
  return (
    <Button
      aria-disabled="true"
      className={small ? 'tk-btn tk-btn--small' : 'tk-btn'}
...
export function StatusBadge({ tone, dot = false, caps = true, children }: StatusBadgeProps) {
  return (
    <Badge caps={caps} className={dot ? 'tk-badge tk-badge--dot' : 'tk-badge'} tone={tone}>
      {dot ? <i aria-hidden="true" className="tk-badge__dot" /> : null}
      {children}
    </Badge>
  );
}
...
export function OptInSwitch({ label }: { label: string }) {
  return <Switch checked={false} className="tk-optin__switch" disabled label={label} onCheckedChange={() => {}} />;
}
