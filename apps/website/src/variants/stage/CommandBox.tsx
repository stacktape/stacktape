...
export function CommandBox({ label = 'Get started' }: { label?: string }) {
  ...
  return (
    <div className={`sg-cmd sg-cmd--${state}`}>
      <div className="sg-cmd__top">
        <Tabs
          appearance="segmented"
          ariaLabel={`${label}: install method`}
          className="sg-cmd__tabs"
          ...
          width="fit"
        />
        <p className="sg-cmd__hint">opens the wizard in your browser</p>
      </div>

      <div className={overflows ? 'sg-cmd__row sg-cmd__row--overflow' : 'sg-cmd__row'} id={panelId} role="tabpanel">
        <span aria-hidden="true" className="sg-cmd__prompt">
          $
        </span>
        <code className={long ? 'sg-cmd__code sg-cmd__code--long' : 'sg-cmd__code'} ref={codeRef}>
          {entry.command}
          <span aria-hidden="true" className="sg-cmd__caret" />
        </code>
        <Button aria-label={`Copy command: ${entry.command}`} className="sg-cmd__copy" onClick={copy} variant="primary">
          <span className="sg-cmd__labels" aria-hidden="true">
            <span className="sg-cmd__label sg-cmd__label--copy">
              <CopyIcon />
              Copy
            </span>
            <span className="sg-cmd__label sg-cmd__label--copied">
              <CheckIcon />
              Copied
            </span>
            <span className="sg-cmd__label sg-cmd__label--failed">Select and copy</span>
          </span>
        </Button>
      </div>

      <p aria-live="polite" className="sg-visually-hidden">
        {status}
      </p>
    </div>
  );
