...
/*
 * The product's own controls, rendered on the server inside the windows.
 *
 * None of these behave: a window is a picture of the product, not the product. The `.astro` files
...
/** A button that looks pressable and is not: windows never navigate or submit. */
export function PreviewButton({ variant, icon, iconPosition = 'start', small = false, children }: PreviewButtonProps) {
  return (
    <Button
      aria-disabled="true"
      className={small ? 'ss-btn ss-btn--small' : 'ss-btn'}
...
export function StatusBadge({ tone, dot = false, children }: StatusBadgeProps) {
  return (
    <Badge className={dot ? 'ss-badge ss-badge--dot' : 'ss-badge'} tone={tone}>
      {dot ? <i aria-hidden="true" className="ss-badge__dot" /> : null}
      {children}
    </Badge>
  );
}
...
export function RuleSwitch({ label }: { label: string }) {
  return <Switch checked className="ss-rule__switch" disabled label={label} onCheckedChange={() => {}} />;
}
