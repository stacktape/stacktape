/*
 * Everything the "session" page says and shows.
 *
 * The page is one terminal transcript, so besides the shared copy, the example project and the
 * screens, this file holds the transcript's own lines: the working directory in the prompt, the
 * timestamps that open each block, the commands the session runs and what they print before a window
 * opens. Nothing runs at import time; the helpers at the bottom are pure functions the screens call.
 */
...
export const SIGN_IN_LINE = 'already set up? sign in →';
...
/** The title bar's tabs, named the way a shell would name them. */
...
export const NAV_ACTIONS = {
  signIn: { label: 'sign-in', href: LINKS.console },
  demo: { label: 'book-a-demo', href: LINKS.demo },
  start: { label: 'get started', href: LINKS.getStarted }
} as const;
...
export const FOOTER_GROUPS = [
  ...
  },
  {
    ...
    links: [
      ...
      { label: 'ci-cd-and-gitops', href: 'https://docs.stacktape.com/ci-cd-and-gitops/overview' },
...
export type SectionId = 'designs' | 'packages' | 'deploys' | 'monitors' | 'secures' | 'incidents' | 'costs';
...
const full = (url: { host: string; path: string }, scheme = 'https') => `${scheme}://${url.host}${url.path}`;

/* ── The transcript's own lines ────────────────────────────────────────────────────────────── */
 ...
 * the CLI's own or the transcript's shorthand; the timestamps jump as the session continues across
 * days.
 */
export const SESSION: Record<
  SectionId,
  { stamp: string; command?: string; opening?: string; error?: string; alert?: string; resolved?: string }
> = {
  designs: { stamp: 'Mon 09:32', opening: full(URLS.wizard, 'http') },
  packages: { stamp: 'Mon 09:35', command: DEPLOY_COMMAND },
  deploys: { stamp: 'Mon 09:38', command: 'git push origin main', opening: full(URLS.deployments) },
  monitors: { stamp: 'Tue 09:41', command: 'stacktape console monitoring', opening: full(URLS.monitoring) },
  secures: {
    stamp: 'Tue 16:10',
    command: 'stacktape deploy --stage staging',
    error: 'deploy to staging blocked · mainDatabase would get a public address',
    opening: full(URLS.guardrails)
  },
  incidents: {
    stamp: 'Wed 14:02',
    alert: 'incident opened · uptime check failed · api.acme.com/health',
    resolved: 'resolved 14:13 · 11 min'
  },
  costs: { stamp: 'Tue 30 Sep 23:59', command: 'stacktape costs --stage production', opening: full(URLS.costs) }
};
...
export const PACKAGE = {
  phases: [
    { name: 'Initialize', time: '2 s' },
    { name: 'Package', time: '58 s' }
  ],
  rows: [
  ...
  next: { name: 'Deploy', note: 'starting…' }
} as const;
