...
export const cfLogicalNames = {
  ...
  httpApiAuthorizer({ method, path, stpResourceName }: { method: string; path: string; stpResourceName: string }) {
    return buildCfLogicalName({
      stpResourceName: '',
      specifier: {
  ...
  httpApiRoute({ method, path, stpResourceName }: { method: string; path: string; stpResourceName: string }) {
    return buildCfLogicalName({
      stpResourceName: '',
      specifier: {
        type: `${stpResourceName}-${method === '*' ? 'Any' : method}-${path === '*' ? 'Default' : path}`
  ...
  identityProvider(stpResourceName: string, type: string) {
    return buildCfLogicalName({
      stpResourceName,
      specifier: { type },
...
const buildCfLogicalName = ({
  ...
  stpResourceName?: string;
  specifier?: { type: string; typeIndex?: number; subtype?: string } | undefined;
  suffix: {
