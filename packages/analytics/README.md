...
This package owns the versioned vocabulary shared by the public sites, CLI, Console UI, and Console API. It contains no
SDK client and no application lifecycle code. Each application owns initialization and delivery for its runtime.
...
- Expected validation, authentication, authorization, user-code, and cloud-provider errors remain product outcomes; only
  unexpected application failures are error-tracking issues.
...
Production traffic uses Stacktape's managed first-party PostHog proxy. That proxy is bound to the production project, so
non-production projects use PostHog's direct EU ingestion host to preserve project isolation.
