...
import { ConfigDiagram } from './ConfigDiagram.js';
...
test('renders the default empty state when no normalized config is available', () => {
  const markup = renderToStaticMarkup(<ConfigDiagram config={null} />);
...
test('lets a compiling host suppress the otherwise misleading empty state', () => {
  expect(renderToStaticMarkup(<ConfigDiagram config={null} emptyState={null} />)).toBe('');
});
...
test('the public component derives and renders a scene from normalized configuration', () => {
  ...
  const markup = renderToStaticMarkup(<ConfigDiagram config={config} ariaLabel="Fixture architecture" />);
