...
const parseViteOutput = (output: string): BuildOutputInfo | null => {
  ...
  if (sizeLines?.length) {
    ...
    sizeLines.forEach((line) => {
      ...
      if (size !== undefined && !Number.isNaN(size)) totalKB += size;
      if (gzip !== undefined && !Number.isNaN(gzip)) totalGzipKB += gzip;
    });
...
const parseAngularOutput = (output: string): BuildOutputInfo | null => {
  ...
  if (chunkMatches?.length) {
    ...
    chunkMatches.forEach((match) => {
      ...
      const [size, transferSize] = sizes ?? [];
      if (size !== undefined && transferSize !== undefined) {
        totalKB += Number.parseFloat(size);
        transferKB += Number.parseFloat(transferSize);
      }
    });
    info.totalSize = `${totalKB.toFixed(2)} kB`;
...
const parseVueOutput = (output: string): BuildOutputInfo | null => {
  ...
  if (sizeMatches?.length) {
    ...
    sizeMatches.forEach((match) => {
      ...
      const [size, gzipSize] = nums ?? [];
      if (size !== undefined && gzipSize !== undefined) {
        totalKB += Number.parseFloat(size);
        gzipKB += Number.parseFloat(gzipSize);
      }
    });
    info.totalSize = `${totalKB.toFixed(2)} kB`;
