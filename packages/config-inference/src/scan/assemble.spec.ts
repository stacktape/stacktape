...
describe('assembleCandidateFacts', () => {
  ...
  it('cites real lines, so the provenance UI can point at the user own code', async () => {
    const repoRoot = await makeRepo({
      'package.json': JSON.stringify(
        { name: 'api', scripts: { start: 'node index.js' }, dependencies: { pg: '^8.0.0' } },
        null,
        2
      ),
      'pnpm-lock.yaml': ''
  ...
  it('identifies a managed provider without disclosing the host', async () => {
    ...
    expect(facts.dependencies[0]).toMatchObject({ kind: 'postgres', currentlyHostedOn: 'supabase' });
  ...
  it('treats a Redis-backed queue library as proving both the queue and the Redis', async () => {
    const repoRoot = await makeRepo({
      'package.json': JSON.stringify({
        name: 'worker',
    ...
    expect(facts.dependencies.map((dependency) => dependency.kind).toSorted()).toEqual(['queue', 'redis']);
  });
  ...
  it('reports a migration tool without inventing when it runs', async () => {
    ...
    expect(facts.migrations[0]).toMatchObject({ tool: 'prisma', runsAt: 'unknown' });
  });
